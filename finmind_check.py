...
def main():
    ...
    if any_paid and paid_tok:
        ...
        for label, url in variants:
            ...
            # 🚨 只有 A~F 才是「一次拿到全市場**所有**分點」。
            #    ⛔ G 不算 —— 它是「**一家**券商 × 全市場」,要湊齊全市場得每家券商各打一次。
            #    (2026-08-30 第一版把 G 也算進來,結論寫成「245 次呼叫」→ 錯了 2 個數量級。
            #     ⭐ 教訓:探針的自動結論也要驗,「回 200」不等於「回的是我要的東西」。)
            if st == 200 and rows > 50 and label[0] in 'ABCDEF' and best is None:
                best = label
        print()
        if best:
            print(f'   ⭐ **有可用的全市場批次寫法:{best}** → 1 年回算只要約 245 次呼叫,'
                  '把 `_fetch_chips_bulk` 改用這個寫法即可。')
        else:
            print('   🚨 **「一次拿全市場所有分點」的寫法全部不通**(A~F 都回 data_id 不可為空)')
            print('      → 照「每檔股票各打一次」算:245×2,653 ≈ 65 萬次 = 108 小時,⛔ 不可行。')
            print('   ⭐⭐ **正解是換一個軸:按「券商」抓,不是按「股票」抓**(上面的 G)。')
            print('      實測分點淨額極度集中(400 檔抽樣、809 家分點):')
            print('        前 30 家 → 覆蓋 86.2% ・1 年 7,350 次 ≈ 1.2 小時')
            print('        前 200 家 → 覆蓋 98.0% ・1 年 49,000 次 ≈ 8.2 小時')
            print('      → `scripts/chips_backfill.py` 就是走這條。')
            print('   ⚠️ 若對照組(八大行庫)是 ✅ 而分點 A~F 全紅 → 那是**這個資料集**不給省略 data_id,')
