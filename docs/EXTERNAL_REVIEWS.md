# StockAI-DB 外部參考資料評估紀錄

> 從 `CLAUDE.md` 搬出來的**決策紀錄**(2026-09-09,一個字都沒刪)。
> ⛔ 這裡是「當時做了什麼、為什麼」的完整敘事;**現行規則**仍以 `CLAUDE.md` 為準。
> ⭐ 搬出來的原因:CLAUDE.md 原本 50.7 萬字元(約 38~50 萬 token),超過 context window,
> 每個 session 一開場就把額度吃光 → `Prompt is too long`。

---

### 📚 外部參考資料的評估紀錄㊱:k66inthesky/tw-stock(2026-09-25 使用者提供,「檢視內文有沒有可用資料,另外有歷史股市資料可以拿來回測」)
- **是什麼**:證交所 STOCK_DAY 爬下來的**上市** 997 檔日 K(2010-01-04 ~ 2023-12-29,約 288 萬列)+ 爬蟲 `get_history.py`;README 寫的回測程式與 get_ids 是付費贊助才給(repo 裡沒有)。沒有授權聲明 → ⛔ **資料不進本 repo**,只在暫存資料夾用(`scripts/merge_k66_history.py` 只放轉接程式)。
- **品質(實測)**:跟本站資料重疊的 2021-01 ~ 2023-12 共 669,744 列,**94.72% 收盤逐位相同**(同一個來源);其餘是本站自己的調整(整數倍分割還原 / 約 200 檔有 0.93~0.99 的比例漂移)。⚠️ 它是**原始價**:552 次超過漲跌停的單日跳動(減資 / 除權 / 分割沒還原)→ 回頭調整 1,417 次後只剩 3 次。
- **沒有的**:上櫃、ETF(沒有 0050)、指數、下市股(只有 2023 年底還活著的 → 倖存者偏誤)。
- **用在哪**:接到本站 K 線前面,把回測從 2022 往前推到 **2011**(多了 2011 / 2015 / 2018 三次空頭與 2020 崩盤)→ 2011~2020 成了現有策略的**真樣本外**。結論見 docs/DECISIONS.md V77.6.2。
- ⏳ **還缺的**:2021-09 以前的**真加權指數**與 **0050**(沙箱連不到證交所 / Yahoo)→ 大盤先用等權代理;要補得在 GitHub Actions 抓(FinMind 有 TAIEX 報酬指數與 0050),那要改 workflow → 先問使用者。

### 📐 外部參考資料的評估紀錄㉟:20 份「ATR」逐字稿(2026-09-24 使用者上傳,「檢視 ATR 策略有沒有什麼可以精進的,附件有什麼值得做的,還是有什麼我沒想到的」)

⚠️ 20 份裡 **9 份不是交易**(ATR 42/72 飛機 ×6、光譜 ATR、饒舌 MEMO、GTA6 閒聊),1 份沒有內容(Understanding ATR)。
交易相關 10 份(外匯 / 美股 / 加密,多數是 YouTube 教學)約 14 條主張,逐條對表:

#### A. 早就測過(⛔ 不重測)
| 逐字稿主張 | 本站實測 |
|---|---|
| 停損 2×ATR、目標 2:1 風報比 | `rr2`/`rr3`、固定停利 tp10~20 全輸(V74.4.8「封頂 = 砍贏家」)|
| ATR 通道碰上軌就停利 | 同上,固定停利家族 |
| 吊燈出場(Chandelier)K 值 | 已是設定選項 chand2;K=1.5/2/2.5/3 正式配置是孤峰(V74.3.8);17 條路徑總獲利跟唐奇安分不出來、**回撤 17/17 較小**(V77.4.9)|
| 動態 ATR 吊燈 / ATR 收緊停損 | chandd2.5 +448 萬;atrt2/3 錢卡死(V74.4.8)|
| 低 ATR = 盤整、之後會突破 | TTM 擠壓六關 0 過(V74.6.3);ATR 壓縮→擴張 12 格只 1 格 = 孤峰(accel_probe)|
| 高 ATR = 反轉訊號 | 方向相反 —— 🧬 就是「高波動才買」,六關全過 |
| Supertrend | indicator_zoo 48 個指標六關 0 過 |
| 依風險算張數(帳戶 1%/2%)| V73.0.1:資金只用 59%、回撤沒變小(⚠️ 當時的總獲利數字另有 bug,見 V77.5.9)|
| ATR 金字塔加碼 | ㉝ 已拒(違反 V73.0.0 一天 2 檔固定部位)|
| 10% ATR 當停損 | 太緊;所有「提早出場」都輸 |
| 收盤前 20~30 分量 ATR | 本站本來就尾盤進場(V72.9.0);`_exitDistance` 用今天的 ATR14 |
| 當沖 ATR 停損/進場 | V77.5.0 當沖規則扣成本全負 |

#### B. 沒測過 → 這一版測了(結論見 DECISIONS V77.5.9)
| 主張 | 結果 |
|---|---|
| 依 ATR 調整部位(波動大少買)| ⛔ 比隨機調整部位還差(唐奇安 451 vs 安慰劑 530 萬)|
| 突破 K 要 1~2 倍 ATR 才可靠 | ⛔ 沒有一桶六關全過,1~1.5 倍反而偏弱 |
| ATR 天數依風格調(7/10/20/50)| ATR 追蹤出場:14~20 天可以,7、10 天崩 |

#### C. ⭐ 我們自己沒想到的(逐字稿都在講「停損放哪」,照出本站停損「賣在哪」從來沒量過)
回測把收盤跌破的停損記成剛好賣在停損價;自動下單用現價、智慧單盤中觸價 —— 三種做法。改用做得到的賣價:
唐奇安 20 日 49 個月 **526 → 107(收盤)/ 205 萬(盤中碰到)**,同期 0050 約 264 萬 → 見陷阱 #45。

---

### 🎣 外部參考資料的評估紀錄㉞:「股王釣魚選股攻略 v13」兩張截圖(2026-09-24 使用者上傳,「為什麼附件釣起來的魚隔天漲停那麼準」)

同一套外部 App 的第二批截圖(第一批見㉙)。兩張:① 族群輪動雷達(5 日 × 20 日四象限,泡泡大小 = 波動)② 🎣 CATCH 卡(凌華 6166:收盤 132、+4.35%、排名 #28、「強力持有」信心度 30%、籌碼面 70/100、AI「老漁夫評語」)。

| 截圖上的東西 | 本站判定 | 依據 |
|---|---|---|
| 魚缸標題「台股**漲幅**排行」 | 🚨 **「準」的真因** —— 畫面上的魚 = 今天已經漲的 | `pond_probe`:今日漲幅前 60 每天平均 **6.02 條**隔天漲停、**96.1%** 的日子至少一條(隨便抽 60 條也有 1.25 條、62%)|
| 「隔天漲停」 | ⛔ 碰得到 ≠ 賺得到 | 隔天**上漲**機率 47.4% vs 隨便抽 46.0%(= 一樣);**34.6%** 今天就鎖死買不到;隔天漲停的魚只有 **28.3%** 今天買得到;買得到的收盤買扣成本 **−0.51%**(比隨便抽 −0.42% 還差)|
| 5 日 × 20 日四象限(「轉強↗短期發動」) | ⛔ 不做 | 「轉強」象限 = 加速度;`sector_rotation_probe` 各窗口 −0.88~+0.20pp、5~10 日窗口為負(pro.html 已寫明)|
| 信心度 30% / 籌碼面 70/100 | ⛔ 不做 | 融合分數(陷阱 #38);而且它自己寫的信心度只有 30% |
| AI 老漁夫評語 | ⛔ 不做 | App 內 AI 不上網、`ailevel_probe` 無預測力 |
| 連買 2 日徽章 | 已測 | `fstreak_probe`:外資連買本身六關 0 過(投信同步才過)|
| 排名 #28(成交額) | ✅ 已有 | `kingpool_probe`:前 50 六關全過 → 本站 👑 池子 |

⭐⭐ **結論:它不是準,是分母沒寫出來**。任何「每天從強勢股裡放 60 條」的魚缸都會天天看到幾條漲停,
人只記得那幾條。本站做的不是「也讓畫面看起來準」,而是**把分母與對照組印出來**(📏 ④ 紀律卡 + 🧺 計分板,V77.5.8)。

---

### 🐢 外部參考資料的評估紀錄㉝:17 份唐奇安通道/海龜交易法逐字稿(2026-09-24 使用者上傳,「檢視唐奇安策略有沒有什麼可以精進的」)

17 份 YouTube 轉錄逐份讀完(唯讀解到 scratchpad,⛔ 沒有放進 repo)。來源:TradingView 過擬合驗證流程、ACY 證券 MT4 指標教學、
Royal 老師兩場晚間直播(6K 反轉策略、通道交易法)、「最古老的唐奇安通道現在還有效嗎」CCI 版、Richard Donchian 20 條交易指南(兩份重複)、
獨大兩支(抓行情發動點/期貨實戰)、老貓海龜歷史+TradingView 策略測試器實測、斐波那契加倉、GKFX 海龜核心、Royal 語錄、
增強版唐奇安策略(程式碼實作)、CCI+海龜實戰、BIAS/WR/PSY 速覽。

| 類 | 條目 |
|---|---|
| **A 進場端內容(跟本站「唐奇安當出場」是不同應用)/ 已測過** | 20/10、55/20 兩組經典海龜突破參數(進場)・CCI 超買超賣反轉過濾(進場濾網)・費波那契 0.382/0.5/0.618 回撤進場・「Base K + 前高連線」趨勢線破位反轉進場(左側進場)・假突破反轉(已在 `_SIGNAL_EDGE` 涵蓋)・跌破 21 日低後 8-10 天均值回歸統計(進場端的等待時間、非出場;時間停損類概念已在 `crash_exit_probe`/`portfolio_backtest` 系列否證)・Donchian 20 條交易指南(定性原則,已融入既有鐵則)・人物傳記・BIAS/WR/PSY 其他指標速覽・銷售話術類(WinSmart/獨大)無嚴謹回測數字的示範 |
| **B 海龜經典 ATR 金字塔加碼** | 每漲 1N(1 ATR)加碼一次、初始停損 2N、最多疊 4 單。⛔ **不建**:這是部位大小/槓桿邏輯不是出場,直接衝突 V73.0.0(27 種變體實測「一天固定 2 檔・固定金額」最穩定),而且無法套進現有「每天最多買 2 檔・每筆固定金額」組合回測骨架(部位大小會依賴進場後走勢,兩種回測架構不相容) |
| **C 不可量化** | 「下跌角度 >70°、反彈角度必須 >30°」的反轉確認。⛔ 角度依賴螢幕座標尺度(x/y 軸怎麼縮放,角度就跟著變),同一段走勢換張圖就不一樣,寫不成客觀可回測的規則 |
| **D 值得做,已測** | 唐奇安「中軌」出場(增強版唐奇安策略那份提出):出場線 = 前 N 日最高與最低的**平均值**,比現行下軌(`don20`)鬆、出場更早。本站首次測。 |

#### D:`donmid20`(唐奇安 20 日中軌)已測,**不採用**
`lib_exitsim.mjs` 加 `donmid{N}` 規則(唯一一份出場模擬器)+ 用**現成的** `breakout_exit_probe.mjs`(只把 `donmid20` 加進規則清單,判定邏輯一行沒改)重跑
(2,136 檔・15,533 個「創 60 日新高」事件・含 2022 空頭):**六關全過(是真訊號,不是雜訊)、贏 sham +0.73pp,但每趟贏基準只有 +0.68pp,
在 12 種出場裡排第 10,遠輸現行預設 don20 的 +1.43pp**(平均只抱 13.2 天 vs don20 的 25.9 天)。
⭐ 符合 `breakout_exit_probe` 早就證明的「砍得越早賺越少、且單調」規律,中軌只是再驗證一次同一條規律,不是意外。
⛔ 不換預設(V75.0.9)、⛔ 不加進 App 可選的出場規則(在每一個軸都輸給至少三種既有選項,不是取捨型,加 UI 選項只是多一個沒人會選的死選項)。
數字見 `docs/DECISIONS.md` V77.5.6。

🚨 **這次的價值在於篩選**:17 份裡 16 份是本站已知/不適用的東西,真正新的定義明確的變體只有 1 個。
⭐ 收到一批新資料時,先分類(A 進場端/已測 / B 架構不相容 / C 不可量化 / D 值得測),⛔ 不要每一份都當成待辦逐項照做。

---

### ⚡ 外部參考資料的評估紀錄㉜:22 份當沖/極短線逐字稿(2026-09-22 使用者上傳,「不要漏掉了」)—— ⭐ **「今天做多/做空」的規則扣完成本全部站不住,唯一站得住的是「收盤鎖漲停的持股留到隔天」**

22 份全部逐份讀完(兩個代理各 11 份,最長的 41KB 那份分四段讀)。來源:朱家泓開盤四法、Mokka(下單前看風向 / 五檔與成交明細 / 勝率 vs 風報比 / 神速選股 / 練盤感)、權證小哥直播與鎖漲停隔日沖、阿Sir 籌碼極短線、Larry Williams 短線秘訣(Oops / 攻擊日)、A股雙 60 / 60 分K 戰法、散戶 7 口訣、SMC/ICT 1 分K、NQ 期貨、加密貨幣日內、學吧當沖入門兩份、以小搏大片段、美股短線心得。

| 類 | 條目 |
|---|---|
| **A 早就測過** | 連次量/買盤竭盡(#3,volseq/volstall 兩次)・開盤跳空追多(gap_probe)・ORB 突破 / 第二根過高(orb_probe、intraday_probe)・ADX / TTM Squeeze / MACD / FVG / SMC 類(indicator_zoo、pdf_daytrade、intraday_probe)・布林沿軌(kobo)・分點跟單(broker 三支)・漲停預測/買不到(limitup)・買陰不買陽 / 高位橫盤 / 盤整不交易(maxim_probe)・跌停後紅K(✅)・跳一檔回本(`_dtCostGate` 已上線)・時段口訣(kbar5_tod)|
| **B 沒測過 → 這一輪測了** | 日K:Oops、攻擊日、連續跌停打開、主投同買/同賣、投信連 5 買、起漲第一天、高檔爆量黑K 隔天空、漲停隔天依月線、開高空/開低買、流動性池、大盤開低、盤前分數套個股 ・ 5 分K:開盤四法 8 組合、大盤順風、高低點時段、收盤磁吸、尾盤拉漲停、勝率 vs 期望值 ・ 1 分K:跳空的進場時點 |
| **C 量不到** | 五檔掛單 / 成交明細 / 5 秒內外盤(#3 #6 #7 #17 #18)・盤中權證流(#3)・族群龍頭帶動(#14,量前 80 常缺同族)・NDN 指標沒有公式(#9)・付費沒公開的(#14 139 名單 / #16 出場 / #17 掛價 / #18 四形態 / #21 進場)・處置股黑K 70~80%(#3,官方名單只存了 3 個月)|
| **D 不做** | 心理與資金管理(每日虧損上限、不攤平、固定金額、練盤感三週計畫)—— 收進說明但不是訊號;加密 / NQ / A股原樣不測,改用台股代理 |

⭐ **每一條的數字**見 `docs/DECISIONS.md` V77.5.0。最值得記住的三件:
1. **「勝率 80%」教學的真相**(#11 #16 #21):同一種進場只換停利停損,勝率 19%~74% 隨你調,**16 格期望值全負**。#21 自己舉的「90% 勝率仍賠錢」正是這個。
2. **開盤四法**(#16,22 份裡最完整、最像規則的一份)照做 −0.28%/趟,**跟不分類一律照第一根做差不多** → 分類沒有帶來東西。⚠️ 他說出場規則在付費班才教 → 我們用的是他公開的那一版(停損第一根、停利 2%、碰漲停賣)。
3. **#12「鎖漲停就留」是對的**,而且是 22 份裡唯一六關全過的:月線上彎隔天開盤賣 +2.19%(扣 0.44%)、勝率 69.7%。⚠️ 但他的另一半「月線下彎隔天要賣」→ 下彎那組抱到收盤反而略好。⛔ 買不到鎖住的漲停 → 只給已經有的人。

🚨 **方法學收穫**(寫進 LAB method):當沖回測一定要 ① 流動性分層 ② 拿掉最好 10 天 ③ 用分K 驗晚 1/5 分鐘進場 —— 「開高就空」三道檢查全部被打掉(冷門股 +0.85% → 1 分K 晚 1 分鐘只剩 +0.02pp)。
🚨 **推翻了一個本站以前的讀法**:盤前分數偏多那天「加權開盤買收盤賣 +0.28%」,套到熱門個股是 **−0.27%** → 大盤層級的結論要在個股重跑。

---

### 💎 外部參考資料的評估紀錄㉛:《不盯盤、不看線圖,50萬滾出50億》「價值 × 成長 × 週期」54 節開發規格 v1.0(2026-09-21 使用者貼上)

使用者貼了 54 節規格(資產價值 PB×ROE 三級 / 收益價值 12 季趨勢與五感 / 週期價值景氣底部反轉 / 安全邊際 P25-P75 /
X 光機 / 風險排除 / 價值陷阱 / 催化劑 / 5~20 年回測 / walk-forward / 參數敏感度 / Dashboard / VALUE_FISHING_POOL),
結尾指示「先建資料模型與三個核心 strategy module,不要先做 UI;完成後先跑 unit tests / data validation / look-ahead bias test /
basic backtest,再進行 Dashboard」。照 ㉙/㉚ 的流程:**A 早就做了或測過 / B 沒測過 → 探針 / C 量不到 / D 不做**。

#### A 早就有(或已證偽)
| 規格 | 本站 |
|---|---|
| §6 12 季趨勢(Revenue/GM/OM/EPS/FCF 各 YoY/QoQ/趨勢) | `_finTrend`(算 12 季顯 8 季)+ `_FIN_MARK_BASE` 三種事實標記;⚠️ 營益率 `opi` 要等回算 |
| §8 Revenue acceleration | `accel_probe` 六關全過 +0.82pp,V77.4.3 決策台開關(預設關) |
| §14/§15 多重估值 vs 自身 / 產業 | PE 五分位**單調反向**、比同業便宜 +0.01、PEG<1 +0.06、`pe_band` 位階已在 §15 |
| §16 安全邊際 P25/中位/P75 三情境 | `_rpSensitivityHtml`(9 格,⛔ 沒有機率欄)+ `_rpValRows` |
| §17 X 光機 | `_xrayScoreOf` 10 維 + `_lossCapOf` 虧損封頂 |
| §18/§36 風險排除 | 處置 / 興櫃 `_offListed` / 虧損 / 面額變更 `par_chg_q`;⛔ 負債比/利息保障/關係人**沒有資料** |
| §19 價值陷阱 | 低 PE×營收衰退 −0.47、便宜×跌深 −1.07/−3.35 六關全過(負向)—— LAB r:36 避雷 |
| §20 催化劑 | 法說會 `confcall.json`、除息、財報法定日;重大訊息 = V77.4.3 MOPS 探針(等 Run) |
| §24/§46 不做總分 / 目標價 / 機率 | 本站鐵則(陷阱 #38) |
| §30 零前視 | 可用日一律 `pubDate`(法定截止日,更保守);倖存者偏誤已記 |
| §34/§35 walk-forward / 參數敏感度 | 六關的前後半 + 逐年 + 去最好年;門檻高原檢定 |
| §32 vs 加權 vs 0050 | `portfolio_backtest` 印 twiiRet / ret50;`hold_switch_probe` |

#### B 沒測過 → `scripts/value_probe.mjs`(結論見 `docs/DECISIONS.md` V77.4.4 與 LAB)
- **PB × ROE 交叉**:✅ 唯一站得住(主設定六關全過 +4.27pp、贏 sham +3.40、16 格高原),⚠️ 但去重改 20/120 日就 5/6(2025 轉負)= 邊緣;疊在決策台上通過率 1.11% → 觀察池,⛔ 不換預設。
- **四項同步改善**:+1.39 5/6(逐年沒過);⭐ 規格 §7「低品質成長」(只營收好)**−1.65** → 品質有資訊。
- **個股自身低谷反轉**:⛔ 全負(−1.24 / 只低谷 −1.99),而且同一條判式套非循環股反而 +1.28 → **「循環」標籤沒有資訊**;寬版六關全過但 vs sham = 隨機。
- **價值陷阱**:vs 同樣便宜 −2.50、惡化項數單調;2022 沒過 → 避雷。

#### C 量不到
Net Cash / TBV / EV/EBITDA / 負債比 / 利息保障(fin_deep 十欄一個都沒有)・5/10/15/20 年與 2010 起訓練段(最長 4.8 年)・實際公告日(只有法定截止日)・營益率 `opi` 未回算。

#### D 不做
§5 五感(質化 → 🔎 免費查)・§49 `/strategy /backtest /data /report` 模組樹(第二份真相;本站已有 lib + 探針 + 回測濾網的骨架)・§43 資產魚/成長魚/週期魚三池(只有站得住的那一個做成池)。

#### 🚨 規格本身要更正的 7 處
1. 「PB<0.5 ∧ ROE>60%」規格自己說只是公開文章示例;本站實測 **PB 單獨零邊際、低 PE 單調反向 → 估值只能避雷⛔ 不能選股**(已定案);這次唯一的新發現是「PB 與 ROE 交叉」才有東西。
2. 「景氣低谷 = 買點」:`doi_probe` 存貨/資本支出谷底六關 0 過、這次自身營收低谷 −1.99 → **低谷之後還會更低**,方向相反。
3. 「便宜可以便宜很久」規格說對了:便宜×跌深、便宜×三項惡化都是負的 → 那是避雷不是進場。
4. 5/10/15/20 年回測與 2010 起訓練段做不到(K 線 2021 起、加權 2021-09 起、財報 2018 起)。
5. `publication_date` 本站只有法定截止日 → 只能做「可用日之後」,做不了「公告後 N 天內」。
6. 「12 季同步改善」照字面幾乎空集合(連續 ≥4 季就只剩 348 筆而且是負的)→ 台股調適成連續 ≥1/2 季。
7. Tier A/B/C 是研究優先序不是評級 —— 跟本站「⛔ 不顯總分」一致,但每一池仍要標實測狀態(`test_fishrod ③`)。

⭐ 通用(第三次):**外部規格的價值在「它點名了哪些沒測過」,不在它的結論** —— 這份 54 節裡真正沒測過的只有三件,而三件裡只有一件站得住、還是邊緣的。

---

### 🚀 外部參考資料的評估紀錄㉚:「台股大漲股捕捉引擎」62 節開發規格(2026-09-21 使用者貼上)

使用者:「上述請檢視有無錯誤,需要回測就回測,要挖礦就挖礦,有更好的辦法就做」。⭐ 照本站規矩先查登記表,再只測沒測過的。

### A. 早就做了 / 測過的(⛔ 不重做)
| 規格章節 | 本站 | 證據 |
|---|---|---|
| Trend Engine(拋竿保留、每日 2 NEW ENTRY、persistence) | ✅ `_recoPicks` = 決策台那套(49 個月 +589.8 萬 vs 0050 +264.1 萬)、`_castStickyHtml` | `_DECK_TRACK49`、V73.0.0、V75.1.0 |
| 突破 20/60/120 日高 | 20/60/252 已測:nh20 −0.72 / nh60 +0.13 / nh252 +1.47(六關沒全過) | `_SCR_EDGE` |
| 壓縮→擴張(布林 / TTM) | 布林壓縮全表最差 −0.89(已進黑名單)、TTM squeeze 六關 0 過 | kobo_probe、V74.6.3 |
| 週轉率 | 水準測過三次,V77.3.3 高週轉沒贏過 sham | turnover / turnover_stage / turnover_edge |
| flag 共振 / Lifecycle 狀態機 | 疊加型三次全負(chain −0.84、td_wt 聖杯、六脈只 🔴)、狀態機七態六種扣成本負 | chain_probe、td_wt_probe、regime_probe |
| Regime / 產業確認 / 廣度 | MA60 五態矩陣(V74.9.1)、板塊內最強 +0.90、板塊 20 日動能 +1.44;廣度只有 250 天 | regime_matrix、sector_pick、sector_rotation |
| 「41.4 vs 36.4 不可直接認定」/ 倖存者偏誤 / 成本 / 漲停買不到 / ⛔ 綜合分數 | 每關都有 z 檢定 + 前後半 + 逐年 + 高原 + sham;其餘全部早有 | kingpool `gates()`、陷阱 #38 |

### B. 沒測過 → `scripts/accel_probe.mjs`(結論見 DECISIONS V77.4.2)
七個 flag 只有**營收 YoY 加速**站得住(六關全過、贏 sham、絕對超額轉正、高原 4/4);RS 加速 / 量能擴張 / ATR 壓縮擴張 / 位階剛衝進 / 突破品質 / 產業前 5 疊在 🧬 上**沒有一個**贏過 sham;共振越多越差;5,480 個大漲起點**沒有前兆**。🔄 疊在 49 個月組合回測上:總獲利 +473 萬 < 同通過率隨機抽 +538 萬 < 不濾 +519 萬 → ⛔ 不換預設(取捨型:勝率 +2.4pp、回撤減半、少賺 65 萬 → 選項)。

### C. 量不到(資料)
Catalyst 整層(news_hist 19 天、confcall ±60 天、MOPS 零資料集)・財報實際公告日(只有法定截止日)・10 年 walk-forward(K 2021 起)・月營收公布日(mrh 只有 12 個月無日期)。

### D. ⛔ 不做
Lifecycle 六態(狀態機,建在這組 flag 上沒有東西)・綜合分數(規格自己也禁,同陷阱 #38)・新開 MOPS 採礦(要先問)。

### 🚨 規格本身的 10 個錯誤
① 「既有六道門檻」是回測關卡不是選股濾網 ② 「成交額前 100」是 `FISH_MAX` 顯示上限,實測是前 50 ③ 10 年 walk-forward 資料不夠 ④ 「公告日新鮮度」本站只有法定截止日 ⑤ Catalyst 零歷史 ⑥ Lifecycle 狀態機類實測全負 ⑦ NEW ENTRY + persistence 早有 ⑧ z 檢定、倖存者偏誤早就做了 ⑨ Big Winner 是事後最高點型(dtflip:碰得到 ≠ 賺得到)⑩ TR5/TR20 擴張比分母抵銷 = 量能比。

### ⭐⭐ 規格最大的盲點
把「捕捉大漲」當目標。本站四支探針一致:大漲**測得到、賺不到、而且沒有前兆**(limitup / dtflip / 這支 Miss Rate 與 Big Winner)。能賺的是已經動起來之後的那一段,而那一段本站早就有了(🧬 + 出場)。「有更好的辦法」= 把七個 flag 裡唯一站得住的那個(營收加速)丟進 49 個月組合回測跟 sham 比 —— 那才是 V75.0.9 換預設的判準。

## 👑 外部參考資料的評估紀錄㉙:「股王釣魚選股」七層漏斗 + React 追蹤介面(2026-09-21 使用者上傳)—— ⭐ **七層裡只有「成交額前 50」站得住,而且它單獨就夠了**

使用者上傳 190KB 的 React 介面(初篩池 XQ 語法 / 雙 AI 提示詞 / 每日追蹤表 + 紀律三條)並轉述那套邏輯的四層
(理念:每天撒網 / 工具:自寫程式 + 雙 AI / 標的:股王級短線爆發股 / 流程:每晚微調、隔天再跑),問「有沒有比較厲害、
優化我的選股、介面照附件改、不要帶入人名、有沒有更好的方式、釣魚介面能不能做動畫」。

| 類 | 內容 | 本站的答案 |
|---|---|---|
| **A 早就測過** | 收盤>5 日線・月線向上・股價>300・成交>5 億・法人 5 日買超・AI 評分・隔日表態・3 日時間停損・單檔 ≤20% | 全部在登記表裡,除了「單檔 ≤20%」方向一致之外**一條都不成立**(數字見 DECISIONS V77.4.0 那張表) |
| **B 沒測過** | 成交額**當日橫斷面排名**前 50 | → `kingpool_probe.mjs`:**這一層自己六關全過**(+0.98pp、勝率 41.4% vs 36.4%),其餘六層疊上去 +0.06pp = 零;疊在 🧬 上 vs sham 扣成本全負 → ⛔ 不換預設 |
| **C 量不到** | AI 對題材/籌碼的質化解讀・「每晚微調」・「成功率 100%」 | AI 三條鏈不上網 → 只問質化不評分;「每晚微調、隔天豐收」= 教科書級過度擬合,正是六關 / 前後半 / 逐年 / 高原存在的理由;「100%」在本站的尺上是 10 日勝率 41.4%、絕對超額 −0.29% |
| **D 落地** | 👑 池子 ・三步驟結構(建池 → 拋竿 → 驗證)・一鍵複製規則/追蹤表 ・拋竿動畫 | V77.4.1;⛔ 不借它的框、橘色、評分欄;追蹤表不用手填(本站 🧺 漁獲籃已自動結算、扣同期加權) |

🚨 **附件介面本身沒有任何動畫**(全檔只有 header 綠點 `animate-pulse`;toast 掛的 `animate-in` class 根本沒定義)——
使用者問「能不能做動畫」時,本站魚缸早就有魚游、同族靠攏、位階線、上鉤光暈;缺的是**拋竿的操作回饋**(`castRod` 那 420ms 是純 setTimeout)。
⛔ 人名在附件出現 8 處(含 2 處在複製出去的字串裡)→ 本站文案一律寫「外部那套」。

⭐ **這一則最值得記的一課**:一套「看起來很多層」的漏斗,**拆層檢定**(各拿掉一層)一跑就知道哪一層在做事 ——
這裡六層拿掉任何一層都在 ±0.1pp 內,只有拿掉排名那層掉到 −0.20 → 功勞全在一層。⛔ 層數多不等於厲害。

---

### 🎯 外部參考資料的評估紀錄㉘:老余交易夜 19 份 YouTube 逐字稿(2026-09-20 使用者上傳)—— ⭐ **~60 條主張裡絕大多數本站早就測過;沒測過的測完只剩一條「避雷」站得住,而他最核心的兩條在日 K 上方向相反**

使用者:「這些資料可以優化我的程式嗎?還有策略需要回測就回測,另外還有什麼我沒有說到的,你覺得好的幫我用,改挖礦就挖礦」。
19 份(2023-11 ~ 2026-09,約 1 MB):裸 K 價格行為派 —— 邊界 / 兩條線 / 破底翻 / 互換 / 尖尖的 / 翻亞當 / 該發生卻沒發生 / 三問(位置 → 慣性 → 圖)/ 訓練法 / 21 點基本策略 / 孫子兵法 / 爆帳號經歷。
⭐ 照 V77.0.1 口訣那次的鐵則:**第一步查登記表,不是回測**。三個代理拆出 ~60 條主張,逐條對過 `_SIGNAL_EDGE`(129 訊號)/ `_STREAK_EDGE` / genezone / calendar / 28 種出場 / 停損距離 / 三種進場法,分四類。

#### A. 早就測過(⛔ 不重測)—— 而且他最核心的「位置」哲學跟本站 20 日窗口實測相反

| 老余主張 | 本站已有的答案 |
|---|---|
| 三問第一問「**位置**」= 只在區間**下緣**做多、上緣不做(「50 漲到 100,50 才是對的位置」「位置對了你贏了八成」) | 🚨 **方向相反**:`genezone_probe` 位階四段單調 −0.64 → +0.87pp(位階 <25 兩格全負、≥75 全正);`screener_edge` 127 條總結「追強 > 抄底」;「等回檔再買」21 個有 18 個負;`near_ma` 快突破 −0.28pp;V74.4.5 掛前一日收盤價是三種進場最糟。⭐ 這次 `laoyu_probe` 再驗一次(見 B):60 日位階三桶 ≈0、一年位階反而 **>67% 那桶最好** |
| 尖尖的(單根長下影線)任何位置 | `_SIGNAL_EDGE` 長下影線(支撐)**C −0.19pp n=23,056**、低檔變盤線 C、低檔母子懷抱 A 但期望 −1.1 |
| 破底翻 = 假跌破站回 | 葛蘭碧買2(假跌破站回)**C −0.40 n=13,718**;並排紅K假跌破 B +0.22;測撐有撐 C −0.23 |
| 互換 = 壓力變支撐、回測不破 | ✅ **本站六關全過的那條**:`_STREAK_EDGE.retest` 創 60 日高後回測不破 +0.73pp(V74.6.0 已在總覽);站上長黑K高點 A +0.62、期望 +0.079 |
| 該發生卻沒發生(破底不續跌 / 突破不續攻) | 多方版 = **站上長黑K高點**(9 個正期望訊號之一);空方版 = 葛蘭碧賣2(假突破跌回)C −0.10、高檔轉折確認(開低)A +0.26。這次 G3 再驗「突破不續攻」+0.07 = 零 |
| 翻亞當到價直接出、出一留一、拉不賠、賺賠比 ≥1:4 才進 | 🚨 V74.4.8 28 種出場:固定停利 tp10/15/20、賣一半 half(全表最慘)、風報比 rr2/3、損益兩平 be3/5 **全部 ❌**,「封頂 = 砍贏家,⛔ 以後別再提這三類」;V74.3.5 時間停損全滅。⭐ 他的系統靠 1:4~1:5 出場,本站 49 個月實測是「**抱得越寬賺越多**」—— 兩套的體質不同(他 5 分 K 台指期一天幾筆;本站日 K 個股勝率 30%、靠少數大賺) |
| 停損放在影線末端外(不用固定 %) | 現行 `STOP=lo5` = `min(訊號日最低, −5%)` 本來就是它;V73.0.1 測過 lo / pct3 / pct8 / atr2 |
| 月底月初做多到中旬、結算日、9 月難做、4 月清明多單 | `calendar_probe` 13 種 + `calendar_stock_probe` 37 種**方向 0 個成立**(結算日兩邊都好 = 不是方向;上旬對這套反而最好,但「大盤那天漲跌」跟「這套賺不賺」是兩回事;月份效應 2 年每個月只有 2 個樣本驗不了) |
| 缺口是邊界、小缺口會補 | `gap_probe` ❌(跳空越大越糟)、`gapfill_probe` 六關 0 過;`_detectGap` 向下跳空未回補 A +0.26 但期望 −1.16 |
| 兇 K 之後不接、大黑棒隔壁不做(避其銳氣) | ✅ 同向:實體長黑棒 A −0.77;`crash_exit --facts` 大跌日的跌破更不容易站回 |
| 長上影線隔壁不做多 | 長上影線(賣壓)A 但只有 −0.06pp(V77.0.1「方向對、幅度遠遠不夠」);這次 G3 自檢 −0.16pp 同向 |
| K 棒變小 / 收縮 / 停頓 = 落底前兆 | VCP A +0.46(期望 −0.92)、三角收斂 C、高檔直漲停頓 A −0.03、均線糾結 C;這次「兇K序列後下影十字」n=405 −0.59 |
| 只做多 | App 本來就只做多 |
| 日線訊號要「尾盤確認」再進 | ✅ V72.9.0「有效進場點只有訊號日尾盤」—— 這一條他講對了,而且本站早一年就用實測定下來 |
| 加碼只加在回邊界不加在噴出 | V74.6.1 加碼實測只講事實不下指令 |
| 「我十年不看籌碼」 | 一半對:分點三支探針(`broker_habit`/`ally`/`ind`)全滅;但 `_CHIP_EDGE.both` +0.99pp、V77.3.1 外資∧投信 +0.91pp 六關全過 → ⛔ 不可照抄 |
| 賺錢部位抱久、賠錢部位砍快(王同學 1.4 小時 vs 9 分鐘) | ✅ 同向:V74.4.8 的結論「太早出場會把贏家洗掉」 |

#### B. 沒測過 → `scripts/laoyu_probe.mjs`(1,683 檔・2022-01 ~ 2026-09 **含 2022 空頭**・552,002 事件・t+1 開盤・鎖漲停剔除・20 日去重・扣同期加權・七關 + 門檻高原)

**G1 尖尖的 × 位置 × 邊界**(對照組 = 所有尖尖的 n=30,951,10 日 −0.55%、勝率 41.2%):
| 桶 | n | 10 日邊際 | 判定 |
|---|---|---|---|
| 真破邊界後收回(單根破底翻) | 5,574 | **−0.34pp**(p=0.005) | ❌ 比「沒破」還差;高原 下影 0.4~0.7 四格全負 |
| 60 日位階 下 / 中 / 上 1/3 | 13,112 / 15,162 / 13,331 | −0.00 / −0.23 / +0.01 | ❌ 他的「位置」量不出來 |
| 一年位階 <33% / 33~67% / >67% | 10,490 / 14,369 / 12,049 | −0.11 / −0.21 / **+0.35** | 🚨 **高位階那桶最好**(5/6,只差扣成本)—— 跟他相反、跟 genezone 同向 |
| 擋子彈 有 / 無 | 28,559 / 11,272 | −0.12 / **+0.23** | 🚨 **相反**:左邊有人擋子彈的反而差 |
| 撞牆 有 / 無 | 27,813 / 12,418 | −0.12 / +0.26 | 方向對(5/6)但扣成本 −0.18 |
| 兇K序列後的下影十字(21 點 Double) | 405 | −0.59 | ❌ |
| 連續下影 1 / 2 / 3+ 根 | 22,412 / 21,330 / 11,862 | −0.14 / −0.08 / +0.00 | ❌ 不單調 |
| 兩隻腳(兩點一線後破再收回) | 3,153 | **−0.43pp**(p=0.014) | ❌ |
| 慣性 低點遞升 / 不成立 | 11,686 / 25,399 | −0.09 / +0.03 | ❌ |
| ⭐ 三問疊加 位置 → +圖 → +不撞牆 → +擋子彈 | 13,112 → 3,845 → 1,635 → **128** | 增量 −0.00 / −0.24 / +0.15 / −0.01 | ❌ 沒有一層為正,疊到第四層就沒樣本 |

**G2 標準圖形狀 / 互換否決條件**:標準圖(中段凶・兩端小・底平)n=105 +0.21(p=0.81)= 量不到;互換回測全體 n=13,359:站穩 +0.07、**帶長上影(他說爛掉)+0.23**、**掉進內側(他說爛掉)+0.04** → 三桶 ≈0,「爛掉」的沒有比較差。
⚠️ 他的互換(15 根小箱子被穿過)跟本站六關全過的 retest(創 60 日高 + 5 天不破 95% + 今天收紅)**不是同一個定義** —— 後者有動能成分。

**G3 空方(空方尺:10 日邊際 <0 且 p ≤0.25;對照組 = 所有股·日 n=42,995,10 日 −0.61%)**:
| 桶 | n | 10 日邊際 | 判定 |
|---|---|---|---|
| ⭐ **下降三角(上斜下平)** | **20,072** | **−0.48pp**(p<0.0001) | ✅ **空方尺過、六關 6/6**:逐年 −0.21 / −0.11 / −0.51 / −0.86 / −0.83(含 2022)、前後半 −0.24 / −0.75、去最好年 −0.40;門檻高原 3×3 **九格全部空方尺過**(−0.43~−0.48,八格六關全過)|
| 突破不續攻(昨破 20 日高・今收回) | 8,452 | +0.07 | ❌ |
| 樓梯爛掉(站上 ≥3 根後收回) | 6,891 | +0.07 | ❌ |
| 長上影線(管線自檢) | 34,116 | −0.16 | 同本站 A 級 −0.06,方向一致 |

⭐ **結論一句話:19 份逐字稿裡,日 K 上唯一站得住的是「下降三角之後偏弱」—— 而且它是避雷不是放空;他整套的核心(位置在下緣、擋子彈、破底翻、兩隻腳)在台股個股日 K 上不但沒有東西,還有兩條方向相反。**
⚠️ 為什麼可能不是他錯:他做的是**台指期 / 小那的 1~15 分 K、一天幾筆**,本站量的是**台股個股日 K、20 日窗口**;日 K 只是他盤中定義的代理。這一點寫進 LAB 每一條的限制。要真的驗他,得等 kbar5 的台指期存滿一年(D 那一項)。

#### C. 量不到(誠實列出)
70 分評分系統(他自承「憑感覺」)/ 八成像 / 市場速度(要 tick)/ 先上後下十字(要 1 分 K 拆解)/ 盤中原地停頓 / 情緒鎖單 / 標準圖的左右寬度比(要人判)。
盤中時段口訣(11:00~11:30 轉折 / 10:30 後下殺反彈 / 12:30 尾盤 / 結算日尾盤)→ `scripts/kbar5_tod_probe.mjs` 用 kbar5 **88 天**先量長相、⛔ 不下結論:45 分窗口翻轉率每個都在 47.8~52.2%(= 50%)、**49.5% 的全日最低與 53.9% 的全日最高都在 09:00~09:30**、尾盤 12:30→收盤 +0.01%、結算日 +0.09%(n=374);量 09:00~10:00 佔 41%(這條他講對了)。⚠️ 母體是當日量前 80、只有 88 天、而且他講的是台指期。

#### D. 落地(「你覺得好的幫我用,改挖礦就挖礦」)
1. 📐 **下降三角避雷** → V77.3.8 放進總覽「⭐ 重點判讀」(`_TRI_EDGE` + `_descTriangle`,定義與探針一字不差;⛔ 不做主動提醒、不進計分、⛔ 不是放空訊號)。
2. 📼 **改挖礦:`kbar5_miner.py` 加收台指期近月日盤 5 分 K**(`d[date].idx.TXF`,08:45~13:45,夜盤不收;股票母體 / bias / `MIN_STOCKS` 完全不動;已有 k 的舊日期只補 idx 欄位)。🚨 順手更正 kbar5 註解「hm = 距 09:00 幾分鐘」是**名實不符**(從第一天起存的就是絕對分鐘,selftest ① 釘 [540,545,550]),格式一個位元組沒改。⚠️ 沙箱無 Shioaji → 要使用者 Run 一次 workflow,log 看「🔎 自我驗證 …第一根 hm=525、最後一根 hm=820」。
3. 🎯 **盲測練習**(V77.3.8):他 14.txt 的訓練法(**未知行情下逐根推進、不得先看右側;只記正確出手次數不記損益;分「標準圖 / 邊界二 / 猜」統計**)—— App 本來就有全部資料 + 偵測器 + 實測成績,零採礦、零 API;做成設定中心一顆按鈕(⛔ 不新增卡片)。
4. 他自己講的三句最值得記進本站方法論:「**K 棒要先停住才能當參考點**」(= 前視偏誤防治,本站探針一直這樣做)、「**最標準的標準圖不代表一定會漲,只是提高勝率**」、「**我沒有統計過勝率,因為我的情緒不一樣**」(= 他的系統沒有自帶績效基準,所以本站只能用自己的關卡量他)。
⛔ 不做:翻亞當出場 / 出一留一 / 拉不賠(三類 V74.4.8 已封)、每日虧損熔斷與出手次數上限(本站一天 2 檔、沒有這個問題)、爆帳號警示卡(不新增卡片;`_lotsForRisk` + 決策台 2 檔/天已經是那個機制)。

### 📋 外部參考資料的評估紀錄㉗:外部 AI 對「📄 個股報告頁」的 22 點評論(2026-09-15 使用者貼上)⭐ **五個建議裡 3 個早就有、1 個是我們刻意拆掉的、1 個沒測過**

使用者:「這一份綜合報告有沒有價值?另外有什麼不對或不足的地方嗎?…還是有什麼黑科技」。

⛔ 照 CLAUDE.md 鐵則**逐條讀原始碼驗真偽**,⛔ 不照單全收。結果:**22 點裡 8 點是誤報**(本站已有)、
**6 點是本站已經回測打掉的**、**3 點對但只對一半**、**只有 3 點是真缺口**。

#### ❌ 誤報(本站早就有,他沒看到)

| 他說缺 | 實際 |
|---|---|
| 風險報酬比 | `_upsideRoom()` 回 `{first, stop, risk, rr}`,**而且 `risk` 走 `_netPL` 扣過手續費 0.1425%×折數 + 證交稅 0.3%** —— 比他寫的 `(目標−現價)/(現價−停損)` 準 |
| 成交量分布 / 籌碼密度壓力 | `_volProfile` 是唯一分桶源,`_overheadSupply` 早就標「近 120 日佔總量幾 %」 |
| PEG | `_xrayMetrics.peg` + `pegLabel`(X 光機在用,門檻 <1 / <2) |
| 三情境估值 | `_rpSensitivityHtml`(§14:獲利 ±20% × P25/中位/P75 = 9 個對照價),而且**刻意不給機率**(沒有母體可估) |
| 自由現金流 | 速覽「自由現金流近4季」那一格就是 |
| 歷史回測勝率 | 這是**全站地基**:`_SIGNAL_EDGE`(2,227 檔・49.9 萬筆基準)、`_STREAK_EDGE`、`_SCR_EDGE`、`playbook_edge`、`_DECK_TRACK49`、探針登記表 60+ 支 |
| 「別把股價位置叫價值位置」 | 本站叫「**價格位置(近一年)**」,本來就是對的 |
| 「存貨/應收/應付/CCC 不可混在一起」 | 🚨 **他讀錯欄位**:`118 天 / 288.9 / 17.2% / 21.3%` 是 **存貨天數 / 自由現金流(億) / ROE / 淨利率**,⛔ 沒有應收應付。`data/fin/*.json` 的 12 季欄位是 `rev cogs inv capex dep ocf eq cap eps gm nm doi fcf ni` —— **真的沒有應收/應付** → CCC 目前算不出來(那是真缺口,但跟他講的「混在一起」無關) |

#### 🚨 本站已經實測打掉的(照做會退步)

| 他建議 | 本站實測 |
|---|---|
| ① **綜合分數 0~100**(權重 25/15/25/15/15/5) | 🚨 **這正是本站刻意拆掉的東西**(陷阱 #38:憑空權重)。`_overallGaugeHtml` 的融合總分 V76.1.2 下架、`_gaugeStripHtml` 明文 ⛔ 不顯總分、§19 因子評分與 §20 ★ 評等 V76.2.3 砍掉;`ailevel_probe` 實測 AI 五級分層**沒有預測力**(各層邊際在雜訊內、前後半不同向)。⭐ 他自己給的那組權重**一次都沒驗證過** |
| 訊號**共振**可信度 %(多條訊號疊起來) | 🚨 **測過兩次都反向**:`chain_probe.mjs`「籌碼進駐 → 帶量突破」串起來 vs 單獨帶量突破 = **−0.84pp**;`td_wt_probe` 的「聖杯堆疊」= **越疊越差** |
| 事件給「對本檔影響 ★★★★★」 | `calendar_stock_probe.mjs` 37 種財經行事曆日逐檔 + 期外驗證 → **方向 0 個成立** → ⛔ 只准講「那幾天波動會不會變大」(CLAUDE.md 連動清單有這一條) |
| 題材「兌現程度 ★★★★」 | 同上,憑空評分 |
| 支撐/壓力「可靠度分數」 | ⭐ **這件事本站已經量化而且量得比他細**:`_SUPPLY_EDGE`(2,259 檔・11.9 萬筆・含 2022 空頭)—— 牆越大越穿不過去是**單調**的(小 79.0% → 中 73.3% → 大 **47.6%**),⭐ **但 10 日邊際只有 −0.19pp < 成本 0.44%、六關 0 過、門檻 5 格 0 過** → **穿不穿得過量得到、賺不賺得到量不到** → 所以只准描述,⛔ 不可做成「可靠度 72/100」那種會被當成買賣依據的分數 |
| 「基本面強 + 股價弱」當買點 | 方向跟本站實測**相反**:`pe_probe`「本益比低會漲」**單調反向**、`screener_edge_probe` 127 條的總結是「**追強 > 抄底**」 |

#### ⚠️ 對一半 —— 而且其中一條是他這份評論裡**最危險**的

🚨 **他示範的風報比 12.4 倍,是「假精準」的教科書案例**:
停損 532 距現價 537 只有 **0.93%**(500~1000 元級距跳動單位 1 元 = 只有 5 檔)。
⭐ **停損越近,風報比數學上必然越漂亮,而被洗出去的機率也同步變高** ——
本站 28 種出場、49 個月回測的結論正好相反:**太早出場會把贏家洗掉**
(整套勝率只有 30%,會賺完全靠讓對的那幾筆抱久一點,V75.0.9)。
→ ⛔ 風報比**不可以脫離「這個停損撐不撐得住」單獨顯示**;
本站 `_upsideRoom` 的 `stop` 走的是實測換過的出場規則(唐奇安 20 日),⛔ 不是隨手抓一個近的價位。

#### ✅ 真缺口(他講對的三件)

| # | 缺口 | 為什麼成立 |
|---|---|---|
| **A** | **財報只印最新一季,但我們手上有 12 季** | `data/fin/{sym}.json` 每檔 **12 季**(`rev/gm/nm/eps/doi/fcf/roe`),海報與 §4 卻只印最後一季。⭐ 「營收 +50% 但毛利率往下」跟「兩個一起往上」是**完全不同的公司**,而這個判斷**零採礦、零 API 就做得到** |
| **B** | 事件只列日期,沒講「會動到哪個數字」 | ⚠️ 但⛔ **只能寫「這件事會影響哪個變數 + 波動會變大」**,⛔ 不可給 ★ 重要度或方向(37 種行事曆日方向 0 個成立) |
| **C** | 多空分歧(跨面向背離)沒有被當成一件事講 | 本站目前把「打架」當 **bug** 在抓(`_bearGate` / `_mktGate` / `page_sweep ⑤ 跨卡打架` / `_chipAnalystLine` 的 `clashNote`),⛔ 沒有把它當**訊號**測過。⭐ 值得寫探針 —— 但⛔ **先回測再顯示**,而且方向很可能跟他想的相反(見上表最後一列) |

#### ⭐ 我的不同看法(寫下來免得下次又被同一種建議帶走)

1. ⭐⭐ **他的第一名建議(綜合分數)正是本站走過而且退回來的路。**
   把五個面向壓成一個數字,好處是「好懂」,代價是**那個數字的權重沒有人驗證過**,
   而使用者會拿它當買賣依據。本站的做法是**五格分開顯示 + 每一格自己的實測成績**,
   ⛔ 不加總 —— 這不是還沒做,是**做過、下架、並寫進鐵則**。
2. ⭐ **他整份評論最大的盲點:完全沒看到本站的回測層。**
   他建議的「把訊號記下來、統計 5/10/20 日勝率」——`_SIGNAL_EDGE` 就是這個,
   而且結論很殘酷:**129 個訊號裡期望值為正的只有 9 個**。
   ⭐ 一個系統真正的價值不在「指標多」,在於**它敢說自己哪些指標沒用**。
3. ⭐ **「資料很多但決策密度低」這個評價我接受**,但處方不同:
   他的處方是「加 5 個綜合欄位」;我的處方是**繼續砍**
   (`card_inventory.mjs` 量過,報告頁 91.2% 不重複 → 砍的優先序要靠量不是靠印象)。

#### 🧪 落地(⛔ 這一輪只評估,程式一行沒改)

真缺口 A/B/C 各自的規格寫在下一版待辦;C 一定要**先寫探針、六關過了才准顯示**。
⛔ ①②(綜合分數 / 共振可信度)**永久不做**,理由如上,⛔ 下次有人再提要先讀這一節。

### 📚 外部參考資料的評估紀錄:「台股 AI 投資評價模型」(2026-08-03 使用者提供)
使用者上傳了一份 GitHub 專案(`taiwan-stock-ai-valuation`,純 Markdown、無程式碼,
需搭配 Claude Desktop Cowork + **手動填 5 項財務數字**,一次分析一檔)。
逐份讀完後的結論,**免得日後有人再問一次或重做一遍**:

**⛔ 不能照抄的部分(跟本專案鐵律衝突)**
- 它讓 **AI 自己算分、自己搜尋數字** → 直接違反「禁 AI 算數」鐵律,而且會幻覺。
- 它要**手動輸入**;本專案是 2,700 檔全自動採礦,不可能人工填。
- Capex/折舊、FCF margin、研發費用率、法說會態度、庫藏股、良率、產能利用率、
  地緣政治風險、Revenue breakdown —— **全都沒有資料源**(它是靠 AI 網路搜尋填的)。
  沒有結構化來源就做,等於做假數字。⛔ 別為了「補齊模組」去接這些。
- 「歷史 P/E 價格帶」:我只有**當前** PE、沒有歷史 EPS → 用現在的 EPS 回推歷史 PE,
  算出來只是把價格區間換個名字(等於位階溫度計),**是假的河流圖**,不要做。

**✅ 已經有了(比它更好:全自動、純公式、全市場)**
同業相對 PE(V71.4.5)、全市場 P/B 分位數(V16.2/V71.6.2)、景氣循環股標記
(`is_cyclical`)、毛利率趨勢、月營收 YoY、PEG、殖利率/填息、外資投信籌碼。

**⭐ 真正值得吸收的一個觀念:同一條門檻不能量所有產業**
它的半導體次產業門檻表(晶圓代工毛利 >45% 才算優、**封測 >22% 就算優**、
IC 設計負債比要 <25%)點出本專案的真缺口:X 光機用**同一組絕對門檻**量所有股票 →
日月光(封測,毛利 ~20%)會被判「差」,但那在封測業是正常水準。
⚠️ 但它的表**不能直接抄**:① 我沒有次產業分類(`industry_map.json` 只有 TWSE 33 大類,
半導體全擠在代碼 24)② 我沒有營益率/淨利率/ROE/負債比的**數值**。
→ **正解是用它自己也承認的那條後路**:「資料不足就改用相對位置」——
   算「該股在**同產業內**的百分位」,不寫死門檻,自動適應各產業結構。
   (這也跟 V71.1.6 外資期貨、V71.8.1 波動率同一個原則。)
- **已做第一步(V71.8.5)**:`fetch_finmind_fundamentals` 本來就算出了三率數值,
  卻只存趨勢箭頭字串、把數字丟掉 → 現在一併存 `gross_margin_pct` /
  `op_margin_pct` / `net_margin_pct`(零額外 API)。測試 `scripts/test_margin_pct.py`。
- **下一步(等數值累積後再做)**:比照 `industry_pe.json` 產一份「各產業三率百分位」,
  X 光機改用「同業百分位」判優/良/差。⛔ 別回頭去寫死那張半導體門檻表。
- ROE / 負債比要另外接 FinMind `TaiwanStockBalanceSheet`,尚未評估,別假設現成。

**✅ 另一個小收穫:用詞限制**
它明訂估值區間「僅能稱**歷史估值價格對照區間**,不得使用『進場點』『建議買點』」——
跟本專案 V71.8.2「指數刻意不給買賣價位」同一個道理,可套用到任何「估值≠時機」的卡。

### 📊 外部參考資料的評估紀錄⑧:24 步「台股 AI 分析」Colab 課程(2026-08-14 使用者截圖)

使用者給了一份 24 步教學(yfinance → 技術指標 → 財報籌碼 → ML → Gradio → Email 通知)。
逐步對照後**只有 1 步是我沒有的**,而那 1 步當場驗完決定**不做**。寫下來免得再問一次。

**✅ 已有且更嚴謹(20/24 步)**
K線分價量表(我還多了 `_overheadSupply` 套牢區)・MA/RSI/MACD/布林・營收毛利(1,790 檔)・
外資投信(已補到 3 年)・融資融券・ATR 停損(`_lotsForRisk`)・領先指標(macro_risk)・
回測扣手續費(我還扣同期大盤 + 六道關卡)・儀表板(完整 PWA)・通知(PWA 推播 + Telegram)・SOP。

**➖ 純教學步驟,不適用**:Colab 環境安裝・pandas 多層索引清洗・封裝成 Python 類別・
Gradio 公開連結(我是靜態網站,本來就公開)。

**⭐⭐ STEP 16「訓練 AI 模型(破解勝率迷思)」—— 它自己的副標就是答案**
原文:「用基礎特徵訓練隨機森林,**見證 50% 擲硬幣勝率**的第一課」。
→ 這份教材**自己承認** ML 在這裡只有擲硬幣的水準,跟本專案「禁 AI 算數 + 一切要實測」
   的立場完全一致。⛔ 別因為「有 AI 兩個字」就以為那是我缺的東西。

**❌ STEP 13「落後指標相關性熱力圖:找出誰最影響股價」→ 當場驗完,不做**
這是唯一我沒有的東西。用**本專案自己的資料**驗(51 檔、各 120+ 交易日):

| 問題 | 中位相關係數 | \|r\|>0.3 的檔數 |
|---|---|---|
| 外資買超 vs **當天**漲跌(同期) | **+0.298** | **25/51** |
| 外資買超 vs **明天**漲跌(預測) | **+0.028** | **0/51** |

⭐⭐ **同期相關 0.298 看起來很漂亮,但預測相關幾乎是 0,而且 51 檔裡沒有一檔超過 0.3。**
   真因很直白:**外資買超跟當天漲跌相關是廢話**(有人買才會漲),
   那是「同一件事的兩種記法」,不是因果、更不是預測。
→ ⛔ **不做相關性熱力圖** —— 它會產生一張很有說服力、但對決策毫無幫助的圖。
⭐ 通用鐵則:**看到「相關性」先問「是同期還是隔期」** ——
   同期相關再高都不能拿來預測。本專案的 `_SIGNAL_EDGE`(46 偵測器的**未來報酬**期望值)
   問的才是對的問題。

### 🌅 外部參考資料的評估紀錄⑩:53 份當沖逐字稿(2026-08-15 使用者上傳)
使用者:「有什麼資料適合優化…另外把沒有用的給刪除,不用一直無限疊加,把有用的濃縮精華,
讓使用者明確知道現在要怎麼操作」。53 份、688 KB(權證小哥 / 巨人傑 / 摩卡 / 兆華艾綸說)。

**⛔ 大部分已評估過**:權證小哥那批 = 評估紀錄④⑥(連次量、買盤竭盡、週轉率、處置股、分點);
巨人傑「不談勝率談賠率、期望值 = 勝率 × 盈虧比」= **本專案 V72.0.3 那次自我修正的來源**,已採納。

##### 🔁 2026-08-18 使用者又上傳 55 份 —— **53 份是同一批檔案**(⛔ 別再讀一次)
使用者第二次上傳(問「有沒有更強的策略 / 全球最強當沖策略 / 只有付費才看得到的策略」)。
⭐ **先做 md5 比對再讀**(9 秒 vs 逐份讀完一個多小時):
```
md5sum 兩批 *.txt | sort → 53 份雜湊完全相同
新增的只有 008.txt / 011.txt = 同一場 2026/08/17 郭哲榮直播的「字幕版」與「影片版」
```
內容是當天盤勢評論(哪幾檔漲停、外資買賣超),**沒有任何可驗證的規則**;
而郭哲榮已於**評估紀錄③**(17 場直播)完整評估過 → **無新資訊**。
⭐⭐ **通用做法:收到「同類型再一批」的附件,第一件事是 `md5sum` 比對,不是開始讀。**
   本專案評估外部資料已 10 次,重複率很高。

**❓ 使用者那三個問題的誠實回答(⛔ 別因為問了就硬生一個策略出來)**:
- 「更強的策略」→ 本專案已實測 **86 種**變體,採用的只有 **2 種**(每天 2 檔、高位階+高波動)。
  ⭐ 這個比例本身就是答案:**大多數「策略」加上去都是少賺**。
- 「全球最強當沖策略」→ **不存在**。逐字稿裡每一條有具體門檻的都被實測否定
  (連次量兩次、開盤跳空、ORB、順大盤 80%)。
- 「只有付費才看得到 / 沒人公開的策略」→ ⭐ **本專案已經有付費工具沒有的東西**:
  `_SIGNAL_EDGE`(129 個訊號的**實測期望值**表)、`broker_perf.flip`(分點隔日沖慣性,實測非人工標籤)、
  關鍵分點 / 地緣分點、104 週集保深歷史、`_dtCostGate`。
  ⛔ 付費工具給的是**更多指標**,本專案給的是**哪些指標實測有效** —— 方向不同,不用去補指標數量。

#### ✅ 唯一落地的一條:**當沖成本關卡**(`_dtCostGate`)—— 而且它**不需要驗證**
⭐⭐ 這是 53 份裡唯一「**不是預測、是算術**」的東西,所以可以直接顯示(同 `_trustVolRatioNote` 的原則):
```
來回成本 = 手續費 0.1425% × 折數 × 2 + 當沖證交稅 0.15%(減半) = 3 折時剛好 0.230%
跳一檔賺多少 % = 該價位的升降單位 ÷ 股價
→ 至少要跳幾檔才回本 = ceil(成本 ÷ 跳一檔%)
```
實測對上逐字稿的每一個例子:**105 元 0.48%(1 檔)・60 元 0.17%(2 檔)・91 元 0.11%(3 檔)・45 元 0.11%(3 檔)**。
📍 放在當沖頁 hero 的大字結論**正下方**(它是**先決條件** —— 回不了本就不用談方向)。
⛔ 文案不可出現「會漲/看多/勝率」(測試 ④b 釘住);差的價位要給替代做法(股票期貨 / 換一檔)。
⚠️ 順手抓到**三份**跳動單位階梯散在 `_roundTick` / `_dtLimitPrice` / 新函式(陷阱 #37)→ 已統一走 `app._tickOf()`。

#### ❌ 「開盤跳空法」(摩卡)—— 實測不成立,⛔ 別再做一次
他的三條件:① 跳空 **+2%~+4%** ② 前 5 分鐘收 K 棒上緣 ③ 前 5 分鐘量 ≥ 昨日全日量 10%。
⭐ **先用便宜的資料排除**:②③ 要 1 分 K(雲端、只有 81 天),但 ① 用**日線就驗得動**(全市場 2 年)——
若「跳空 2~4%」這個母體本身就是負的,②③ 只是它的子集合,救不回來。
`scripts/gap_probe.mjs`(2,470 檔、177 萬筆、開盤買收盤賣、扣同期大盤 + 來回成本 0.25%):

| 跳空幅度 | n | 淨超額中位 | 勝率 | vs 對照 |
|---|---|---|---|---|
| (對照組)所有交易日 | 1,773,078 | −0.48% | 36.7% | — |
| 0~1% | 564,088 | −0.66% | 30.7% | −0.18pp |
| 1~2% | 191,705 | −1.19% | 25.9% | −0.71pp |
| ⭐ **2~4%(他說的)** | 87,409 | **−1.63%** | 26.3% | **−1.15pp** |
| 4~6% | 18,211 | −1.77% | 31.1% | −1.29pp |
| >6% | 14,167 | −1.99% | 27.0% | −1.51pp |

⭐ **單調變差**,而且他說的 2~4% 跟相鄰桶沒有分別 → **那條線是隨口訂的**。
前後半段一致(−1.08 / −1.15pp);再加「昨天收上緣」「昨天爆量」都是 −1.0pp 左右,救不回來。
→ ⛔ **不做,而且不必再花雲端額度去測分 K 版。**

##### 🚨🚨 這支探針第一版**犯了前視偏誤**,寫下來免得再犯
我原本用「**今天**收在 K 棒上緣」當條件 —— 但報酬 `ex` 也是用**今天的收盤價**算的
→ 拿答案當條件,跑出 **+1.20%、勝率 66.4%** 的漂亮數字,**完全是假的**。
⭐⭐ **通用鐵則:凡是「開盤就要決定進場」的條件,只能用開盤那一刻已知的資訊。**
⛔ 不可用「全天 K 棒」代替「前 5 分鐘 K 棒」——那不是近似,是洩漏。
⚠️ 而且它**看起來非常合理**(收上緣 = 買盤強),這正是危險的地方:
   數字漂亮到不合理時,**第一件事是檢查條件有沒有用到未來資訊**,不是高興。

#### 🗑️ 「把沒有用的刪掉」:`🔴 盤中連量偵測` 從常顯區**降級**
本專案已經**測過兩次都不成立**(V71.9.8):日線版 6.5 萬個爆量日,跟對照組只差 **+0.14pp**
而且**方向跟坊間說法相反**;分 K 版(101 天真 1 分 K、扣成本 0.25%)八個情境勝率全在 **18~32%**,
而對照組(量價同向)是 **49~52%**。⛔ 但它一直放在當沖頁**常顯區、還用紅綠下多空**。
→ 移進摺疊區、改中性灰、標「⛔ 本站實測不成立」並**把那三個數字寫在卡上**。
⛔ **不刪掉**(刪了使用者會以為沒這回事,跑去別處學了再回來問)—— 同 `_SIGNAL_EDGE` 對 C 級的處置。

#### ➖ 其餘逐條對照(⛔ 不用做)
| 他說的 | 判定 |
|---|---|
| 「順大盤風向,勝率 50%→60~70%;再看族群領頭羊 →80%」 | ⛔ 那 80% **無實證**。而「順大盤」本專案測過(V73.2.0 regime):少賺但回撤小 = **取捨不是變強**。當沖頁本來就已顯示大盤方向當背景(陷阱 #38 之後改成「⛔ 不計入勝率」) |
| 「選股三關:要有波動、要有量能、價格要好衝」 | ✅ 前兩關 = 自訂選股的「高波動」「成交額」條件;第三關 = 這次做的成本關卡 |
| 「一天只做 N 檔、部位別太大」 | ✅ V73.0.0 已實測**每天 2 檔**最好(27 次實驗裡唯一沒有任何一項變差的) |
| 「大部位要先想好怎麼退場」 | ✅ 出場提醒**不限量**(V72.9.1);停損/停利規則已回測 14 種變體 |
| 「用股票期貨降成本」 | ➖ 觀念正確(已寫進成本關卡的建議),但**本站沒有個股期貨資料源**,⛔ 不做報價 |
| 五檔掛單 / 內外盤 / 委買委賣量 | ⛔ **無歷史資料源**(逐筆委託簿),V72.0.2 買盤竭盡已是能做的極限,且明標未驗證 |
| MACD 零軸起飛當沖術 | ⛔ 他自己另一支影片就叫「為什麼看 MACD 做當沖還是一直賠」;本站 `_SIGNAL_EDGE` 已有 MACD 交叉的實測成績,不另做 |

### 🤖 外部參考資料的評估紀錄⑨:Gemini「進階優化方向」6 項(2026-08-14 使用者截圖)⛔ 全部不做
承評估紀錄⑧(同一份課程的加購頁),6 項各配一顆「加入課程,解鎖實戰程式碼」按鈕。
**逐項驗完:6 項全部不做**,其中 2 項是**當場實測**否定的,不是憑意見。

| # | 它建議的 | 判定 |
|---|---|---|
| 1 | 終極全自動化腳本(Colab + Gradio 儀表板,輸入代號跑全流程) | ➖ **已有且大一個量級** —— 我是 2,700 檔全自動 PWA,不是一次一檔的筆記本;Gradio 那條在⑧已評過 |
| 2 | 引入 **XGBoost / LightGBM**(GridSearch 調參 + Feature Importance 砍指標) | ❌ **實測否定**,見下 |
| 3 | 引入 **LSTM 深度學習**(2D reshape 成 3D、MinMaxScaler) | ❌ 同上,而且更糟(參數更多、樣本更少) |
| 4 | **NLP 情感分析**(爬 PTT 股版 + Gemini 判多空) | ⛔ 違反禁 AI 算數;且 V72.8.0「分析師焦點」8 輪實測後**整組移除**,機房 IP 連 YouTube/Google News 正文都拿不到 |
| 5 | **強化學習**(FinRL / PPO / DQN 交易 agent) | ⛔ 本質是「在同一段歷史上搜尋最佳策略」= 大規模過度配適。本專案手動已測 **86 種**變體、只採用 2 種,RL 的搜尋空間大好幾個數量級 |
| 6 | **隱含資本成本 ICC**(JFQA 2026,含分析師預測偏誤修正) | ⛔ **核心輸入拿不到** —— ICC 要「分析師一致預期 EPS(未來 1~3 年)」,台股**沒有免費結構化來源**;「修正分析師偏誤」更需要歷史預測序列。我的「法人目標價」是純公式(年化 EPS × 產業中位 PE),**不是**分析師預測,⛔ 不可拿來充當 ICC 的輸入 |

#### 🧪 `scripts/ml_probe.py` —— 沙箱沒有 sklearn/xgboost,所以自己寫了一支
實測環境:`numpy / sklearn / xgboost / lightgbm / pandas / torch` **全部 ModuleNotFoundError**
→ 用**純 Python 自己實作 LightGBM 的核心**(直方圖梯度提升 + 深度 2 + logistic loss),
零依賴、只讀 `data/`、50 秒跑完。**2,252 檔 / 211,505 個樣本。**
標籤 = 未來 20 日**超額**報酬(扣同期加權)> 0;⭐ **時間切分**(⛔ 不可隨機切,那會把同一天
不同股票分到兩邊 = 抄答案)+ 中間**挖掉 25 個交易日**(標籤看得到未來 20 日,不挖就是洩漏)。

| | 樣本外(2025-12 ~ 2026-07,57,115 筆) |
|---|---|
| ③ 測試集**基準**(贏大盤比例) | **28.5%**(⛔ 不是 50%) |
| 訓練集 AUC | 0.5680 |
| ⭐ **測試集 AUC** | **0.5216**(0.5 = 丟銅板) |
| 🌲 模型前 10% | 30.3%(**+1.7pp**)・超額中位 **−4.63%** |
| 🌲 模型後 20% | 26.4%(−2.1pp) |
| 🆚 **App 現行「位階 ≥75」一條規則** | **37.7%(+9.1pp)**・n=14,079 |

⭐⭐ **三個結論**:
1. **那 +1.7pp 沒過穩健性** —— 測試集再切前後半:**前半 −1.7pp / 後半 +5.4pp,方向相反**。
2. ⭐⭐ **模型輸給 App 現在用的一條 if**(+1.7pp vs **+9.1pp**)。
   而且**特徵重要度前三名 `dd60` 26.7% / `pos252` 14.9% / `bias20` 13.6%**
   —— 正是 V73.2.3 已經在用的「距 60 日高 + 位階 + 乖離」。
   → **模型只是把既有規則學了一個比較差的版本**,⛔ 沒有帶來任何新資訊。
3. 訓練 0.568 → 測試 0.522 → 那是**過度配適**,⛔ 不是「調參/換 XGBoost 就會好」。

⭐ **佐證**:同一份課程 STEP 16 的副標自己就寫「見證 **50% 擲硬幣勝率**」(評估紀錄⑧)。

⛔ **通用鐵則(這條比 6 項判定本身重要)**:
**「換更強的模型」解決不了「資料裡沒有訊號」。**
判斷任何 ML 提案,先問三件事 —— ① 標籤是不是**未來**報酬(⛔ 同期相關是廢話,見⑧)
② 有沒有**時間切分 + purge**(⛔ 隨機切必定虛高)③ 有沒有贏過**現有那條最簡單的規則**
(⛔ 贏基準不夠,要贏現況)。這三關過不了,再花俏的模型都不用談。
⚠️ ⛔ **這不等於「機器學習在台股永遠沒用」** —— 它證明的是「**這批 K 線特徵 + 這種模型**」
沒有邊際。籌碼特徵要等 2027/05(`foreign_net` 中位只有 28 天)才驗得動。
⭐ `ml_probe.py` 有 `--selftest`(注入必然學得到的洩漏特徵,測試 AUC 必須 >0.8;
實測 **0.9997** 通過)—— ⛔ 沒有這條的話,「AUC 0.52」分不出是沒訊號還是程式壞掉。

### 📚 外部參考資料的評估紀錄⑦:`crawl4ai` 網頁爬蟲(2026-08-07 使用者上傳)
使用者問「我的附件爬蟲有沒有對這個程式有什麼用處」。逐項查完,結論存這裡**免得再讀一次**。

| 可能的期待 | 實際 |
|---|---|
| 拿 YouTube 逐字稿 | ⛔ **它沒有這個功能** —— `grep -ril "timedtext\|captionTrack\|transcript" crawl4ai/` = **0 個檔** |
| 繞過 YouTube 同意頁 | ✅ 做得到(真瀏覽器 + `playwright-stealth`)|
| 把新聞頁變成乾淨正文 | ✅ **這才是它真正的價值**(`PruningContentFilter` / `BM25ContentFilter`)|

⛔ **決定不整包引入**,三個理由:
① 依賴很重:`playwright` + chromium(~400MB)+ `litellm` / `nltk` / `numpy` / `shapely` / `alphashape`,
   而 `news_express` 一天跑 10 次 → 每次多 2-4 分鐘安裝,不划算。
② 我們要的只有「HTML → 乾淨正文」那一小塊 → **照它的觀念自己寫 10 行**(`analyst_miner._main_text`:
   先砍 script/style/nav/footer/aside,只留 `<p>`,丟掉 <20 字的段落)。
③ 它的 `LLMExtractionStrategy` 讓 AI 做抽取 → 跟「禁 AI 算數」同族的問題,而且燒 token。

⭐ **真正解決問題的是它的「觀念」不是它的「套件」** —— 這條可以套用到日後任何外部工具評估:
   先問「我需要的是它的**哪一個函式**」,而不是「要不要裝它」。

⚠️ 若哪天真的需要繞過同意頁(YouTube 搜尋頁對 GitHub 機房 IP 回同意頁,V72.6.8 實測
   `CONSENT=YES+cb` cookie **無效**),再評估**只裝 playwright**(專案已有 Chromium,
   `PLAYWRIGHT_BROWSERS_PATH=/opt/pw-browsers`),⛔ 不必為此裝 crawl4ai。

### 🕸️ 外部參考資料的評估紀錄⑮:Gemini 籌碼策略對話(2026-08-31 使用者上傳 .docx)⛔ 一行功能都沒改
230 段 / 18,091 字。⭐ **先做對照,再決定讀什麼** —— 它列的東西本專案這一輪剛好幾乎都測過了。

**⛔ 已經測過、而且有些**方向相反**(⛔ 別照它做)**
| 它主張 | 本站實測 |
|---|---|
| 「主力淨買超佔量比高 → 籌碼集中 → 看多」 | ❌ `chips_deep_probe`:前5大買超佔量最高 10% 的 20 日 **−0.60pp**,**買賣兩端都是負的** = 活躍度不是方向 |
| ⭐「計算主力淨買超時**自動扣除隔日沖分點**的買超量」 | 🚨 **方向相反**。`broker_cross_probe` A 組:大漲日**隔日沖佔比高**的 10/20 日 **+1.39/+2.11pp** —— 隔日沖圈選股本身就是動能標記,扣掉等於把資訊丟掉。`broker_skill_probe` E 組另測「避開高翻臉率券商的買」隔日差 **0.00pp** |
| 「地緣券商 = 波段主力」 | ❌ `broker_skill_probe` C 組:**+0.01pp**(n=110,100),排除雙北 −0.07pp,前後半都不同向 |
| 「連續小量買進的分點」 | ⚠️ 只有配上「**價格已經動了**」才成立(C 組 +0.85/+1.36pp,已落地 `_chipRunBuy`);單看連買 +0.14pp < 成本 |
| 「籌碼集中度 >+2.0σ 才算異常」 | ➖ 門檻改成 σ 不會改變結論(每一桶、每一年都輸對照組) |
| 「千張大戶 × 股東人數 四象限」 | ❌ 評估紀錄(V72.5.2):偏多與偏空兩格 60 日只差 **0.12pp**,交互作用全負 |
| 「週頻籌碼選池 → 盤中觸發」兩階段 | ✅ 觀念本站已有(`playbook_scan` 每晚選池 → `_eodTriggerSweep` 盤中觸發) |
| 「嚴禁未來事件偏誤:分點收盤後才公布」 | ✅ 本站所有籌碼回測進場價一律用**隔天開盤**,早就照做 |

**⛔ 沒有資料源(⛔ 別再評估一次)**
- **MQI(融資買進**筆數**÷ 融資餘額張數)** —— 台灣**不公布融資買進筆數**,TWSE `MI_MARGN` 只有張數。⛔ 分子拿不到。
- **借券賣出餘額 / Unused SBL** —— FinMind 借券是付費層資料集,而且**本站從來沒存過歷史** → 就算現在開始抓也要等一年才驗得動。
- **鉅額交易溢價錨定** —— `blocktrade.json` 實測**只有當前快照**(`{n, m, last}`,7.8 KB),沒有歷史 → ⛔ 現在測不了。這是「現在不存以後永遠沒有」那一類,想做要先開始存。

#### 🕸️ 唯一「沒測過 + 資料剛好夠」的一條:分點同盟集團 → **實測不成立**
它的主張:「主力為避免被追蹤把大單拆到多個分點 → 算分點兩兩 cosine similarity,>0.85 視為同一個影子集團;
同盟內 ≥3 個分點同天大買 = 高信賴度主力建倉」。`scripts/broker_ally_probe.mjs`(467 天深歷史 × 前 120 家券商 = 7,140 對)。

⭐⭐ **對照組是這題的成敗關鍵,而文件自己沒講**:事件是「**3 家**券商同天大買」。
對照組若拿「單一券商大買」,量到的是「**很多人一起買**」⛔ 不是「**同盟**」。
→ 對照組必須是「同樣 ≥3 家同天大買,但**彼此不是同盟**」。

| | 相似度算法 | 結果 |
|---|---|---|
| 第一版 | 原始 cosine | ❌ 前 2 名是 **元大↔富邦 0.960**(78,057 次共同出現)= **純規模效應**;sim≥0.7 只有 +0.65pp 且前後半不同向 |
| 第二版 | ⭐ 先減掉**每個(股·日)的共同成分**再算 | 三個門檻全部「看起來」通過(見下) |

**⚠️ 第二版(正向:前半學同盟、後半驗收)—— 看起來非常強**
| 門檻 | n | 10日 | 20日 | 勝率(對照 33.0%) | 驗收段前/後半 | 格內對照 |
|---|---|---|---|---|---|---|
| sim ≥ 0.85 | 254 | **+1.52pp** | +1.90 | 42.5% | +0.68 / +2.51 | **+1.18pp** |
| sim ≥ 0.7 | 732 | +1.35pp | +2.05 | 40.3% | +1.10 / +1.66 | **+1.45pp** |
| sim ≥ 0.5 | 4,872 | +1.09pp | +2.06 | 38.5% | +0.91 / +1.19 | +0.56pp |
單調(門檻越嚴邊際越大)、全部高於成本 0.44pp、前後半同向、**連「位階×波動×成交金額」格內對照都撐住**。

**🚨🚨 但反向驗證(後半學同盟、前半驗收)整個翻掉**
| 門檻 | n | 10日 | 20日 | 格內 |
|---|---|---|---|---|
| sim ≥ 0.85 | 402 | **−0.14pp** | −0.62 | +0.05 |
| sim ≥ 0.7 | 16,278 | **−0.35pp** | −0.38 | −0.19 |
| sim ≥ 0.5 | 17,284 | −0.34pp | −0.39 | −0.19 |

⭐⭐⭐ **而最決定性的證據不是報酬,是「同盟本身穩不穩」**:
```
sim ≥ 0.85:前半學到 5 對 ・後半學到 30 對 ・兩段都出現 0 對 → Jaccard 0.0%
sim ≥ 0.7 :前半 8 對 ・後半 56 對 ・兩段都出現 0 對 → Jaccard 0.0%
sim ≥ 0.5 :前半 12 對 ・後半 83 對 ・兩段都出現 0 對 → Jaccard 0.0%
```
前半段最像的是 `第一高雄↔群益中壢`、`元大↔富邦`;後半段變成 `華南竹北↔中信託文`、`元大大天↔永豐新竹`
—— **一對都沒重疊**。真的影子集團會在兩段都出現;學到完全不同的人 = **那不是結構,是被那一半資料湊出來的**。
⛔ **不做。**

⭐⭐ **三條可以帶走的通則(比這條策略本身重要)**:
① **相似度天然會把「規模」算進去** —— 兩家大券商在每一檔都有量,cosine 自然接近 1。
   要問「行為像不像」,必須**先減掉每個(股·日)的共同成分**,⛔ 不可直接算原始向量。
② ⭐⭐ **「學到的東西本身穩不穩」比「報酬好不好」更快、更決定性。**
   這次正向那組六道關卡 + 格內對照全過,看起來是本輪最強的發現;
   但只要問一句「換一半資料學,學到的是不是同一批人」,**Jaccard 0% 當場結案**。
   ⛔ 以後任何「先從資料學出一組成員/群集/名單,再拿它去預測」的做法,
   **一律要先報這個重疊率**,再談報酬 —— 報酬會騙人,成員名單不會。
③ **雙向驗證不是形式** —— V72.9.2「排點估計值必挑到僥倖股」、V74.0.5「元大永寧訓練段第 1 名 → 驗收段 −2.17%」
   都是同一件事的不同版本。這次是**第三次**同一個陷阱換皮出現。

#### 👩 順帶解鎖:投量比終於測得動了 → 但**六關 0 過**
文件另一條:「投信買超佔比(投信單日淨買 ÷ 當日成交量)**>10%** 且持股率 0.5~3% = 低檔初升期」。
⭐ V71.9.5 做這張卡時 `trust_net` 只回溯到 2026/05 → 當時**刻意只做單位換算、不下方向**。
2026-08-31 實測 **555 檔 ≥100 個非零日** → 現在驗得動了(⭐ 這是「等資料」等到的,不是新想法)。
`scripts/trustvol_probe.mjs`:**2,529 檔 / 對照組 1,194,979 個(股·日) / 事件 93,555 筆 / 2024-04-18 ~ 2026-07-21**,
進場 = 隔天開盤、扣同期加權、排除開盤鎖死、同檔同桶 20 日去重。**對照組 10 日 −1.15% ・勝率 37.5%**。

| 投量比 | n | 10日 | 20日 | 勝率 | 前半 / 後半 | 逐年(24/25/26) | 格內 | 疊在🧬之上 |
|---|---|---|---|---|---|---|---|---|
| 投信**賣**超 | 10,919 | **+0.21** | +0.59 | 39.5% | −0.15 / +0.58 | −0.3 / +0.1 / +0.9 | +0.11 | +0.47 |
| 沒買沒賣 ±0.1% | 58,974 | −0.06 | −0.22 | 36.7% | −0.10 / −0.02 | −0.1 / −0.1 / +0.0 | −0.05 | −0.27 |
| 買 0.1~1% | 6,493 | +0.41 | +1.08 | 41.3% | −0.05 / +0.84 | −0.3 / +0.5 / +1.1 | +0.26 | +0.23 |
| 買 1~3% | 5,333 | +0.44 | +1.24 | 41.1% | −0.02 / +0.87 | −0.1 / +0.3 / +1.3 | +0.26 | +0.23 |
| ⭐ **買 3~5%** | 3,652 | **+0.58** | **+1.71** | **41.9%** | −0.09 / +1.26 | −0.0 / +0.2 / +2.0 | +0.37 | +0.55 |
| 買 5~10% | 4,056 | +0.53 | +1.34 | 41.5% | −0.06 / +1.09 | −0.2 / +0.4 / +1.6 | +0.37 | +0.93 |
| 🚨 **買 ≥10%(文件說的)** | 4,116 | **+0.19** | +0.52 | 39.3% | −0.26 / +0.56 | −0.2 / −0.1 / +1.2 | +0.06 | +0.51 |

⭐⭐ **三個結論**:
1. 🚨 **它訂的 10% 是全部買超桶裡最弱的一格**(+0.19pp),最好的是 **3~5%**(+0.58pp)——
   **非單調** → 那條線是隨口訂的(同摩卡「跳空 2~4%」、權證小哥「地板股 100 檔」)。
2. 🚨 **投信「賣超」也是正的**(+0.21pp)→ 又一次「**兩端同號 = 活躍度不是方向**」
   (同集保隱藏大戶、融資兩端、分點集中度)。⛔ 不可做成多空。
3. 🚨 **七個桶的前半段全是負的、後半段才轉正,逐年也是 24 負 → 26 正**
   → 那個邊際**只存在 2026 這一段**,六道關卡**一關都沒過**(最好的 3~5% 扣成本後 +0.14pp,但前後半不同向)。
⛔ **`_trustVolRatioNote` 維持原樣(只做單位換算、不下方向、不計分)** ——
   ⭐ 現在它有實測數字可以寫上去了,但結論仍是「**沒有驗證出方向**」,⛔ 不可改口說有用。
⚠️ 「持股率 0.5~3%」那半 ⛔ **測不了** —— 累計持股需要**起點**,`cumsum(trust_net)` 從 2023 開始不是真持股率。

⚠️⚠️ **這支探針自己犯了兩個 CLAUDE.md 已經記過的錯**(都靠實跑輸出抓到,⛔ 不會報錯):
① 🚨 **前後半的中點用了整條日期軸**(`^TWII` 從 2021-08,個股 2023-06 + 250 根暖身)
   → **前半永遠是 NaN、那一關等於沒作用**。這是 V74.0.2 `limitup_probe` 的**同型再犯**
   → 改成從**實際樣本**推中點,並把區間印出來。
② selftest 的假雜湊寫成 `((a*2654435761 + b*40503)>>>0)/2**32` —— 固定 a、b 每 +1 只讓值增加 9.4e-6
   = **那是斜坡不是雜湊** → 事件全擠成一整段、波動率算成 0.1%。
   ⭐ 跟 V74.0.5 `broker_skill_probe` 的散兵排班是同一個錯 —— **測資自己要先驗一遍**。

### 📐 外部參考資料的評估紀錄⑭:「AI 產業成熟度系統」開發規格(2026-08-29 使用者上傳)→ V74.1.0 落地
使用者上傳一份 49 節的開發規格(要把「AI 演進五級」升級成資料驅動的成熟度與預警系統),
並提出 9 項需求。**規格本身有一半是對的、一半跟本專案鐵則直接衝突**,逐條判定如下。

**✅ 採納(而且它說對了本專案原本沒做的事)**
① §2「Level ⛔ 不得依時間固定增加」—— 完全正確,跟「憑空門檻」鐵則同源。
② §20 Leading/Coincident/Lagging + §38 供應鏈五層 + §49 Core/Infra/Enabler/Bottleneck/Frontier 分類
   —— ⭐ 這是整份規格最有價值的部分,而且**純標籤、不需要新資料源**。
③ §23/§33「不產生買賣訊號、這是研究模型不是預測器」—— 跟本專案立場一致。
④ §15 資料信心分級、§16「Why Score Changed」、§17 歷史不可覆蓋 —— 觀念正確(本專案已有同類機制)。

**⛔ 不採納(⛔ 別再接回來,理由寫下來免得下次又被說服)**
| 節 | 它要的 | 為什麼不做 |
|---|---|---|
| §3 | Level 指標(企業採用率/Agent 營收/使用量/CAPEX) | **台股與全球都沒有免費結構化來源** → 做了就是編數字 |
| §8 | `總分 = AI受惠度×20% + 成長性×20% + …` 八維加權 | 🚨 **陷阱 #38 的加強版**:八個維度都要憑空打分再憑空加權 |
| §9 | Market Expectation Gap =「產業實際進度 − 市場預期」 | **兩個數字都是編的**,相減之後看起來很精確,其實是兩個猜測 |
| §10/§11 | Next Wave Score、Forecast Level(30/90/180 天預測成熟度) | **完全無法驗證**;而且違反「預測性主張要先實測」 |
| §26/§25 | REST API + 12 張資料表 | 本專案是**靜態網站**,沒有後端 |

**🚨 規格對台股的一個實質錯誤(已修正)**:§35/§49 把「記憶體/HBM」列為 **AI Core**。
那對**全球**成立(HBM = 海力士/三星/美光),⛔ **但台廠沒有 HBM** —— 台廠是利基型 DRAM / NOR / 模組,
沒有 HBM 的定價權。標成 Core 會讓人以為台系記憶體有同等地位 → `pro.html` 標成 **B 層(Core+瓶頸)但
文案明寫「台廠做的是利基型記憶體不是 HBM」**,轉折卡也補了同一句。

**📍 V74.1.0 落地(全部在 `pro.html`,index.html 一行未改)**
| 使用者需求 | 做法 |
|---|---|
| 個股可點跳散戶救星、返回原位 | `gotoStock()` 同分頁跳 `index.html?sym=`(⛔ 不開新分頁 —— 手機會堆積、iOS PWA 留空白頁);跳前把捲動位置/分頁/篩選/估值輸入存 sessionStorage,回來時 bfcache 生效就只補捲動、整頁重載才完整還原(30 分鐘時效) |
| 手勢 | 左右滑切分頁 + 頂端下拉重新整理。⛔ **三個守門不可拿掉**:①起點在可橫向捲動容器(表格)內不攔截 ②水平位移要 **2 倍**於垂直(touchmove/touchend **兩處**都要判)③下拉只在 `scrollY<=0` 起算、拉超過 70px 才觸發。⚠️ `hardRefresh` 一定要清 `_cache`(⛔ 不清等於什麼都沒做) |
| 低/中/高基期 | ⭐ **拆成兩種且必須分開**:股價基期(`pos252`,價格在近一年的位置)vs 估值基期(PE 在自己近3年的位階)。**背離時要主動點出來**(股價高位+估值低位 = 獲利長得比股價快,⛔ 不是「貴」) |
| 目標價 | ⛔ **不叫目標價**(違反 V71.8.2 用詞限制)→ 做成「**歷史估值對照價位**」表:回到 P5/P25/中位/P75/P95 與同業中位 PE 各對應多少錢、距現價幾 %、**一張差多少元**。⛔ 表格自己那一段必須寫「這不是目標價,也不是預測」(測試釘住,⛔ 不可讓卡片別處的免責替它背書) |
| 落後產業 | `_frontLevel()` = 「最近 20 天資金打哪一層」(各層成員 `chg20` 中位最高者),每檔標 🎯主戰場/⏳落後/🌱太早。⛔ **樣本不足 5 檔不判定**;文案必須寫「不是 AI 進度預測、沒有實測過預測力」;成熟度那條 bar 要註明「人工框架、不會隨時間自己增加」 |
| Prompt 帶個股 | 新增 `buildStockPrompt(i)`:單檔版,帶入兩種基期 + 5 檔 PE 對照價位 + **AI 鏈定位(段/題材/層級/供應鏈層/風險)**,與多檔版共用同一份防幻覺 header |
| Perplexity | `_askAi()` 照 index.html `_freeAiOpen` 的做法:附今日日期前綴 + **用真實 `<a target="_blank">` 點擊**,⛔ 不可用 `window.open`(iOS PWA 會留空白 Safari 分頁) |

測試 `scripts/test_prohtml.mjs` 40 條,⭐ **5 種注入缺陷全部驗過**。
⚠️ 過程中測試自己犯兩個錯,兩個都是「測資/非同步」而不是程式:
① 灌「L3 成員都很強」時把 4585 的 null 一起蓋掉 → null 排序那條驗不到(**測資要先自己驗一遍**);
② `selLayer()` 內部的 `renderChain` 是 async,沒 await 就數 DOM → 數到舊的。
⭐ 另外兩條斷言第一版**太鬆**(注入後別處還有同樣字串 → 假通過)→ 改成「只驗它自己那一段」與「兩處都要有」。

### 🤖 外部參考資料的評估紀錄⑬:AI 產業鏈儀表板包(2026-08-29 使用者上傳 4 檔)→ V73.9.9 落地
使用者上傳 chain_growth.py / update_chain.yml / aistrategydashboardv3.tsx / ai_chain_data.json
(AI 演進五級 × 台股 64 檔受惠股 + 每週 FinMind/LLM 成長引擎),要「當作未來股票成長的參考、介面用散戶救星風格」。

**✅ 內容查證(逐檔對過 gh-pages 真資料)**:64 檔代號全部真實存在(K 線檔全有)、名稱正確、
segment/levels 框架合理(對應 OpenAI 五級 AGI 框架)。⭐ **真價值:其中 17 檔上櫃股
`industry_map.json` 完全沒收錄**(證交所 33 大類只涵蓋上市)→ 這份名單正好補了「個股沒有歸類到的產業」。
補了 3 檔它漏的:**4585 達明**(協作機器人本尊 —— 它只列母公司 6188 廣明,但達明 2025 已分拆上市)、
**3035 智原**(ASIC 設計服務第三家)、**8210 勤誠**(AI 伺服器機殼)→ 共 67 檔。

**⛔ 四個不採用(⛔ 別再接回來)**:
① **動能分數(YoY×60% + 法人×40%)與熱度門檻(35/50/65)** —— 權重門檻全是憑空訂的(陷阱 #38 同型)
   → 前端只顯示**原始數字**(近20日/外資10日/YoY,讀 `screener.json`,實測覆蓋 65/67 檔),⛔ 不混成假分數。
② **每週 FinMind 採礦引擎** —— 它要抓的 YoY/法人 App 本來就有(零採礦優先);而且 FinMind 付費層剛過期。
③ **`update_chain.yml` 每週 cron** —— 撞 GitHub 排程配額(V73.9.0:全 repo 一天只進得來 7~10 筆 schedule run);
   且改 Actions 本來就要先問。名單更新方式 = 使用者叫 Claude 改 `_AI_CHAIN` seed(跟其他人工整理內容一致)。
④ **LLM 每週自動提案新受惠股** —— 未驗證的主張不進 App(它的防幻覺三閘門設計得不錯,但價值低於維護成本)。

**📍 落地(V73.9.9,全部深色散戶救星風格)**:選股頁新 tab「🤖 AI 鏈」(`_AI_CHAIN` 靜態表嵌 index.html,
約 15KB,同 `_SIGNAL_EDGE` 做法):五級演進條(點一級=看台鏈受惠+篩名單)+ 67 檔個股地圖(段內按近20日排,
null 排最後顯 —)+ 10 個技術轉折 + 利潤池/傳導鏈 + 檢核清單(標明哪些 App 已有對應功能),全區收合防資訊爆炸;
K線頁 `_aiChainChipHtml` 一行成員 chip(⛔ 不是新卡)。免責固定:**人工整理的產業說明書,⛔ 不是買進名單、
預測力未實測、循環股別當成長股抱**。測試 `scripts/test_aichain.mjs` 18 條(3 種注入缺陷驗過)。
⚠️ 測試又踩兩個老坑:「動能分數」禁止句被自己的 ⛔ 註解擋下(**第 7 次**,驗渲染輸出不驗原始碼);
⭐ 新坑:**emoji 字元類別 regex 沒加 `u` flag 會拆成 surrogate 半碼 → 🔄 被誤判成 🔴**,以後 emoji 斷言一律加 `u`。

### 📕 外部參考資料的評估紀錄⑫:5 份教學檔(2026-08-22)—— **一行功能都沒改,但學到一課**
⚠️ 使用者明示:**這批不要寫進 App 的更新紀錄**,也**不要記作者是誰** → 只記在這裡,⛔ 不進 `_CHANGELOG`。

**先做重複比對與量化門檻掃描(⛔ 別直接開始讀,5 份 ≈ 66 頁 + 6 萬字)**:
| 份 | 內容 | 判定 |
|---|---|---|
| 2 份 | 「飆股怎麼來的」PDF 版 + 彙整版 | 🚨 **內容完全重複**,只需讀一份 |
| 3 份 | 新手教學(63 頁入門)・交易心法・股市基礎 | ⛔ **通篇沒有可量化門檻**,掃出來的「數字句」全是舉例與敘事 → 測不了 |
| 1 份 | 「飆股怎麼來的」 | ⭐ 唯一有可回測主張 |

#### 📊 `scripts/breakout_probe.mjs` 實測(2,233 檔 / 14,820 事件 / 對照組 1,679,699 個股·日)
| 他的主張 | 實測(20日 vs 對照) | 判定 |
|---|---|---|
| ⭐「**底部**(位階≤25)+ **連 2 根漲停** = 主力表態,該買」 | **−2.66pp**(n=498) | ❌ **六關全滅** |
| ⛔「**非底部**(位階≥60)連 2 漲停 = 主力成本早拉開,追進去被出貨」 | **+4.58pp**(n=4,558) | 🚨 **方向完全相反,而且六關全過** |
| ⛔「第一波沒用漲停 = 意願不強,不必切入」 | 底部強紅K −1.68pp vs 底部連2漲停 −2.66pp | ❌ 反而**沒漲停的比較好** |
| 「**連續**漲停才算數」 | 連 2 根 −2.66pp vs **只有 1 根 +1.20pp** | ❌ 「連續」**沒有加分** |
| ④「洗盤:**量縮但價不跌**」 | +2.60pp vs 爆量/破底 +0.15pp | ⚠️ **方向對**,但前後半不同向、去最好年 −0.99 → ❌ |

⭐ 前四條的方向跟本站既有結論一致(V73.2.9 追高 > 抄底、V73.8.3「等回檔」21 個有 18 個負)。

#### ⭐⭐ 這次最值得記住的是**方法論**:「六關全過」還不夠,還有兩關
「非底部連 2 漲停 +4.58pp、六關全過」看起來是很久以來最好的發現。⛔ 但它被兩道追加檢定剝到歸零:

| 關卡 | 結果 |
|---|---|
| 六道關卡(全期正・前後半同向・逐年同向・去最好年・扣成本) | ✅ +4.58pp → 扣成本 +4.14pp |
| 🚨 **A. 買得到嗎** —— 漲停鎖死時**當天收盤買不到**,改成隔天開盤買 | +4.58 → **+1.48pp**(買不到的代價 **3.1pp**) |
| 🚨 **C. 疊在現行配置(🧬 高位階+高波動)之上還有增量嗎** | **+0.46pp**,但去最好年 **−0.17**、扣成本只剩 **+0.02pp** |
| **結論** | ⛔ **不做**(重疊率 68.2% —— 大半只是把「高位階+高波動」再數一次) |

⭐⭐ **通用鐵則(比這 5 份文件本身重要)**:
① **回測用的成交價必須是「真的買得到」的價** —— 漲停/跌停鎖死那天的收盤價**不是**可成交價,
   用它回測等於前視偏誤的近親。任何「訊號當天就是漲停」的策略,都要另外跑一次「隔天開盤買」。
② **「六關全過」不是終點** —— 還要問「買得到嗎」與「疊在現有配置上有沒有增量」(V73.2.5)。
   這次三道關卡一層一層把 +4.58pp 剝到 +0.02pp。
③ 這也再次印證:**一個看起來很強的發現,越漂亮越要多查兩關**(同 V72.9.2「排點估計值必挑到僥倖股」)。

⛔ **結論:App 一行判定邏輯都沒改,版本號也沒動。** 探針留著,走完一次空頭後可重跑
(尤其「底部連 2 漲停」那格 —— 現在的窗口整段偏多頭,底部型態本來就吃虧)。

### 🎫 外部參考資料的評估紀錄⑪:《哥有籌必爆》S1+S2 再一批 40 份(2026-08-21)—— **測了 2 條,0 條成立**
使用者:「檢查有沒有有參考價值的地方,篩選出來後逐批來優化程式」。
⚠️ 這批(40 份 / 0.81 MB)= **評估紀錄⑥ 的同一個系列**(⑥ 是 57 份)。
⭐ 依鐵則先掃量化門檻再決定讀什麼,**40 份裡只有 2 條是「⑥ 沒測過 + 用既有 `data/` 測得動」的**。

#### ⛔ 掃完後確定不測的(理由寫下來免得再問一次)
融資維持率 130% / 使用率 >70%(⑥ 已評估,使用率無歷史)・週轉率一天 >20%(⑥ **已測兩次**不成立)・
券資比 >30% / 回補力道 >50%(`short_balance` 只到 2026/05,同 V71.9.2 死因)・
主力5日籌碼集中度 >5%(需**逐日**分點歷史,只有滾動快照)・可轉債市值 >120(V71.9.1 已測)・
大戶 400 張 ±3%(V71.9.0 已測)・投量比 >10%(V71.9.5 已有)・均線扣抵 / 葛蘭碧 / 光頭大紅棒 ≥5%(偵測器都在)・
權證差槓比 <0.3%(**無權證報價源**,V73.4.0 實測仍 422)・盤前試撮(無逐筆歷史)・庫藏股(無資料源)。

#### 📊 `scripts/kobo_probe.mjs` 實測(2,233 檔 / 236,262 事件 / 對照組 561,163 個股·日,六道關卡)
| 事件 | vs 對照(20日) | 前後半同向 | 逐年同向 | 去最好年 | 扣成本 0.44% | 判定 |
|---|---|---|---|---|---|---|
| 帶寬 <5%(他說「很窄」) | −0.41pp | ✅ | ❌ | −0.73 | −0.85pp | ❌ |
| 帶寬 10~20%(他說「正常」) | −0.02pp | ❌ | ❌ | −0.05 | −0.46pp | ❌ |
| **帶寬 >20%(他說「就寬」)** | **+0.42pp** | ✅ | ✅ | **+0.37** | **−0.02pp** | ❌ **卡在成本** |
| **帶寬自身最低 20%(壓縮/夾娃娃)** | **−0.89pp** | ✅ | ✅ | −1.12 | −1.33pp | ❌ **全表最差** |
| 壓縮後開始擴張 | −0.72pp | ✅ | ❌ | −0.32 | −1.16pp | ❌ |
| 站上中軌且帶寬>10% | +0.42pp | ✅ | ✅ | +0.30 | −0.02pp | ❌ 卡在成本 |
| **【B】地板(跌≥9%)+ 量≥2倍均量** | **+0.70pp** | ❌(−0.87/+2.06) | ❌ | **−1.03** | +0.26pp | ❌ |
| 【B】地板但量 <2倍(他說不能接) | −0.30pp | ❌ | ❌ | −1.34 | −0.74pp | ❌ |

⭐⭐ **三個值得記住的**:
1. **布林帶寬是單調的:越寬越好**(−0.41 → −0.32 → −0.02 → +0.42)——
   ⛔ 而他最強調的「**壓縮/夾娃娃之後會噴出**」**方向剛好相反**,壓縮那格是全表最差(−0.89pp),
   「壓縮後擴張」也是負的(−0.72pp)。⛔ 別做布林壓縮訊號。
2. ⭐ **「帶寬 >20%」與「站上中軌+帶寬>10%」通過了前四關,只卡在成本**(+0.42pp vs 來回 0.44%)
   → **統計上真的有邊際,但邊際小於手續費**。這正是 V72.0.3「統計顯著 ≠ 值得做」的again。
   ⭐ 而且**增量檢定**(V73.2.5 的做法)實測:帶寬>20% 的樣本裡 **78.4%** 同時也是「高波動」
   → 大半是把 App 已經在用的 `高位階+高波動` 再數一次。⛔ 不加。
3. 🚨 **【B】地板+有量 看起來成立(+0.70pp、方向跟他說的一致、差無量組 1.00pp),
   但「拿掉最好那一年」直接變 −1.03** —— 它整個 edge 靠 2025 那年的 +2.86。
   ⭐ **沒有那道關卡就會誤判成「他說對了」** —— 這是 V73.2.0 那道檢定第二次救場。

⛔ **結論:兩條都不上功能,App 一行判定邏輯都沒改。**
⚠️ 窗口整段偏多頭 + 倖存者偏誤 → 空頭未驗證;走完一次空頭後可重跑(尤其【B】接刀那條)。
⚠️ 這支探針第一版又踩了 **`push(...arr)` 大陣列爆堆疊**(CLAUDE.md V73.2.0 已記過一次)——
   ⭐ 通用:**已經寫進文件的陷阱,下次寫新程式時還是會再踩**,所以測試/守門比文件可靠。

### 📚 外部參考資料的評估紀錄⑥:57 份逐字稿(權證小哥《哥有籌必爆》S1+S2 全系列 + 兆華艾綸說 + 理財達人秀,2026-08-05)
使用者上傳 57 份逐字稿(~1.5MB),問「有沒有厲害的策略…**右下角策略、左側交易、抓到起漲點、題材獵人**」。
⚠️ **先澄清那四個詞**(免得日後有人去找一套不存在的方法):
「右下角」是隨口形容股價圖跌到右下角(= 跌深);「左側交易」只出現在一段 Q&A 且沒有規則;
「題材獵人」是**來賓漢偉哥的稱號**不是策略名。→ 這四個詞指向的其實是**同一件事:跌深的股票要不要撿**。

**✅ 已做(V72.4.9):全市場「地板股家數」= 大盤跌完了沒**
他的原話:「**地板股有大概 100 檔,那大概就是短線的低點**」。
⭐ 這跟個股版 `_detectFloorBounce`(V71.8.9,實測**接刀平均輸大盤**)是**完全不同的問題** ——
那張問「這一檔該不該接」,這張問「**大盤**跌完了沒」。⛔ 別把兩者混為一談或合併。
`floorcount_probe.py` 實測(2,251 檔 × 486 個交易日):

| 地板股家數(不看量) | n(交易日) | 5日邊際 | 10日邊際 | 20日邊際 |
|---|---|---|---|---|
| 10~49 | 118 | +0.12 | +0.36 | +1.64pp |
| 50~149 | 242 | −0.15 | −0.53 | −0.51pp |
| 150~299 | 74 | +0.10 | −0.33 | −0.58pp |
| ⭐ **300+** | **51** | **+1.55** | **+1.44** | **+1.45pp** |

⛔ **四條必須留在卡上的限制**:① **非單調** —— 中間段反而略差,⛔ 不可做成「越多越好」的連續計分因子
② 他說的「**有量** 100 檔」在 2 年窗口**只出現 11 天** → 樣本不足無法驗證,所以主指標改用**不看量**的、
門檻改用**實測有邊際的 300 檔**(⛔ 別「照原話」改回 100)③ 回測窗口整段是多頭(基準 20 日勝率就有 73.6%)
→ ⛔ 不可外推到空頭 ④ 交易日高度重疊 → n=51 不是 51 個獨立樣本。
📍 併進**大盤 → 市場廣度卡**(⛔ 沒開新卡);歷史走 `build_breadth_history` 的 schema self-heal
**一次回算 250 日**(⛔ 不是從今天開始累積)。測試 `scripts/test_floorcount.mjs` 31 條。

**❌ 實測不成立(⛔ 別再做一次)**
| 他的說法 | 實測(`turnover_stage_probe.py`,2,236 檔) |
|---|---|
| 「**低檔**高週轉(>20%)= 起漲」(S2 #19) | 低檔各週轉桶邊際 −0.06 ~ +0.09pp = **雜訊**,不成立 |
| 「**高檔**高週轉 = 出貨、不是好事」(同上) | 高檔 20~40% 桶邊際 **+0.44/+0.58/+0.87pp** —— **方向剛好相反** |
| 「**月線斜率 ≥1%/日 = 飆股**」(兆華艾綸說 07-08) | 5/10/20 日邊際 **−0.67 / −1.06 / −2.07pp**,n=5,207,**越陡越糟** |

⭐ 月線斜率那條特別值得記:**勝率反而較高(42.8% vs 基準 39.5%),但中位報酬更負** ——
又一次「**常對但賠更大**」(同 V72.0.3 的教訓)。⛔ 看到勝率高就放行,是這個專案最容易再犯的錯。
⚠️ 週轉率這次是**第二次**測(V72.0.1 `turnover_probe.py` 測的是「週轉率 × **昨日漲跌幅**」)——
⭐ 通用:**同一個指標配不同的第二變數,是不同的假設,要分開測**;但也要記得**前一次測過什麼**,
別重複測同一組(這次是刻意補測「× 位階」那組)。

**✅ 已經有了(逐條對照過,⛔ 不用做)**
地板股個股版(V71.8.9)・融資維持率 130%/追繳價 0.78×(V71.7.1 + V72.0.3)・
兩上兩下大戶散戶融資(V71.9.0/V71.9.7)・CB parity(V71.9.1)・投量比 >10%(V71.9.5)・
關鍵分點 + 地緣分點(V71.9.8)・集保戶數(V71.9.7)・軋空券資比(V71.9.2,**已驗不成立**)・
董監申報轉讓(`insider_miner.py`)・分批進場三步驟(= 艾綸「試探→確認→確立」,positionSizerCard)・
位階溫度計(= 他的「打幾折」)・處置股官方名單。

**⏳ 唯一真缺口但這次決定不做:融資使用率(融資餘額 ÷ 融資限額)**
他的「4 大融資指標」我有三個(增減/餘額/維持率),**只缺使用率**(>70% 天險、>50% 留意)。
⭐ 資料**其實拿得到而且零額外 API** —— TWSE `MI_MARGN` 同一份回應就有「融資限額」欄
(OpenAPI 是 `MarginPurchaseLimit`)。⛔ **但這次不做,三個理由**:
① `margin_balance` 是走 **SQLite `stock_history` 中介**進 `data/{sym}.json` 的 → 加欄位要動 DB schema,
   風險遠高於「多解析一欄」② **無法回算**(限額是每日快照)→ 只能從今天開始存
③ **無法驗證**:`margin_balance` 本身只回溯到 2026/05,他那三個門檻(20/50/70%)驗不了。
→ 要做的話正確順序是:先確認 DB schema 加欄位不影響既有匯出 → 存起來 → **累積滿 1 年再談門檻**;
   在那之前只能當**算術事實**顯示(同 `_trustVolRatioNote` 的處置),⛔ 不計分、不下方向。

**⛔ 缺資料源,別再評估**:盤前試撮 3 訊號(無逐筆歷史)・
借券/券差成本(無資料源)・可轉債「拉灌吹爆換」炒作階段(需 CB 市價/賣回價,只有轉換價)・
「浪子回頭」處置股回月線(處置系統 V70.3.1 已依使用者要求下架,⛔ 不重建)。

#### 📺 兆華艾綸說那 13 集(V72.5.0 使用者追問「有沒有新的有價值的策略」)—— 結論:**幾乎都已經有了**
逐集比對後,**沒有一條需要新做**。寫下來免得日後再讀一次:

| 他反覆教的 | 我的現況 |
|---|---|
| ⭐ **朱家泓「回後買上漲」完整定義**(f06 + f16 教了兩次):① 多頭回檔且**沒破前低** ② **在月線之上** ③ 出現**大量紅K上漲**;停損固定 5% 或跌破前低 | ✅ `_detectChuLongEntry` **已經是最嚴格版**(波段多頭 + 沒破前低 + 剛站上 5MA + 紅K≥2% + 量增 + 過昨高 + 站月線**且月線上彎**)。🚨 **而且已實測**:`高勝率做多買點` A 級 n=1,825 但**期望值 −0.485%**、`力道偏弱` 版 −0.999% → **這招被教得最多次,實測卻是負期望值**。⛔ 別因為「大師教兩次」就把它升級成主打訊號 |
| **報酬風險比 >3 倍才值得交易**(陳威良 f13/f14) | ✅ `_upsideRoom` 的 `rr` 早就有,而且門檻是 ≥2 划算 / ≥1 普通 / <1 不划算。他的 3 倍更嚴格但**沒有實證來源**,⛔ 不改門檻 |
| 融資維持率 **<135% 恐慌 / <140% 斷頭潮**(f07) | ✅ 全市場維持率已有(V72.0.3,實跑 127.9%)。⛔ **不加那兩條線** —— `margin_hist` 從 V72.2.1 才開始存,**沒有歷史可驗**,加上去就是又一個沒驗證過的預測性門檻 |
| 「**闖紅燈心法**」:能波段翻倍的股「第一天 **99% 都是漲停**」(f14) | ⛔ **敘述反了不能用** —— 那是「飆股→曾漲停」的機率,不是「漲停→會變飆股」的機率(base rate fallacy)。V72.0.1 已實測漲停隔日動能:**只有次日有效**(+1.54%),3/5 日就轉負 |
| 10 年期公債殖利率 4.5% ↔ 油價 120(f10) | ⛔ 他個人的相關性觀察,無實證;而且油價我只有單一數列,驗不了 |
| 台股平均本益比 31 倍 vs 美 32 / 日韓 20~23(f18) | ⛔ **缺資料源** —— 各國市場整體 PE 沒有免費結構化來源。我有的是**全市場 P/B 分位**(V71.6.2) |
| 分批進場「試探→確認→確立」(艾綸 f53) | ✅ `positionSizerCard` 分批進場計畫 |

⭐ **這次最值得記住的一條**:「大師講最多次的那一招」跟「實測有沒有邊際」是兩回事。
回後買上漲被教了兩集、還有專門的教戰日誌,但它在本專案 2 年全市場回測裡是**負期望值**。
⛔ 以後看到「某某老師的招」,先去 `_SIGNAL_EDGE` 查有沒有測過,別急著實作第二份。

### 📚 外部參考資料的評估紀錄③:郭哲榮(摩爾投顧)17 場直播逐字稿(2026-08-04)
使用者問「他為什麼那麼準、有什麼可以借鏡」。三個代理逐字讀完 4/13~8/4 共 17 場直播
+ 新聞,並用真實資料對帳。**結論存這裡,免得日後重讀一遍。**

**他真正的方法(可檢驗的部分)**:
- **低基期**(核心):「這檔股價對應的是幾萬點時代的大盤」——中美晶 230 元時大盤 2 萬多點,
  現在大盤 4 萬多它還是 231 → 買。本質 = 相對位階,跟本專案位階溫度計同族。
- **三五法則**:強勢股回檔 25% 可買/落難績優股回檔 35% 可買/回檔 >50% 不對勁。
  ⭐ `kuo_probe.py` 實測(2,227 檔、扣大盤、去重):**有小邊際但別神化** ——
  30~40% 桶 20 日邊際 +1.33pp/60 日 +2.17pp(勝率 41% vs 基準 33%);
  25% 桶幾乎沒邊際(+0.34pp);⚠️ **他說「不對勁」的 40~55% 桶在本樣本反而最好**(+2.24pp)
  → 那條線不是魔法,只是「跌越深反彈越大」的連續現象。中位數全是負的(樣本落在下跌窗口)。
- **EPS × 20 倍本益比 = 目標價**(= 本專案 V35.7 法人目標價純公式,他用同一招批花旗給國巨 40 倍)。
- **有價有量大型股**(動輒 2-300 億成交量)、**人棄我取**(跌停/處置日買 —— 環球晶 730 跌停加碼)、
  **產業供需只看擴產難度**(記憶體難擴產 → 缺貨鏈上矽晶圓)、韓股頸線/季線當台股領先指標。
- **一億 0050 的真正邏輯**(數據對帳):高點 47,742 → 7/30 低點 39,933 = **-16.4%/五週急跌**;
  40,000 = 整數關卡 + 他要「V 轉的感覺」;選 0050 因為①分析師不能買個股②指數不歸零③拒槓桿
  (韓國 2 倍 ETF -70% 前例)。7/28 預告 → 7/29 盤中破 40,000 買 1,100 張(93.45/93.2,永豐板新,
  可查分點)→ 7/31 +3,187 點、0050 漲停 102.85 → +9.5% ≈ 千萬。
  **本質 = 指數深跌後的正期望 + 扛得住 + 敢公開,不是預知**。

**「準」的另一半是話術結構(⛔ 本專案不學,但要能識別)**:
- **條件式預告的不對稱性**:「跌破 X 我就買」—— 沒觸發=沒錯,觸發後漲=神,跌=長期投資(他自己說「三贏」)。
- **機率化免責**(「我只有 60%」「合格分析師不能說 100%」)+ **雙向皆贏**(「利多繼續漲、利多出盡都對」)
  + **事後選擇性回放**(智原「四天兩根漲停」,少一根歸因颱風假)+ **高頻預告**(「全台灣最敢預告的男人」)。
- 實錘的錯也不少:42,000「多重底不會破」三天後破、外資史上最大賣超 1800 億賭輸(實際 1431)、
  8/1 怕「週一再崩」結果週一收漲 —— 錯的被「修正預測是分析師的權利」吸收掉。

**✅ 已有(不用做)**:位階溫度計、法人目標價、處置股見低點(attention data)、韓股/費半連動
(macro_risk 有 kospi)、期現價差、價格提醒(= 他的條件式預先承諾,使用者自己就能設)。
**⭐ 唯一值得評估的新顯示**:「這檔股價 = 幾萬點時代的大盤」—— 零採礦可算
(個股價格回到歷史哪個時期 → 對應當時 ^TWII 點位 vs 現在),比裸位階 % 更白話。⚠️ 未實測預測力,
要做先掛「參考資訊」。
**⛔ 不學**:all-in 煽動、無停損(指數可以不停損因為不歸零,個股不行 —— 他自己也是個股有停損)、
「最高已經獲利 X%」的績效口徑、複利敘事。

### 📚 外部參考資料的評估紀錄⑤:`aiagents-stock` 開源專案(2026-08-04)
使用者上傳一份 **A股** 的開源專案(Streamlit + DeepSeek 多 AI agent,48,926 行 Python、
100+ 檔、README 69KB),問「有沒有可以優化/參考的」。逐份讀完的結論存這裡,**別再讀一次**。

**⛔ 四個「不能學」——而且都是本專案早就繞開的坑,拿它們當反面教材**:
1. `longhubang_scoring.py` 龍虎榜評分:**寫死游資名單**(赵老哥/章盟主/92科比…)+ 拍腦袋權重
   (頂級 +10 分・知名 +5・普通 +1.5),**零實測**。= 本專案 V71.7.2 已修掉的 `_BROKER_GOD`
   人工標籤問題(實測「美商美林」被標「最兇隔日沖」但真效果 ≈ 0)。⛔ 別回頭去做人工分點標籤。
2. `news_flow_model.py`「接盤總量 = 流量 × 轉化率 × 客單價」:轉化率係數(政策 2.0/利好 1.8/
   漲停 1.8…)、基礎轉化率 0.0001、平均客單價 50,000 元、時間衰減 0.95 —— **全部是編出來的**,
   乘起來得到一個看起來很精確的數字。這正是「⛔ 預測性主張一定要先實測」在防的東西。
3. **整包沒有任何回測模組**(`grep -rl "backtest|回测" --include=*.py` → **0 檔**),
   但 README 寫「净利增长策略…**回测年化100%+**」。唯一的 `win_rate`
   (`value_stock_strategy.py:258`)是模擬交易記錄不是回測,UI 自己還寫「實際交易存在滑點、
   手續費等成本」。→ 對照本專案 `signal_backtest.mjs`(全市場 2,227 檔・扣同期加權・扣來回成本
   0.44%)+ 11 支探針,**方法論領先一整個層級,不需要向它學任何策略**。
4. 美林投資時鐘/康波周期的**象限判定丟給 AI 判**(`macro_cycle_engine.py:93`
   `merrill_lynch_clock_agent(formatted_text)`)→ 違反「禁 AI 算數」鐵律。

**⭐ 唯一真正的收穫:它照出了本專案自己的一個 bug(已修 V72.3.2)**
去比對「它有的宏觀資料 vs 我有的」時發現:`macro_risk.json` 的
`business_signal`(國發會景氣對策信號)= `{'light':None,'score':None,
'error':'Expecting value: line 1 column 1 (char 0)'}` —— **抓不到,而且是陷阱 #23 的簽名**
(站方對不存在的路徑回 HTTP 200 + HTML,拿去 parse JSON 就爆這句)。
🚨 更要命的是 **`scripts/data_audit.py` 的 C 類只掃頂層 `*_error`**,
這個 error 包在 dict 裡 + 頂層 `business_signal_error` 是 None → **體檢一路放過它**。
→ 兩處都修:C 類**往下走一層**掃巢狀 `error`/`err`(⛔ 只走一層,再深會開始誤報);
  `fetch_business_signal` 照 `_taifex_list_endpoints()` 的做法,失敗時記下 content-type +
  回應開頭、並去官方頁面 regex 撈候選端點寫進 error 字串(沙箱連不到 NDC,只能讓官方自己說)。
⭐ **通用教訓:「有 `*_error` 欄位」的檢查只做頂層是不夠的** ——
   把錯誤包進 dict 就等於對體檢隱形。新增任何「回一包 dict 且內含 error」的欄位要記得這件事。

**➖ 它有但本專案更好(不用做)**:多 AI agent 角色分工(我有首席分析 + 模型分工鐵則)、
持倉定時分析(我有庫存 + PWA 推播 + 排程)、K 線技術指標(我有 46 個偵測器**而且有實測成績表**)、
風險評分(我的 `_calcRiskScore` 吃真實 VIX/外資期貨/日圓)、通知(PWA + 鈴鐺 3 天歷史)、
董監質押/內部人(`insider_miner.py` 已有,= 它的「限售解禁/大股東減持」台版對應)。

**⚠️ 它的架構教訓(反面)**:選股功能依賴**同花順問財**,而問財對 requests 做 TLS 指紋識別 →
它得開 Playwright 無頭瀏覽器 + **要使用者自己先登入**才能用。
⛔ 本專案的「全部走官方免費 API、不靠登入態」是刻意的,**別為了多一個資料源去接需要登入的站**。

**🧪 第二輪深挖(2026-08-05,使用者問「還有什麼資料/邏輯可參考」)**
第一輪看策略與回測,第二輪翻了 `.db` 實際資料、指標實作、監控與通知。結論:

- **`.db` 檔全是 A股 開發者自己的測試資料**(龍虎榜 1,731 筆、中國平台熱搜 8,464 筆、
  AI 決策 116 筆…)→ ⛔ 對台股零用處,而且**不該把別人的資料放進本 repo**。
- **技術指標只有 RSI/MACD/KDJ/BOLL**(`ta` 套件現成的)→ 我有 46 個偵測器**而且有實測成績表**,無一可學。
- ⭐ **唯一「我沒有、而且純公式算得出來」的是 ARBR(人氣/意願指標)**
  `AR = Σ(H−O)/Σ(O−L)×100`、`BR = Σ(H−昨收)/Σ(昨收−L)×100`,只要 OHLC → 零採礦。
  照鐵則寫 `arbr_probe.py` 實測(2,227 檔、扣同期加權、去重 20 日、含相對自己位階版):

  | 情境(傳統說法)| n | 10日邊際 | 20日邊際 | 60日邊際 |
  |---|---|---|---|---|
  | AR 穿 150(過熱)| 7,993 | −0.18pp | −0.48pp | −0.61pp |
  | **AR 穿 50(超賣**可買**)** | 6,710 | −0.28pp | −0.34pp | **−1.55pp** |
  | BR 穿 300(極熱)| 1,605 | +0.06pp | −0.76pp | −0.73pp |
  | BR 穿 50(超賣)| 4,715 | +0.23pp | +0.01pp | −1.55pp |
  | AR/BR 雙低(底部)| 5,224 | +0.00pp | −0.07pp | **−1.88pp** |
  | AR 位階 ≥90% / ≤10% | 11,971 / 13,171 | −0.24 / −0.06pp | −0.55 / −0.11pp | −0.56 / −0.53pp |

  ❌ **7 個情境全部不成立**,邊際都在雜訊範圍(±0.5pp)內,而且**「超賣可買」方向跟傳統說法相反**
  (AR<50 的 60 日勝率 22.5% vs 基準 26.1%;AR/BR 雙低 −1.88pp)。
  ⛔ **不做 ARBR**,也別因為「書上都這樣寫」再評估一次。
  ⭐ 這跟 `volstall`/`volseq`(連次量)是同一類:**傳統技術指標的門檻多半沒有實證來源**。

- ⭐⭐ **唯一真正值得借鏡的是一個「邏輯」而不是指標**:它的 `smart_monitor.db` 有
  `ai_decisions` 表,欄位含 `executed` / `execution_result` —— **設計了「回頭驗證 AI 判斷」**,
  但實際上 116 筆全是 `executed=0`、`execution_result=None`,**從來沒真的驗過**。
  → 本專案可以做得比它好,而且**做得成**:V72.4.2 起深度診斷的 `invalidate` 欄位
    (「收盤跌破季線 179.78」)本來就是**可自動驗證的單一條件**。
  ⚠️ **但 AI 的歷史判斷沒存過 → 不能回算**(違反「要馬上就能用」鐵則)。
  ⭐ **可以馬上做的是回測純公式那半**:`_playbookMode`(低檔布局/區間操作/順勢做多/先不做)
    是純公式 → **完全可回算**,可以直接回答「程式說順勢做多時,後續 20 日報酬是多少?說先不做時呢?」
    —— 這是驗證**我自己的功能準不準**,比再加一個沒驗證過的指標有價值得多。⏳ 尚未做。

**⏳ 唯一可評估的候選(尚未做,要先有資料才談)**:
- **投資時鐘四象限**(成長方向 × 通膨方向)本身是**純公式可算**的,它只是實作方式錯了。
  我已有 `business_signal`(景氣對策信號分數)+ `m1b_yoy`,缺通膨那條腿。
  ⛔ **但現在做不了**:`risk_history.json` 只有 **26 筆**、而且 `business_signal` 欄位
  **有欄位但 0 筆有值**(又一次「欄位存在 ≠ 有資料」,同 V72.1.6 的教訓)→ 無法回測。
  → 先把上面那個抓取 bug 修好、讓燈號真的存進 `risk_history`,累積滿 1 年再談,
    **⛔ 在那之前不准上任何四象限卡**(不然又是一個沒驗證過的預測性主張)。
- **庫藏股**:權證小哥那次評估寫「⛔ 無庫藏股公告資料源」。它用問財撈得到 → 至少證明
  這類資料在別的市場是結構化公開的。台灣的「買回本公司股份」也是 MOPS 公告項目,
  而 `insider_miner.py` 已經在打 TWSE OpenAPI。⛔ 沙箱連不到 TWSE **無法驗證**,
  要做就寫一支 `buyback_probe.py` 丟 Actions 跑、先列端點清單,別憑猜的加欄位。

### 📚 外部參考資料的評估紀錄④:權證小哥兩份直播逐字稿(2026-08-04)
使用者上傳【台股止跌了嗎(真反彈還是誘多)】+【神祕分點建倉清單】兩份逐字稿,
問「怎麼運用、放在程式哪裡」。代理逐字讀完 + 探針實測,結論存這裡免得重讀。

**⚠️ 三個「標題 vs 內容」落差(以後看到標題別假設內容)**:
① 標題寫「外資」但全程**沒講外資買賣超**(講的是投信停損/分點/融資維持率)
② 標題寫「鎖碼」但他本人**沒講過這兩個字**(逐字稿的「標的股」是「飆」的辨識誤字)
③「神祕分點」**全片沒有定義**,開場講一次後全改口「關鍵分點」。

**✅ 他講的方法,本專案幾乎都已經有(對照後不用做)**:
關鍵分點低檔布局(=V71.9.8 關鍵分點,他的元大文英新安案例就是「低檔大買」)、
融資維持率(=V72.0.3 全市場維持率 + `_marginCallState`,他只給案例 109 沒給門檻)、
集保戶數暴增(=V71.9.7 四因子的股東人數項)、價籌背離(=主力動向/籌碼乾淨度)、
CB 轉換價 parity(=V71.9.1)、處置聽牌/連兩天第一款(=官方 attention 卡)、
折數比較「打幾折」(=位階溫度計,郭哲榮那份也同族)。

**⭐ 唯一給明確數字門檻的指標已實測落地:「乖離年線 200% 撐不久」**
`bias240_probe.py`(2,227 檔、扣同期加權、事件=向上穿越門檻、去重 20 日):
| 桶 | 10日邊際 | 60日邊際 |
|---|---|---|
| 150~200% | −0.64pp | **−3.61pp** |
| 200%+(n=135)| +0.03pp | **−6.24pp**(中位 −17.9% vs 基準 −11.7%)|
→ **短線(10/20日)完全沒有邊際**(勝率還比基準高 —— 動能還在,看到 200% 就空會被軋);
**60 日才顯現壓力** → 落地成 `_detectBias240`(warn,150% 以下不顯),
文案明寫「中期別重壓的提醒,⛔ 不是放空/立刻賣的指令」。他自己也是配處置/弱勢時機才空。

**⛔ 不做(缺資料源或無法定義,別再評估一次)**:
- 庫藏股實施「前」分點先買(他最看重的一招)→ **無庫藏股公告資料源**
- CB 到期年化殖利率(成德/佐登玩法)→ `cb_overview.json` 只有轉換價,**無賣回價/到期日/CB市價**
- 盤中停損單(「內外盤差很多、一直砍內盤」)→「差很多」無定義,且需逐筆歷史
- 雙刀配對(空處置買同族群)→ 處置系統 V70.3.1 已下架,別重建
- 「離開下軌的紅K」/「布林平行 vs 壓縮」→ 他自己判定都靠目視(「有點像」「只差沒離開」),無法程式化
- 他的全方位選股參數(主5/10/20>0+月線斜率低=剛起漲)→ 概念=飆股雷達已有;分點日史不足無法回測,不另開榜

### 📚 外部參考資料的評估紀錄②:「stock-analysis-team」多代理分析包(2026-08-03)
使用者上傳第二份參考(Coze/Claude Skill,對象是 **A股+美股**,不是台股)。
內容 = 8 個 AI agent 提示詞(基本面/情緒/新聞/技術分析師 → 多空辯論 → 交易員 → 風控一票否決)
+ yfinance/`ta` 抓 MA/MACD/RSI/BB 的腳本 + matplotlib 圖表 + HTML 報告產生器。

**⛔ 核心做法跟本專案衝突**:所有判斷都由 AI 產生(違反「禁 AI 算數」);它是**一次一檔的
報告產生器**,本專案是 2,700 檔的即時終端機;訊號用北向資金/上證指數(台股沒有)。

**✅ 它算的東西我幾乎都有,而且更好(全自動、純公式、全市場)**
- MA5/10/20/60 多頭排列、MACD/RSI/布林、支撐壓力 → 早就有
- 「A股三段式:進攻/均衡/防守 + 建議倉位%」→ 我的 `_chuPositionAdvice()`
  (年線/月線/季線 + 風險指數)已是同一件事,而且接在頂部跑馬燈
- 「風險評分 1-10」→ 我的 `_calcRiskScore()` 0-100,而且吃真實 VIX/外資期貨/日圓/派發日
- 「風控一票否決」→ 我的 regime 已經是「風控優先:跌破月線一律先降級」
- Sharpe/最大回撤/盈虧比 → `backtest.py` 早就有,而且是**扣手續費後**的淨報酬

**⚠️ 它的回測指南其實比本專案已知的還弱**(⛔ 別照它做):
完全沒提 ① 交易成本(ORB 探針的教訓:毛利正、扣成本後全虧)② 乾淨對照組
(sector_flow 的教訓)③ 扣掉同期個股漲跌(broker_habit 的教訓:不扣會得到相反結論)
④ 倖存者偏誤。這四點本專案都已寫成鐵則。

**⭐ 唯一真缺口(已補,V71.8.6):「幾次才算數」**
我原本只寫「樣本少、勝率別當真」,但沒回答**多少次才夠**。
→ `_winRateP(wins, n)` 二項式尾機率 + `_winRateConfidence()` 白話結論:
   假設沒有優勢(勝率 50%),純靠運氣出現這種成績的機率 p。
   p≤5% ✅ 站得住腳 / p≤25% ⚠️ 參考就好 / 其餘 ⛔ 跟丟銅板差不多;n<10 一律「還不能當結論」。
   實例:12 次 67% → p=19%(弱);40 次 68% → p=1.9%(有意義);50 次 52% → p=44%(等於銅板)。
   ⭐ 通用:**任何顯示「勝率 X% ・N 次」的地方都該配這個** —— 同樣 67%,12 次和 40 次
   的可信度差很多。⚠️ 它只檢定「跟丟銅板有沒有差」,不代表未來延續,也未扣交易成本。
   測試 `scripts/test_winrate.mjs`(用課本值釘住:P(≥8/10)=56/1024、P(≥10/10)=1/1024)。

### 🔌 外部參考資料的評估紀錄㉖:六個已連接的 MCP 連接器(2026-09-12 使用者截圖)⭐ 實測推翻我自己前一輪講的話

使用者截圖已連接:**Bigdata.com 17 / Canva 32 / Context7 2 / Figma 42 / Gmail 16 / Zacks Data 9**
(另有未連接的 Alpha Vantage、IBKR、LunarCrush、Similarweb、TomTom Maps),問
「這些可以幫助我的程式嗎?還是還有其它外掛,有推薦的嗎?」

⛔ **沒有憑印象答** —— 當場實際呼叫工具測了四次,其中兩次的結果跟直覺相反。

#### 🚨 四次實測(本輪真的跑過的)
| 測什麼 | 結果 |
|---|---|
| `find_securities("2330")` + 國別 TW | **0 筆** ← 我第一個假設「它沒有台股」就是從這裡來的 |
| `find_securities("TSMC")` / `("Yageo")` | ✅ **有台股**:`XTAI:2330`、`XTAI:2327`、country=Taiwan、ISIN `TW0002327004`<br>⭐ **前一次查不到只是因為查詢要用「名稱或英文代號」,⛔ 不是台股四位數字** |
| 2327 的**券商報告本體**(INVESTMENT-RESEARCH / RATING_REPORT) | ⛔ **0 筆** —— 台股**沒有**原始研究報告 |
| 2327 的**新聞**(fast 模式 + entity 過濾) | ✅ **8 筆全中**(工商時報 / Taipei Times),而且**內文直接有目標價**:<br>「maintains its buy call at **1450 yuan**」・「Yageo Target Price **NT$688**」(元大投顧)・「target price remaining unchanged at **900 yuan**」(第一金投顧) |

⭐⭐ **通用教訓(這條比結論本身有用):查不到的時候,先確認是「真的沒有」還是「查法不對」。**
第一次 `2330` + TW 回 0 筆,如果就此收工,我會寫下「Bigdata.com 沒有台股」這個**錯的結論**
—— 而它其實連 ISIN 都有。同 V73.6.1「卡了五輪的真因是 TPEx 擋 IP,不是格式」。

#### 🚨 要更正 V76.1.0 對使用者說的話(一半)
V76.1.0 我說「台股**沒有免費、整理好的**分析師共識資料源」——
✅ 這句**仍然成立**(免費 + 結構化 = 確實沒有),📖 彈窗那段文案**經查證不用改**。
⛔ **但我讓他以為「外資喊多少我一定查不到」是不準確的** —— 用 Bigdata.com **當場就查到三個目標價**,
只是它是**付費連接器 + 新聞文本**,不是結構化欄位。

#### ⭐⭐ 最關鍵的一條分界(以後有人再問就直接引這條,⛔ 不用重測)
**MCP 連接器只有「Claude 在對話裡」用得到。**
本專案的 App 是 **GitHub Pages 靜態網頁**、採礦是 **GitHub Actions 跑 Python** ——
**兩邊都呼叫不到任何連接器**。所以「這些外掛能不能幫助我的程式」必須拆兩層答:
- **(a) 幫助「Claude 寫程式 / 幫你查」** → 有兩個真的有用
- **(b) 幫助「App 的功能與資料」** → ⛔ **一個都不行**,因為接不進去

#### 判定
| 連接器 | 判定 |
|---|---|
| ⭐⭐ **Context7**(2) | **留**。查函式庫的**當前**文件;實測 ECharts 有 **32,810 段**片段,另有 Playwright / Tailwind。<br>⭐ 直接對應 V69.7.7 的教訓(stub 測試全過、真 ECharts 直接 throw)。只有 2 個工具 → **幾乎不吃 context**,CP 值最高。 |
| ⭐ **Bigdata.com**(17) | **留,但定位要說清楚**:台股**新聞**覆蓋好、新聞內文帶外資/投顧目標價 → **「你問我、我幫你查」**。<br>⛔ 不是 App 的資料源、⛔ 不進採礦、⛔ 不寫進任何判定邏輯、⛔ 台股沒有原始券商報告。 |
| ⛔ **Zacks Data**(9) | 它自己的說明白紙黑字 "covering **North American** companies" → **沒有台股**,對這個 App 是 0。 |
| ⛔ **Canva**(32) + **Figma**(42) = **74 個工具** | 本專案是**單一 `index.html`、深色終端機風格**,CLAUDE.md 明文「版面決策一律以本節為準」「一致性 > 獨特性,這是工具不是官網」。<br>🚨 而且 74 個工具描述**吃 context** —— CLAUDE.md 自己就有慘痛前例(漲到 50 萬字元 → 每個 session 講沒幾句就額度用光)。 |
| ⛔ **Gmail**(16) | 跟這個專案無關(他別的用途要用就留)。 |
| ⛔ 未連接的五個 | Alpha Vantage(台股只有零星 ADR)・IBKR(美股券商,他用永豐)・LunarCrush(**加密貨幣**社群情緒)・Similarweb(網站流量)・TomTom Maps(地圖)→ **全部跟台股無關**。 |

#### 🚨 老實話:這個專案的瓶頸**不在外掛**
CLAUDE.md 記錄的三個真瓶頸,**沒有任何 MCP 連接器解得掉**:
① **GitHub Actions 排程配額**(已用 Cloudflare Worker 繞過,但 ⏳ `GH_DISPATCH_TOKEN` **使用者還沒設**,那段目前整段跳過)
② **FinMind 帳號等級**(台指 VIX / 分點深度都卡在這)
③ **資訊爆炸 / 邏輯打架**(純前端工程,V76.0.x 這一串就是在解這個)

→ 給使用者的兩個行動建議:**關掉 Canva / Figma / Zacks**(一次省 83 個工具的 context)、
**把 `GH_DISPATCH_TOKEN` 設一設**(那個才是現在真正卡住盤中採礦的東西)。

---

### 🔌 外部參考資料的評估紀錄㉕:四個 Claude Code 外掛(2026-09-11 使用者貼社群貼文 + 另一個 AI 的評論)⭐ 兩條建議照做會**弄壞這個 repo**

使用者貼了一篇社群貼文(推薦 4 個外掛,「留言『外掛』我私訊給你」)+ 另一個 AI 對它的完整評論
+ 一份 Windows 安裝步驟,要求照慣例評估。

⭐ **這份跟前 24 份都不一樣**:它談的不是選股策略,是**開發工具鏈**。
所以判準不是六道關卡,而是「**這個 repo 的實際工作流會不會因此更好或更壞**」。

#### 🚨 三件跟原文不一樣的事(全部實查,⛔ 不是推理)

| # | 原文說 | 實查(2026-09-11) |
|---|---|---|
| 1 | 星數 63,153 / 93,548 | ✅ **是真的**(GitHub API:OmniRoute **64,210**、claude-mem **93,640**)。⭐ 我原本懷疑灌水(93.5k 會排進全 GitHub 前 30),**查完證明我自己錯了** —— 這條寫下來提醒:**「這數字看起來太誇張」也是一種推理,不是證據** |
| 2 | 第二個 AI 說 OmniRoute「你的程式碼和提示字**全部經過它**、誰在收集你的數據無法審計」 | ⚠️ **結論對、理由要修正**。README 明寫 local-first、`"no hosted prompt-processing hop"`、telemetry 預設關、憑證 AES-256-GCM 加密。**真正的風險不是 OmniRoute 自己收集**,而是它的 **4 層降級(訂閱 → API → 便宜 → 免費)**:額度用完會自動退到 **56 家 keyless「免費永久」供應商**,那些人的資料政策才是真的審計不了。⭐ 通用:**批評一個工具要打在它真的做的那件事上**,理由錯了下次就防不到同型風險 |
| 3 | 兩個 AI 給的 `frontend-design` 安裝指令**互相矛盾** | ✅ **第二個是對的**:官方市集現在會自動註冊 → `/plugin install frontend-design@claude-plugins-official`。第一則引的 Boris Cherny 貼文寫 `@claude-code-plugins`,那是**先手動 `/plugin marketplace add anthropics/claude-code` 之後**的舊路徑 |

#### 🚨🚨 兩條照做會弄壞這個 repo 的建議(這才是重點)

**① 「把 `CLAUDE.md` 加進 `.gitignore`」—— ⛔ 絕對不可以做**

它的理由:「GitHub Pages 的儲存庫是公開的,不排除的話你的架構筆記和進度會在網路上公開」。
實查:`git ls-files CLAUDE.md` → **已在版控**;GitHub API → repo `"visibility":"public"`。

**四層錯誤**:
- 🚨🚨 **Claude Code on the web 每個 session 靠 repo 裡的 `CLAUDE.md` 載入專案規則。**
  移出版控 → **四驗證、41 條陷阱清單、燈號鐵則、部署規則、探針登記表、連動檢查清單全部消失**
  —— 而那正是使用者實際的工作方式(V75/V76 全部是在 web session 做的)。
- **`.gitignore` 不會讓已經 push 出去的東西消失**(還在 git 歷史裡)→ 它想達到的目的**根本沒達到**,
  代價卻是真的。⭐ 通用:**「停止追蹤」跟「已經公開的收回來」是兩件事。**
- ⭐ **歸因錯了**:公開的原因是 **repo 本身是 public**,跟 Pages 無關;
  而且 **main 分支的內容從來不會被部署到網站**(`deploy_pages.yml` 只覆蓋 `index.html` + `sw.js`,
  gh-pages 其餘是 `data/`)→ `CLAUDE.md` **一次都沒有出現在網站上過**。
- ⭐⭐ **那個隱私顧慮本身是假的**:所有判定邏輯與回測成績(`_SIGNAL_EDGE`・`_STREAK_EDGE`・
  `_DECK_TRACK49`・`_SCR_EDGE`…)**本來就嵌在公開的 `index.html` 裡**,而那份是真的掛在網站上。
  藏 `CLAUDE.md` = **假隱私、真代價**。
- ✅ 真正該守的是**金鑰**,而這條本專案早就做對了(「⛔ key 絕不硬編進 index.html」,
  金鑰全在 GitHub Secrets)→ `CLAUDE.md` 裡**沒有**任何金鑰,不用改。

→ 📌 已把「⛔ `CLAUDE.md` 不可加進 `.gitignore`、不可移出版控」寫成 CLAUDE.md 的明文鐵則,
   免得下一個 AI(或下一個 session)再建議一次。

**② 「新增 `PROGRESS.md`」—— ⛔ 這個 repo 加它會製造第三份真相**

本專案已經有 `CLAUDE.md`(現行規則)+ `docs/DECISIONS.md`(188 節,**含「📋 待辦盤點」**)。
再開一個 `PROGRESS.md` 寫「做到哪」= 第三個地方 → 正是使用者講最多次的
**「邏輯不打架、資訊不爆炸」**的反面(同 V69.7.5 主力成本、V72.0.8 三個「主力」那類同名不同義)。
⭐ 但「收工前更新進度」這個**行為**是對的 → 接到**既有**的 `docs/DECISIONS.md` 待辦節,⛔ 不新開檔。


#### 🎨 追記(同日稍晚,使用者說「好,那幫我裝 frontend-design 吧」+「網頁介面專業美觀要裝什麼」)

**① 我原本說「我裝不到」是錯的 —— 專案層裝得到,而且那正是官方對雲端 session 的指定做法**

| 查證 | 結果 |
|---|---|
| 容器有沒有 `claude` CLI | ✅ **v2.1.268**,有 **`claude plugin install <p> --scope project`** shell 指令(⛔ 不必打 `/plugin`) |
| 官方市集在雲端 session | ⚠️ `claude plugin marketplace list` 一開始是 **「No marketplaces configured」** —— 自動註冊只發生在**互動式**啟動 |
| 官方文件 | 逐字:「If Claude replies that `/plugin` isn't available in this environment, … or **declare the plugin under `enabledPlugins` in `.claude/settings.json` for cloud sessions**」 |

🚨 **scope 只能選 project,⛔ 不能選 user**:容器是 **ephemeral** 的,`~/.claude/` 下次 session 就沒了。
⚠️ 而且 `claude plugin install --scope project` 只寫 `enabledPlugins`,**市集是寫進 user settings** ——
新的雲端 session 會拿到一個「指向不存在市集」的設定 → **手動把 `extraKnownMarketplaces` 也補進專案設定**,
讓 `.claude/settings.json` 自給自足。⚠️ `.claude/` 在 `.gitignore` 裡 → **`git add -f`**(⛔ 漏了就只活在這台機器上)。

**② 🚨 我對這個外掛的描述**(上一則寫進 CLAUDE.md 的那句)**是錯的,而且錯得離譜**

我引的是**部落格的二手摘要**(「bold aesthetic choices・distinctive typography・**high-impact animations**」),
說它「跟視覺降噪鐵則**直接對立**」。⛔ 讀完它真正的 `SKILL.md` 之後:**它其實是降噪派的**——
> 「**Spend your boldness in one place**,其餘保持安靜克制,**砍掉任何不服務主題的裝飾**」
> 「**gradient washes as decoration**」= 它列為「一眼看出是 AI 做的」tell(**跟本站「⛔ 禁用霓虹漸層」同一邊**)
> 「非使用者觸發的動畫要**極少且刻意**;每段 fade-and-slide-up、每張卡 hover 特效 = AI 味」

⭐⭐ **通用教訓(這次自己犯的)**:**引用要引一手來源**。二手摘要會把意思帶偏,
而我還把那個偏掉的版本**寫進了 CLAUDE.md 的鐵則區** —— 那會變成長期誤導,比講錯一次嚴重。
⛔ 下次寫「某個外部工具的取向是 X」之前,**先把它的原始檔讀過**(這次只花一個 `find` + `head`)。

**③ 真正的衝突是另一件事,而且更具體**(已改寫進 CLAUDE.md UI 規範那節)

它把三樣列為「AI 生成的 tell」,而那三樣正是本專案**刻意**的設計(仿券商 App):
near-black 背景 ・ 一堆一模一樣的圓角卡 ・ 小資料標籤用等寬字。
⭐ 判準:它服務「**做出不像 AI 做的網頁**」;本站是**每天用的密集型金融終端機**,
**一致與可掃視 > 獨特** → 版面決策以 CLAUDE.md 為準,它的「克制/砍裝飾/動畫要有理由/標籤不全大寫」照收。

**④ 「網頁介面專業美觀要裝什麼」的誠實答案**

- **官方就這一支**(`frontend-design`),⛔ 不用再裝別的;`figma` / `canva` 這個帳號**已經連著**了。
- ⭐⭐ **但對 StockAI-DB 來說,真正的瓶頸不是「美不美」是「資訊密度」**:
  使用者反覆講的是「版面不好看 / 文字太多 / 資訊爆炸」,而 `scripts/card_inventory.mjs`
  實測 2330 **單頁攤開 12,419 字**(最重四張 playbookCard 1,522・chipVerdictCard 1,446・
  chuMergedCard 1,283・dayTradeBody 1,276)。
  ⛔ **裝任何設計外掛都不會讓那 12,419 字變少** —— 那要靠既有的 `_TIDY` 精簡機制與砍卡流程。

⚠️ **這個 session 不會立刻生效,而且我沒辦法讓它生效**:啟用要 `/reload-plugins` 或開新 session,
而官方文件寫明該指令「從遠端連線送進來時會拒絕執行」→ ⛔ 不假裝已經啟用。

#### ⚠️ 環境錯配:那份安裝步驟改不到這個專案

安裝步驟是給**使用者的 Windows 機器**(`C:\Users\23\Desktop\AI專案區\`)本機 Claude Code 的。
但這個專案實際上是在 **Claude Code on the web(雲端 Linux 容器)**開發 → 我從這裡**碰不到那台機器**。
而且 ③ Playwright 在**這個雲端環境本來就裝好了**(`PLAYWRIGHT_BROWSERS_PATH=/opt/pw-browsers`),
專案更已經有 **128 支 headless Playwright 測試** + `scripts/page_sweep.mjs`(實際渲染後掃 innerText)
—— 比 MCP 版**更嚴格**(有四道空過守門 + `test_sweep_selfcheck.mjs` 注入自我驗證)。

#### 📋 四個工具的處置

| | 判定 | 理由 |
|---|---|---|
| ① OmniRoute | ⛔ **不裝** | 降級到 56 家 keyless 免費供應商時,**程式碼與提示字會送到那些人手上**;「不斷線 ≠ 不降級」(切到非工具型模型時 Claude Code 最核心的讀寫能力會斷)。⚠️ 「違反 Anthropic ToS」這條 README 沒提,**未經證實**,⛔ 不當成事實引用 |
| ② claude-mem | ⛔ **不裝** | 功能是真的(五個生命週期鉤子 + SQLite),但本專案的 `CLAUDE.md` + `docs/DECISIONS.md` 已經做到同一件事,而且**是 git 版控的、可以 diff、可以 revert**。🚨 它會記錄**工具輸出** → Claude 讀過含金鑰的檔案就可能把金鑰寫進記憶庫;壓縮本身還要另外呼叫 Claude 吃 token(跟「省額度」這個痛點自相矛盾) |
| ③ Playwright MCP | 🟰 **本專案已經有,而且更好** | 他**其它**專案值得裝;本 repo ⛔ 不新增 `.mcp.json`。⚠️ 頁面快照很吃 token,就算裝了也該驗收時才開 |
| ④ frontend-design | ✅ **已裝(專案層)** | Anthropic 官方、純設計指引、沒有執行風險。⚠️ 我第一版寫「跟視覺降噪鐵則直接對立」是**引二手摘要引錯了**,詳見下方追記 |

#### ⭐ 額度問題的正解(⛔ 不是把流量導去第三方)
`/model` 手動切:雜事 / 跑測試 → Sonnet,架構決策與大改 → Opus。
零風險、零依賴,而且**不會把程式碼送出去**。

#### 🧾 這一輪實際改了什麼
**一行程式碼都沒改**(同評估紀錄⑫⑮)。只動三個文件:
本節 ・`CLAUDE.md`(索引 + 兩條新鐵則)・`.gitignore` 加 `.mcp.json`
(MCP 設定會帶**機器專屬絕對路徑**,commit 進去會讓雲端 session 拿到壞掉的設定)。

⚠️ **誠實標註**:那條「Windows 上 `npx` 會讓 MCP 的 stdio 接不上 → 改用 `node <path>/cli.js`」
**我在 Linux 沙箱無法實測 Windows**(陷阱 #40:別的環境測得過不算數)→ ⛔ 我沒有替它背書。

### 🧪 外部參考資料的評估紀錄㉔:兩套完整策略規格 + 回測框架(2026-09-08 使用者提供)
使用者給了兩套寫得很完整的策略(布林壓縮突破 / 強勢股多頭拉回)+ 一份績效報表規格,
指定用 `vectorbt` / `backtrader` + `plotly`。⭐ 照鐵則:**先回測再談要不要做框架** ——
策略是負的話,再漂亮的框架也只是把賠錢的東西包起來。

#### ⛔ 沒有裝 vectorbt / backtrader / plotly(三個都是刻意的)
| 他要的 | 判定 |
|---|---|
| vectorbt / backtrader | ⛔ **不裝** —— 本站已有一支跑得動全市場的回測引擎(`portfolio_backtest.mjs`,已測 120 種變體、六道關卡、每月損益、逐年拆解)。再裝一個 = 同一件事有兩份真相。照評估紀錄⑦ 的原則:先問「我需要它的**哪一個函式**」→ 只有「績效統計」那一塊 → 自己寫 200 行(`scripts/lib_perf.py`) |
| plotly | ⛔ **不裝** —— App 的圖表是 ECharts;`lib_perf` 輸出**自足的 HTML**(同一套視覺語彙、台股紅漲綠跌),⛔ 不引入第二套繪圖庫 |
| pandas / numpy | ✅ **沙箱本來就有**(pandas 3.0.5 / numpy 2.4.6)。⚠️ CLAUDE.md V73.4.3 記的「沙箱沒有 numpy」是**舊環境,已過期** |

#### 📊 `scripts/lib_perf.py` —— 這是本站**真的缺口**(現有回測只算累積損益/回撤/每趟)
`generate_performance_report(trades_df, equity_curve)` → 總報酬 / CAGR / 勝率 / 盈虧比 /
獲利因子 / 期望值 / 最大回撤 / 夏普 / 平均持有天數 / **最大連續虧損筆數** / 月報酬矩陣 + HTML 圖。
⛔ **四句限制跟著數字一起回傳,⛔ 不可只給數字**(測試 ⑫~⑯ 釘住):
① 夏普的**無風險利率設 0**、日報酬 ×√252 → ⛔ 不可跟別人算的直接比
② **CAGR 在短窗口會誇大** → 一律同時輸出窗口月數
③ **樣本 < 10 筆標「不能當結論」**(同 `_wrEnough`);⚠️ 樣本夠時語氣要換掉,⛔ 不可永遠在道歉
④ 「這支不會替你扣成本」—— 扣沒扣取決於呼叫端的 pnl
⚠️ **月報酬矩陣用「月底權益的變化」算,⛔ 不是把當月每筆交易損益加起來** ——
   後者在有複利/同時多檔部位時對不起來(同「快照檔不可沿時間軸加總」)。
   而且**第一個月要用期初權益當基準**,⛔ 不可讓它憑空變成 0%。
測試 18 條,每一條都是**手算得出唯一答案**的(勝率 2/5、盈虧比 16/6、獲利因子 32/18、
最大回撤 99/110−1、最大**連續**虧損 2 筆而不是總虧損 3 筆)。

#### 🚨 回測結果:**兩套都輸給對照組或打平,⛔ 都不做**
`scripts/strat_probe.py`(2,363 檔 ・2021-09 ~ 2026-09 ・60 個月含 2022 空頭)。
🚧 五個守門:**進場 = 訊號日隔天開盤**(訊號用收盤價算,當天收盤買不到)・排除開盤鎖漲停・
成本雙邊全扣(手續費 0.1425%×6折×2 + 證交稅 0.3% + 滑價 0.1%×2)・
🚨 **對照組共用那條腿**(同一批股票、同一段期間、**同一套出場規則**,只有進場日隨機)・
法人那條腿只在「這檔真的有法人資料」的期間算(2022 那段 `foreign_net` 全 0)。

| | 每趟 vs 對照 | 對照組每趟 | 訊號組每趟 | 逐年 | 六關 |
|---|---|---|---|---|---|
| **A 布林壓縮突破**(他的出場) | **−0.426pp** | −0.203% | −0.629% | 23:−0.67 24:−0.51 25:−0.56 26:+0.33 | ❌✅❌❌❌ |
| A(換成本站實測過的出場) | **−0.496pp** | −0.627% | −1.123% | **四年全負** | ❌✅✅❌❌ |
| **B 強勢股多頭拉回**(他的出場) | **+0.301pp** | −0.301% | −0.001% | 22:−0.8 23:+0.31 24:+0.59 25:−0.3 26:+1.04 | ✅✅❌✅✅ |
| ⭐ B(換成本站實測過的出場) | **+0.589pp** | −0.415% | **+0.174%** | 同上但幅度更大 | ✅✅❌✅✅ |

⭐⭐ **兩套的失敗方式完全不同,⛔ 不可混為一談**:
- **A 是進場條件本身沒用** —— 換哪一種出場都輸對照組,而且換成好的出場之後**更差**(−0.496)。
  ⭐ 這是本站對布林壓縮的**第三次獨立驗證**:`kobo_probe`「帶寬自身最低 20% 是全表最差 −0.89pp」、
  `pdf_daytrade_probe`「TTM 擠壓發射 −0.19pp、他的『終極訊號』−0.15pp」→ **方向跟坊間說法相反**。
- **B 是進場有東西、被出場洗掉** —— 贏對照組 +0.30pp,換成放寬的移動出場之後 **+0.589pp
  而且絕對值從 −0.001% 轉正到 +0.174%**。⭐ 正好對上 V74.4.8 的實測:
  **贏的全是「放寬的移動出場」,輸的全是「早點鎖獲利」**(他的 B 出場正是「15 天強制平倉」+ 緊的 2×ATR)。

🚨 **但 B 仍然不做**:① **逐年沒過**(2022 −0.53、2025 −0.37 = 空頭與震盪年為負)
② 絕對每趟只有 **+0.174%**,而 App 現行配置(每天 2 檔 + 🧬 + 尾盤進場 + 跌破 5 日線)
   在同樣 49 個月是**每趟 +0.76%**(⚠️ 兩支的組合模擬配置不同 → ⛔ 只能比每趟,不可比總報酬)
③ 它本質是「拉回買」,而本站實測「等回檔再買」21 個訊號有 18 個是負的。

#### 🚨🚨 順帶把「固定 1.5% 風險模型」再驗一次 —— **第二次得到同一個結論**
B 策略出現一個看起來矛盾的組合:**每筆百分比報酬 +0.174%,金額加總卻是負的**。
真因就是部位大小:風險模型用 ATR 停損距離回推股數 → **波動小的買很多、波動大的買很少**
→ 賺得多的那些(高波動)部位最小。同一批交易只換部位大小:
| B ・出場=本站實測版 | 總報酬 | 最大回撤 | 獲利因子 | 每趟% |
|---|---|---|---|---|
| 1.5% 風險模型(他的規格) | −26.34% | −58.1% | 0.94 | +0.174 |
| ⭐ 等權每筆 15% | **−2.11%** | **−41.1%** | 0.98 | +0.174 |
⭐ 這是 V73.0.1 結論的**獨立複驗**(那次:風險法比等權**少賺 112 萬**、資金使用率只有 59%)。
⚠️ **每趟% 完全一樣**(部位大小不影響百分比報酬)→ ⭐ 那正好證明差別**全部來自資金怎麼配**,
   ⛔ 不是進出場變了。⚠️ 但 `spec` 出場下等權反而更差(−76.5 vs −45.3)→
   **總報酬是資金路徑的函數**(V74.3.8 的教訓),⛔ 只能比每趟,不可拿總報酬排名。

⛔ **App 一行判定邏輯都沒改**(這是回答問題,不是加功能)。兩支腳本留著:
`lib_perf.py` 以後任何回測都可以直接拿來出報表;`strat_probe.py` 走完下一次空頭後值得重跑
(B 的兩個負年正是 2022 與 2025)。

### 🔗 外部參考資料的評估紀錄㉓:四模組「事件鏈交易系統」規格(2026-09-08 使用者提供)⛔ 三個模組早就有,唯一新的那個實測反向
使用者給了四個模組:①資料與特徵引擎 ②事件鏈狀態機 ③機率與回測引擎 ④盤中監控與盤後推播。
⭐ 照鐵則:**先問「它有沒有我沒有的東西」**,再談要不要做。逐項對照:

| 規格 | 本站現況 | 判定 |
|---|---|---|
| ①-1 K線/三大法人/主力籌碼/融資融券/MOPS 申報轉讓 | `data/*.json`(2021 起)・`chips/`・`insider_miner.py` | ✅ 早就有 |
| ①-2 新聞爬蟲 + 情緒評分 | `universal_radar` 已用 Groq 判 pos/neu/neg;`news_hist.json` V74.2.8 起累積(**目前 8 天**) | ✅ 有;⚠️ 情緒對報酬的預測力**已測過不成立**(評估紀錄⑰:利多 +0.60 / 利空 +0.32 兩端同號) |
| ①-3 族群分類 + 族群動能 | `PRO.THEMES` + `stock_tags`(殘差相關)+ `sector_rot` 20 日動能(**+1.44pp 六關全過**) | ✅ 早就有,而且有實測 |
| **②** 事件鏈狀態機(利多 → 籌碼進駐 → 帶量突破)+ 觀察名單日誌 | ❌ 沒有 | 🚨 **實測反向,不做**(見下) |
| ③-1 依股本/波動度動態調條件 | 🧬 位階×振幅(唯一六關全過的個股條件);成交額分層 | ✅ 有;⚠️ 這次分層再驗:鏈的邊際在小/中型股 −0.9/−2.1pp、大型 +0.2 → 「動態調整」也救不回來 |
| ③-2 含流動性限制的回測 | 排除鎖死(limitup/streak/…)・`錯過` 次數(portfolio_backtest)・可交易性門檻 0.15%(`_castPick`) | ✅ 早就有 |
| ③-3 勝率/盈虧比/漲停機率 | `_SIGNAL_EDGE` 129 訊號・`_LU_ODDS` 漲停聯合機率・`_wrTag` | ✅ 早就有 |
| ④-1 盤後挖明日標的 | `playbook_scan` → `playbook_edge.json` → Telegram `playbook` 模式 | ✅ 早就有 |
| ④-2 盤中觸發即時警示 | `_eodTriggerSweep`(13:00~13:28)・Telegram `eod` 模式・V74.8.9 大視窗 | ✅ 早就有;⚠️ 刻意只掃尾盤(V72.9.0:開盤買少賺一半) |

#### 🔗 ② 的前提直接測(`scripts/chain_probe.mjs`,27 秒)
「利多發布」那一段測不了(自己的新聞歷史 8 天;外部那份方向已驗無效)→ 測後兩段。
**2,250 檔 ・帶量突破 19,612 筆 ・對照組 = 所有帶量突破(⛔ 共用那條腿)・進場隔天開盤 ・排除鎖死 ・扣加權 ・10 日去重。**
| 組 | n | 10 日 vs 對照 | 20 日 | 扣成本 | 六關 |
|---|---|---|---|---|---|
| 🔗 **籌碼先行(前 10 天內 5 日法人淨買 ≥ 量 2%)→ 帶量突破** | 16,225 | **−0.14pp** | −0.21 | −0.58 | ❌ 逐年四年全負 |
| **沒有籌碼先行的帶量突破** | 3,387 | **+0.69pp** | +0.99 | **+0.25** | ✅ **全過** |
| 突破當天法人才大買(同步) | 11,562 | +0.28 | +0.27 | −0.16 | ❌ 卡在成本 |
| 🚨 籌碼先行但突破當天法人沒買 | 6,118 | **−0.78pp**(勝率 35.1%) | −0.74 | −1.22 | ❌ |
| 故事停在第 2 階段(進駐但 10 天內沒突破) | 69,266 | 絕對 **−2.69%**(勝率 30.9%) | | | ❌ |
→ **鏈 − 沒鏈 = −0.84pp(10 日)/ −1.20pp(20 日)**;分層:小型股 −0.92、中型 −2.06、大型 +0.21;高振幅 −2.13。
⭐⭐ **籌碼「先來」不是加分,是「已經被買過一輪」** —— 跟本站三次獨立發現同向(隱形吃貨 +0.06・投信+分點又大買 −1.40・法人佔量高的反而不漲停 0.64x)。
⛔ **不做事件鏈狀態機**;「故事演到第幾階段」的日誌若真的要做,只能是描述、⛔ 不可計分。

#### 🚨 探針第一版的對照組是假的(方法備忘)
第一版沒排除「這一檔還沒有法人資料」的日子 → 2022 年(`foreign_net` 全 0)4,119 筆突破**整批落進「沒有籌碼先行」**,鏈 − 沒鏈量到 −1.50pp。
加一道「前 60 天內真的有非零法人資料」守門後對照組 26,794 → 19,612,差距縮成 −0.84pp —— **方向沒變,幅度差快一倍**。
⭐ 通用:**任何「A 有 / A 沒有」的二分,先問「A 沒有」裡有多少是資料還沒開始存**,那些要整批排除(V72.1.6「欄位存在 ≠ 有資料」在對照組上的版本)。
⚠️ selftest 第一版也踩了兩個坑:① 合成籌碼只灌一天 → 5 日累計只有 1% 不過門檻 ② **第 2 階段跟突破共用同一個去重計數器** → 突破前兩天的「籌碼進駐」事件把突破本身吃掉,B 組永遠 n=0。⛔ 不同組要各自去重。

### ⏱️ 外部參考資料的評估紀錄㉑:Gemini「曲博 → 資訊與股價 → 供應鏈前瞻指標」(2026-09-06 使用者上傳 .docx)
198 段 / 7,537 字。前半是「為什麼曲博懂那麼多、他的資訊快不快」(⛔ 沒有可驗證主張),
後半才是重點:**供應鏈前瞻指標 + 3 個「黑科技」演算法 + 建議落地在我程式的哪裡**。
⭐ 照鐵則:**先問「它有沒有我沒有的資料 / 我沒做過的檢定」,最後才看它的結論**。

| 它的「黑科技」 | 判定 |
|---|---|
| ① **時間差交錯相關 TLCC**(星圖改算 `Corr(A[t−k], B[t])`,連線標「領先 N 天」) | ⭐ **純價格就測得動 → 直接測 → ❌ 不成立**(見下) |
| ② **存貨天數二階導數 + 資本支出** | 🚨 **資料實測拿得到(34 季,回溯 2018-03-31)**,但本站一季都沒存過 → 要先開採礦 |
| ③ **財報 Surprise Gap**(前瞻推算營收 vs 法人共識營收) | ⛔ **分母拿不到** —— 台股沒有免費的分析師共識預估(同 ICC 那次);而且它的「70% 機率跳空」無實證 |
| 「晶圓投片量」 | ⛔ **沒有公開來源** |

#### ⏱️ ① TLCC 實測(`scripts/leadlag_probe.mjs`)—— 四個角度全部否定
成交金額**前 400 檔 ・1,214 個交易日(2021-09 ~ 2026-09,含 2022 空頭)**;
🚨 報酬與相關**一律先扣掉當日大盤**(⛔ 不扣的話兩檔都跟著大盤走,落後相關會是大盤自我相關的副產品 ——
同 V74.4.8 stock_tags 的教訓)。

| # | 問題 | 實測 |
|---|---|---|
| ① | 落後相關存在嗎 | 同期 **0.290**,k=1 **0.018**、k=2~10 全在 ±0.02 內 |
| ② | 🚨 **學到的配對穩不穩** | 前半學到 **8 組**、後半 **0 組**、兩段都出現 **0 組**(重疊率 **0%**) |
| ③ | 安慰劑(B 環狀位移 137 天) | k=1 **0.0031** 🆚 真的 0.0178 → 那點東西幾乎就是雜訊 |
| ④ | 他舉的「台積電領先封測廠」 | 上游→下游 k=1 **0.0087** 🆚 **下游→上游 0.0200**(差 **−0.011**)→ **方向反了** |

**⑤ 報酬那段最決定性(A 殘差大漲 ≥5%,進場=隔天開盤,扣同期加權)**:
| 情境 | 20 日 vs 對照 | n |
|---|---|---|
| A漲 且 B 也漲 | +1.61pp | 3,757 |
| ⭐ **A漲 但 B 沒漲(真正的「領先」樣本)** | **+0.81pp**(2022 −0.78 / 2024 −0.93 → 逐年沒過) | 13,362 |
| 🆚 **B 自己漲(A 沒漲)** | **+1.70pp** | 6,831 |
⭐⭐ **那點正報酬完全來自 B 自己的動能;知道「A 漲了」不但沒加分,還比不知道更差**
(0.81 < 1.70)—— 又一次「追強 > 抄底」,而 TLCC 沒有帶來任何新資訊。
⛔ **不要在星圖上加「領先 N 天」的箭頭**:那條線畫得出來,但它明年會指向完全不同的股票。

#### ⭐⭐ 兩條方法論(比結論本身重要)
① **「先報名單穩定度,再談報酬」第三次救場** —— 這題跟**分點同盟集團**是同一個形狀
   (先從資料學出一組配對,再拿它去預測)。那次六關 + 格內對照全過,問一句重疊率就結案;
   這次前半 8 組 / 後半 0 組,**連報酬都不用看**。
② 🚨 **對照組拿錯母體,同一張表可以從「全負」翻成「全正」** ——
   第一版對照組用「成交金額前 120 檔」,而事件裡的 B 是「星圖收錄的高相關股」= **兩個母體**
   → 整張表 −0.36 ~ −1.02pp;換成**同一批 B、同一段期間、不看條件**之後變成 −0.16 ~ +0.68pp。
   ⭐ 通用:**對照組必須跟事件組共用「非受測的那條腿」**,否則量到的是「哪些股票比較好」。

#### 🚨🚨 ② 存貨 / 資本支出:**「本站沒有這些資料源」這個直覺是錯的**
⛔ 差點就照直覺回「沒有資料源」。照鐵則去**實跑**(`finmind_check.py` 加一段財報歷史深度,
只讀、不寫檔、不碰 gh-pages/data),2026-09-06 實測 2330:
```
▸ 資產負債表 Inventories(存貨)                       34 季 ・最早 2018-03-31
▸ 現金流量表 PropertyAndPlantAndEquipment(資本支出)   34 季 ・最早 2018-03-31
▸ 損益表     CostOfGoodsSold(營業成本 = DOI 的分母)   34 季 ・最早 2018-03-31
```
⭐ 也就是 `存貨週轉天數 = 存貨 ÷ 營業成本 × 90` **算得出來,而且有 8.5 年**。
⛔ **但本站一季都沒存過** → 要開一支季頻採礦(一檔 3 次請求 × 約 2,300 檔)才驗得動 →
**照規則要先問使用者**(純探針不用問,「開始存新資料」要問)。
⚠️ 而且「存貨天數加速度轉正 = 落底訊號」是**沒有實證的預測性主張**,存到之後要過六道關卡才准上前端。
⭐⭐ 通用(又一次):**「我掃過一遍沒有」只等於「我用的那個掃法沒有」** ——
   下「沒有資料源」的結論之前,先去**實跑問一次**。

#### ➖ 它建議的落地位置(逐條對照,⛔ 多數已經有了)
「個股卡加前瞻健康度標籤」→ 要 ② 的資料才做得了;「板塊輪動加庫存週期象限」→ 同上;
「今日訊號加預期轉折爆發池」→ 同上;「關聯星圖加領先箭頭」→ **① 已實測否定**。
⭐ 它說「不需要盲目開太多新分頁」這個判斷**是對的**(同 V72.6.0 的教訓),但它列的優化**全部卡在資料**。

### 🕸️ 外部參考資料的評估紀錄⑳:Gemini「產業星鏈應用與觀察指南」(2026-09-05 使用者上傳 .docx)
使用者:「我的產業作戰室相關星鏈想要優化,需要區分頁面嗎?給你決定要怎麼優化」。
內容兩半:前半是**現有星圖的使用指南**(4 大策略),後半是 Gemini 產的一大段 **React 元件程式碼**。

#### ⛔ 先回答「要不要分頁」:**不分頁**
關聯星圖是「**查一檔 → 看它的族群**」的**單一任務**,分頁只會讓使用者多點一次;
而且產業作戰室已經 8 個分頁。⭐ 這跟 V72.6.0 的教訓同型:
**痛點是「頁內少了什麼」不是「分頁數量」** —— 那次把 8 個 sub-tab 合併成 6 也是錯的路。

#### ⛔ 那份 React 元件的 6 個區塊,**5 個沒有資料源**(⛔ 別再評估一次)
營收<b>產品別</b>拆解與「純度分數」・產線據點/擴產時程/產能・ASP 與進入門檻・
供應商梯隊(已量產/送樣中)・實體上下游對照表 —— **台股都沒有免費結構化來源**
(評估紀錄① 已寫過 Revenue breakdown / 產能利用率 那一批)。
⭐ 唯一有資料源的是 `correlation_star_map` —— 而那本站早就有了。
⛔ 硬做就是編數字,那正是本站評估外部資料時最常否決的一類。

#### ✅ 但那份指南的 4 大用法,點出兩個**真的缺、而且有資料**的東西
| 它教的 | 判定 |
|---|---|
| ① 驗證攻擊真偽(族群共振 vs 單打獨鬥) | ✅ **缺** —— 表格只有相關係數與位階狀態,**沒有「今天漲跌」** |
| ② 比價效應找補漲 | 🚨 **⛔ 不做** —— 本站實測「在強勢板塊裡挑最弱的」10 日 **−0.40pp 而且逐年全負**(V74.4.1),挑最強的才是 +1.34pp。**而且要把這件事寫在頁面上**,否則使用者會照那份指南做 |
| ③ 控制持股重疊風險(假分散真集中) | ⭐⭐ **最有價值,而且完全沒有** |
| ④ 辨識族群結構屬性 | ➖ 描述性,星圖已有 |

#### ⭐⭐ 落地(V74.7.3,全部在 `pro.html` 關聯星圖,⛔ 沒新增分頁、零新採礦)
**① 🔁 「互相」欄(雙向確認)** —— `_starMutual()`:對方的前 N 名裡也有你才打 ✔。
⭐ **零新資料,只是把既有清單交叉比對**,而且**一驗就有鑑別力**:
| 檔 | 互相 | 意思 |
|---|---|---|
| 3231 緯創 | **3/5** | 電子代工族綁得緊 |
| 2603 長榮 | 2/5 | 部分互相 |
| 🚨 **2330 台積電** | **0/5** | **大家跟著它動,但那幾檔自己最像的另有其人**(它像的其實是大盤) |
⭐ 這正好解決指南講的「假分散、真集中」:打 ✔ = **同一個系統性風險**,同時抱等於重壓。
⛔ 0 的時候**要說得出為什麼**(⛔ 不可只丟一個 0 讓人以為壞掉)。

**② 📈 「今天」欄 + 一句結論** —— `_starChg()`:盤中用即時價(重用釣魚池的 `_lqPx`/`_lqStamp`,
⛔ 不寫第二份)、盤後用 `screener.json` 的收盤快照;昨收由 `c ÷ (1+chg/100)` 反推(⛔ 不另存一份)。
結論分三種:**整族一起動 / 只有它自己在動 / 族群內部分歧**。
🚨 講「整族一起動」時**必須同時寫「本站沒有驗證過族群共振能不能預測」**(測試 ②b 釘住)。

#### 🚨 實作時抓到的一個會讓數字造假的細節
第一版把 **`+0.00%` 算成「正的」** → 「0.00% 跟 +3% 同方向」→ 實測 2330 從 2/5 變成 1/4。
→ `MOVE = 0.1`:平盤**分子分母都要扣掉**(⛔ 只扣分子會系統性高估「整族一起動」),
而且要**說出「另有 N 檔今天幾乎沒動,⛔ 沒算進來」**;中心那檔自己沒動 → 整段不談方向。
⭐ 通用:**任何 `a/(a+b)` 的同向比例,都要先問「有沒有第三類(不動/沒資料)被硬塞進其中一邊」。**

#### 🚨 注入驗證抓到的:「有沒有警告」的斷言太寬,**反話照樣通過**
把「🚨 ⛔ 別拿它來『找補漲』」改寫成「**補漲機會**」,而斷言只驗 `/補漲/` 與數字 → **全綠**。
→ 改成釘 `/別拿它來「找補漲」/` **並且**加一條「⛔ 不可出現 補漲機會|尋找補漲|補漲空間」。
⭐⭐ **通用:驗「有沒有警告」時,要同時驗「沒有相反的推薦」** —— 只驗關鍵字出現,反話會過。
測試 `scripts/test_starfamily.mjs` 19 條,**5 種注入缺陷驗過**。

### 🏦🧺 外部參考資料的評估紀錄⑲:Gemini 對話「1張改1股 / 證券股 / 景氣循環股 / 資產配置」(2026-09-04)
使用者上傳第二份 PDF(10,089 字,無附言)。⭐ 照既有做法:**先問「它有沒有我沒有的資料」→ 再問「它的方法有沒有我沒做的檢定」→ 最後才看它的結論**。

| 它主張 | 判定 |
|---|---|
| 📏 **1 張改 1 股會讓台股更活絡 → 提早布局證券股** | ⛔ **前半無法回測**(那是還沒發生的制度變更,**沒有實施日就沒有「前 vs 後」的切點**)。後半可測 → 見下 |
| 🏦 **證券股獲利取決於大盤成交量** | ❌ **孤峰不是高原**(見下) |
| 🔄 景氣循環股清單(航運/鋼鐵/塑化…) | ➖ V74.6.9 已用**產業相對位階**測過(`cyclical_probe.mjs`),⛔ 不用再測一次;它給的是清單、我要的是「現在在週期哪裡」 |
| 🧺 兩套資產配置 + 每半年/每年再平衡 | ❌/⭐ 見下(**再平衡是本站從來沒測過的東西**,值得單獨驗) |

#### 🏦 「大盤量大 → 證券股會噴」—— 通過四關,被**門檻敏感度**當場推翻(`scripts/broker_stock_probe.mjs`)
⭐ 這一題最容易做錯的是**證券股是高 beta**:大盤漲它就漲 → ⛔ 不扣同期加權的話量到的是 beta 不是量能的效果。
標的分三組(⭐ 刻意分開:金控旗下證券只佔母公司一部分,被銀行/壽險稀釋):
獨立券商 6023/2855/6005/6016/6015 ・金控 2885/2890/2883/2881/2882 ・🆚 對照 2412/1101/2317。
窗口 2021-09 起(受 `^TWII.json` 的 `amount` 欄限制)、含 2022 空頭。

**① 同期 vs 隔天相關**:13 檔的「成交金額變化 → **隔天**超額報酬」相關**全部 |r| < 0.1 ≈ 0**
→ ⭐ 日線上根本量不到「量能帶來的額外報酬」,⛔ 這條就已經否定了「提早布局」。

**② 事件研究**(當日成交金額 > 近 60 日均量 ×1.5,20 日去重):
| 族群 | n | 事件 20 日 | 對照(所有交易日) | 邊際 | 扣成本 | 前後半 / 逐年 |
|---|---|---|---|---|---|---|
| 🏦 **獨立券商** | 16 | **+3.48** | −0.28 | **+3.76** | **+3.32** | ✅ / ✅(22:+0.7 23:+4.2 24:+4.9 25:+0.7 26:+5.1) |
| 🏢 金控 | 16 | +0.70 | −0.38 | +1.08 | +0.64 | ✅ / ❌ |
| 🆚 對照(非證券) | 16 | −1.05 | −1.39 | +0.35 | −0.09 | ❌ / ❌ |
⭐ 對照組只有 +0.35 → **「量放大之後大家都會漲」不成立**,券商那 +3.76 看起來是真的。

**🚨🚨 ③ 但門檻敏感度一測就破功**(⭐ 本站鐵則:只測一個門檻不算數):
| 倍數 | n | 扣成本邊際 | 逐年 |
|---|---|---|---|
| ×1.2 | 35 | +0.30 | ❌ |
| ×1.3 | 30 | +0.59 | ❌ |
| ×1.4 | 26 | +0.89 | ❌ |
| ⭐ **×1.5** | 16 | **+3.32** | ✅ |
| ×1.7 | 8 | +1.80 | ❌ |
**只有那一格好,而且也只有那一格逐年全正** → **孤峰不是高原** → ⛔ 不成立。
🚨 而且 **n=16 是「16 個大盤日」不是 16 個獨立樣本** —— 5 檔券商在同一天一起動。

⭐⭐ **通用鐵則(這條比結論本身重要):「六關全過」還不夠,凡是用「超過均值 N 倍」「大於 X%」
定義的事件,都要把那個數字左右各挪兩格再看一次。** 是一片高原才算數
(正面對照:V73.2.3 位階×波動 24 格網格**全部**贏基準)。

#### 🧺 兩套資產配置 + 再平衡(`scripts/alloc_probe.mjs`)
那份對話只給**類別權重**、沒給代號 → ⭐ **每一類各跑兩組不同代號**;
兩組結論不同就代表是**選股**決定的、⛔ 不是配置決定的。
**含息**(走共用的 `lib_totalreturn.trSeries`)、本金 100 萬、窗口 2021-01 ~ 2026-09(5.6 年,含 2022)。

| 配置 | 淨賺 | 年化 | 最大回撤 |
|---|---|---|---|
| A1 領息存股型(0056 50% + 中華電/富邦金 + 00713 20%) | **1,838,314** | +20.3% | −22.9% |
| A2 領息存股型(換成 00878 / 元大金) | 1,946,202 | +21.1% | **−18.7%** |
| B1 資本利得型(0050 50% + 2330/2454 + 1519 20%) | **6,052,726** | +41.4% | **−56.3%** |
| B2 資本利得型(換成 3231 / 2308) | 4,387,042 | +34.8% | −30.5% |
| 🆚 **100% 0050 買了放著** | **3,122,082** | +28.6% | −33.9% |

⭐⭐ **三個結論**:
1. 🚨 **領息存股型兩組都輸 0050**(少賺 118~128 萬),而且**含息之後還是輸** ——
   含息把 A 組拉高 **+7.3~8.0pp**(0050 只拉高 +3.8pp),⭐ 那正好再次證實 V74.4.1
   「以前不含息的數字系統性低估高股息」是對的,**但拉完還是輸**:高配息 ≠ 總報酬高。
   它換到的是回撤 −22.9% / −18.7% vs 0050 −33.9% → **取捨不是變強**。
2. ⚠️ **資本利得型兩組都贏,⛔ 但那不是配置的功勞** —— B1 贏 293 萬、B2 贏 126 萬,
   兩組差 167 萬、回撤差 26pp(−56.3% vs −30.5%)→ **被選股決定**。
   而且 B1 的 −56.3% 遠比 0050 的 −33.9% 難抱。
3. ⛔ **沒扣二代健保補充保費與股利所得稅** → 領息型實際到手更少(差距只會更大)。

#### 🔁 「定期再平衡」—— 本站第一次測,結論分兩半
| 配置 | 不再平衡 | 每半年 | 每年 | 回撤(不/半/年) |
|---|---|---|---|---|
| A1 | 1,838,314 | 1,867,640 | **1,882,754** | −22.9 / −22.1 / −22.2% |
| A2 | **1,946,202** | 1,937,843 | 1,923,671 | −18.7 / −18.3 / −18.6% |
| B1 | 6,052,726 | 6,920,388 | **7,507,486** | −56.3 / **−33.5** / −38.2% |
| B2 | 4,387,042 | **5,255,925** | 4,938,970 | −30.5 / −30.3 / −30.3% |

⭐ **報酬那半:量不出一致的邊際** —— 四組最好的頻率是 每年 / **不再平衡** / 每年 / 每半年,
**四組不一致** → 那些差額是各組成分自己的走勢決定的。
⭐⭐ **回撤那半是真的:四組全部變小**(+0.7 / +0.5 / **+22.8** / +0.2 pp)。
🚨 而 B1 那 22.8pp 講的是同一件事:**不再平衡的話 1519 華城會從 20% 膨脹到 47%**
→ 那時候整個組合已經不是「配置」而是「押一檔」。
→ ⭐ **再平衡真正在做的是控制集中度,⛔ 不是讓你多賺**;而且它跟本站測過的「擇時」不是同一件事
(擇時是判斷多空、實測全部少賺;再平衡不判斷方向)。⚠️ 手續費/證交稅**沒扣** → 那一欄是上限。

#### 🚨 這次踩到的兩個坑(⛔ 都不會報錯)
① 💰 **`dividends_hist.json` 的股利在 `d` 底下**(頂層還有 updated/from/n/caveat)——
   第一版直接 `DIV[代號]` → 每檔都是 undefined → **含息與不含息算出一模一樣的年化(差 +0.0pp)**,
   而畫面看起來完全正常。⭐ 是「那一欄**全部**是 0.0pp」這個**不可能的巧合**把它抓出來的。
   → 現在讀不到 0050 的股利就直接 `exit 1`。
   ⭐ 通用:**讀外部 JSON 前先把頂層的鍵印出來**,並留一條「這個值一定不可能是 0」的自我檢查。
② 版面小事:多行字串在 `console.log('…')` 裡會直接語法錯 —— 一律拆成多個 `console.log`。

⛔ **App 一行判定邏輯都沒改**(這是回答問題,不是加功能);結論已入 `PRO.LAB`(ok 1 / trap 2 / method 2 / blocked 1)。
⏭️ 走完下一次空頭後最值得重跑的是**資產配置那組** —— 5.6 年只有 2022 一次空頭,
   而「領息型回撤比較小」正是要靠空頭才看得出價值的主張。

### 🧪 外部參考資料的評估紀錄⑱:XQ 選股腳本集 4,748 支 + twstock-research 研究儀表板(2026-09-01)
使用者上傳兩個 zip:「附件有選股策略對於有沒有優化」「附件有個跟我的散戶救星很像的程式,把他們優點加進來」。
⭐ **先掃結構再讀**(4,748 個檔不可能逐份看)—— 結論:**借了 2 個方法、0 個分數,新開 1 支採礦。**

#### ① XQStrategy(4,748 支 `.xs`)—— ⛔ 沒有可直接優化的東西
掃完發現 **4,748 支全是同一種殼**:`if GetField("欄位") > X then ret=1` ——
它是 **XQ 付費軟體的欄位目錄**,⛔ 不是策略集(零回測、零進出場、零成本)。
| 它列的欄位 | 本站 |
|---|---|
| 借券賣出、當沖張數、內外盤大單、分公司家數、關鍵券商、籌碼鎖定率 | ⛔ **多數沒有免費資料源**(付費層或無歷史)|
| 投信連買 N 天、主力 N 日合計、振幅連續 N 天 | ✅ 選股頁 170 條裡已有(`fdays`/`tdays`/`streak`)|
⛔ **別為了「補齊欄位」去接付費源** —— 本站的優勢是「哪些指標實測有效」(`_SCR_EDGE` 127 條實測),
⛔ 不是指標數量(同評估紀錄⑩ 對付費工具的判定)。

#### ② twstock-research(FastAPI + Next.js + SQLite,架構跟散戶救星很像)
⛔ **不借的**:多因子短/中/長評分的權重 **0.20/0.60/0.20** 與門檻 **65/50** 全是人訂的
(⭐ **它自己的 reviewer 就警告過不可再往前推**);8 個「雷達策略」本質是分數過門檻 = 陷阱 #38 同族;
而且它自己誠實寫著「回測 2 年多數股票 Alpha 為負」。
✅ **借的兩個是「方法」不是「分數」**:
- **(a) rank IC + Newey-West 信賴區間** —— 拿來驗**本站自己**的釣魚池「實測體質」分數,
  當場把「pp 加總可以排序」這個假設打掉(IC **+0.0004**、CI 含 0、沒贏過只看位階的 +0.015)。
  ⭐ 這是這次最有價值的收穫:**借它的檢定,測掉我自己的功能**。
- **(b) 產業 treemap 熱力圖版面**(磚=成交額、色=漲跌)→ 做進板塊輪動總覽,⛔ 只當事實描述、不下輪動訊號
  (20 日動能才是實測有邊際的那個)。
✅ **它有、本站沒有的資料:除權息紀錄** → 照鐵則先寫探針 `div_probe.py` 問資料源,
再寫 `dividend_miner.py`(V74.3.8 上線,見上面那節)。
⏳ 它的 **Chandelier(最高收盤 − K×ATR)出場** → 已用正式配置實測完(V74.3.8 那節)。

⭐⭐ **通則(第 18 次外部評估,這條可以套到下一次)**:
**① 先問「它有沒有我沒有的資料」→ ② 再問「它的方法有沒有我沒做的檢定」→ ③ 最後才看它的分數。**
⛔ 分數幾乎一定是人訂的(這次兩份都是),**資料與方法才可能是真的收穫**。

### 📰 外部參考資料的評估紀錄⑰:五個台股新聞專案(2026-09-01)—— ⭐⭐ **其中一個帶了可用的歷史新聞,消息面第一次真的回測到了**
使用者一次上傳五個 repo。⭐ 先掃結構找「**有沒有我沒有的資料**」,再談方法論。

| 專案 | 有什麼 | 判定 |
|---|---|---|
| ⭐⭐ **EventSignal**(就事論市) | `data/cnyes_news_2026.json` —— **鉅亨網 5,409 筆、2026-01-02 ~ 07-13、含全文(平均 880 字)** | ✅ **可以直接回測**,見下 |
| twstock-wordcloud | 76 天 / 52,264 則 Google News RSS(2026-06-15 ~ 08-31) | ⚠️ **只有標題與摘要、沒有代號** → 要靠股名比對,精度差;可當未來擴充 |
| tw-hot-sector-news | 60 天(2026-06-30 ~ 08-31),每則帶 `stocks[]`(代號/名稱/重點/漲停旗標) | ⚠️ 對照是 **LLM 抽的**,沒有人工驗證;窗口短 |
| news-predict-stock | 只有爬蟲與模型程式,**無資料** | ➖ 方法跟評估紀錄⑯ 同型(TF-IDF + XGBoost 預測台積電),無新資訊 |
| ClaudeCode_Taiwan_News_Website | 每日新聞的靜態網頁 | ➖ 展示用,無結構化資料 |

#### 🎯 為什麼只有鉅亨網那份能用
它的內文固定寫成「群創 **(3481-TW)**」→ **代號直接印在文章裡**,regex 就抽得到,
⛔ 不必靠股名比對(那會踩「南亞 ⊂ 南亞科」那類子字串問題)。
實測 **78.3% 抓得到代號**、涵蓋 **1,351 檔**、每則平均命中 2.4 檔。

#### 🚨 兩個一定要照做的地方(⛔ 少一個結論就不能用)
① **時間戳要用 `publishAt` 轉台北,⛔ 不可用它的 `date` 欄位** ——
   實測 `date='2026-01-01'` 那筆的實際發布時間是**台北 01-02 20:34**(`date` 是 UTC 日期)。
   ⭐ 差一天在事件研究裡就是前視偏誤,而且**完全看不出來**。
② **進場一律「新聞日之後第一個交易日的開盤」** ——
   實測發布時間高峰在 **17:00~20:00(盤後)**,當天收盤價根本買不到。

#### 📊 `scripts/news_event_probe.mjs` 實測(1,294 檔 ・對照組 16,804 個股·日)
⚠️ 對照組 10 日 **−1.36% / 勝率 36.9%** —— ⛔ 基準不是 0 也不是 50%。
| 事件 | n | 10日 | 20日 | 10日勝率 | 10日中位 | 六關 |
|---|---|---|---|---|---|---|
| 📰 同日 **≥3 則**(被大量報導) | 146 | **+1.59** | +1.97 | 45.2% | −1.38 | ❌ 逐月樣本不足 |
| 📰 同日 2 則 | 1,148 | +0.92 | +1.94 | 43.5% | −2.41 | ❌ 02 月 −3.0;去最好月 +0.35 < 成本 |
| 🟥 標題偏**利多**(規則判) | 989 | **+0.60** | +1.82 | 39.8% | −3.14 | ❌ |
| 🟩 標題偏**利空**(規則判) | 158 | **+0.32** | +0.56 | 43.0% | −1.83 | ❌ 前後半不同向 |
| 📰 **有新聞(全部)** | 3,925 | **−0.03** | +0.43 | 37.7% | −3.69 | ❌ |
| 📰 同日只有 1 則 | 3,411 | −0.40 | +0.26 | 35.8% | −4.03 | ❌ |

⭐⭐ **三個結論**:
1. 🚨 **「有新聞」本身完全沒用**(−0.03pp、勝率 37.7% vs 對照 36.9%)——
   ⛔ 別再假設「上新聞 = 有行情」。
2. ⭐ **真正的變數是「被報導幾則」而不是「利多還是利空」**,而且**單調**:
   1 則 −0.40 → 2 則 +0.92 → ≥3 則 +1.59。那是**注意度**的代理。
   ⛔ 但沒有一格通過逐月檢定,最好的那格 n 只有 146。
3. 🚨 **利多 +0.60 / 利空 +0.32 兩邊都是正的** → 又一次「**兩端同號 = 活躍度不是方向**」
   (同集保隱藏大戶、融資兩端、分點集中度、投量比)。**新聞的方向量不出來。**
⚠️ 而且**中位數全是負的**(−1.38 ~ −4.03)配上正的平均 → **賠率型**,⛔ 不能重壓。

#### ⭐ 探針自己內建的一條揭露(⛔ 別拿掉):**成分集中度**
未去重的原始事件裡「同日 ≥3 則」**台積電一檔就佔 49%、前三大佔 77%** ——
看起來會像「台積電那半年漲很多」被誤讀成事件效果。
⭐ **10 日去重之後前三大只剩 17%**(台積電 8%)→ 去重已經處理掉洗版問題。
⛔ 但那一欄仍然要印:**沒有它就分不出「訊號」與「幾檔股票的表現」**,而且只有印出來才看得到。

#### ⛔ 三個刻意不做的決定
① **⛔ 不把那份新聞資料放進本 repo** —— 那是別人的資料而且 13MB(同評估紀錄⑤ 對 `.db` 的處置)。
   探針吃**路徑參數**,要重跑就把檔案放回 scratchpad:
   `node scripts/news_event_probe.mjs <cnyes_news_2026.json>`。
② **⛔ 不拿這個結論去改 App 的新聞卡文案** —— 我這裡的「利多/利空」是**關鍵詞規則**判的,
   App 那邊是採礦端 AI 判的,**兩者定義不同**(同名不同義鐵則)。
   ⭐ 等自己的 `news_hist.json`(V74.2.8 起累積,存的是採礦端判的情緒)滿 3 個月,
   用**同一個定義**重測一次,才談要不要改文案。
③ ⛔ 不接 twstock-wordcloud 那 5 萬則 —— 沒有代號、要靠股名比對,精度撐不起結論。

### 📰 外部參考資料的評估紀錄⑯:BDA 期中專案「用新聞與社群預測航運股漲跌」(2026-09-01 使用者上傳)
某大學「大數據與商業分析」期中專案(25 家航運股 × 2022-03 ~ 2024-02 的新聞/Dcard/PTT)。
⭐ 它跟本專案剛開工的 `news_hist.json` 正好是同一件事,所以值得逐行讀 —— 結論存這裡**免得再讀一次**。

#### ⛔ 先講最實際的一件事:**它沒有附資料集**
`data/bda2024_202203-202402_內容數據_新聞{1,2,3}.csv`、dcard、ptt、籌碼 **全部在 `.gitignore` 裡**,
壓縮檔裡只有筆記本 + 結果 CSV + 一個 88MB 的 `model.pkl`。
→ ⛔ **不能拿它的歷史新聞來補本專案的空白**(那是我最想要的東西,但不在裡面)。

#### 🚨🚨 它的數字**有前視偏誤**,而且是教科書級的位置:**特徵選取做在切分之前**
```python
X = vectorizer.fit_transform(data['content_cleaned'])          # 用全部資料 fit
ch2 = SelectKBest(chi2, k=500)
X = ch2.fit_transform(X, data['label_day3'])   # 🚨 用**全部資料(含未來月份)的標籤**選特徵
...
if backtest:  X_train, X_test = X[train_range], X[test_range]   # ← 切分在這之後才發生
```
⭐⭐ 那 500 個詞是「**知道未來答案**」挑出來的 —— 而且 `backtest=True` 走的是**同一支** `feature_extraction`,
所以**連它引以為傲的移動回測 58.75% 也是灌水的**,不只是那幾張 80~90% 的模型比較圖。
⛔ 它的模型比較(task1/task2)另外還用 `train_test_split(shuffle=True)` —— 同一天的多篇文章
標籤完全相同,隨機切分等於把答案抄過去。
⭐ **佐證就在它自己的報告裡**:隨機切分 80~90% vs 移動回測 58.75%,差距那麼大本身就是洩漏的訊號。

#### 📊 我重新算了一次它的回測(用它自己的 `predictions_by_date` 逐日檔,417 個有出手的交易日)
| 版本 | 逐日準確率 | 多數類基準 | 贏基準 | z |
|---|---|---|---|---|
| 特徵 500 | 58.75% | 52.04% | **+6.71pp** | 2.74 |
| 特徵 500 + 籌碼 | 60.10% | 51.54% | +8.55pp | 3.51 |
| 特徵 800 | 55.88% | 52.04% | +3.84pp | 1.57 |
| 特徵 1000 | 56.46% | 52.39% | +4.07pp | 1.66 |
⭐ **基準是 52% 不是 50%**(航運那兩年跌的日子比較多)—— 這點它報告裡沒講。
🚨 **穩定性很差**:21 個月裡 **8 個月低於 50%**,月準確率從 **0.36 到 0.92**。
🚨 **加籌碼那 1.35pp 不穩健**:逐月配對比較,加籌碼比較好的只有 **5/21 個月**
   → ⭐ 跟本專案「籌碼**當天的量**不帶方向資訊」的實測一致(`chips_deep_probe` / `limitup_probe`)。
⛔ 而且**完全沒扣交易成本**(每天進出來回 0.44%),也沒換算成錢 —— 同 V72.0.3 的鐵則:
   **勝率贏基準 ≠ 會賺錢**。

#### ✅ 值得帶走的兩件事(⭐ 這才是這份資料的價值)
① ⭐⭐ **它的回測骨架是對的,而且正好是本專案 `news_hist.json` 成熟後該用的規格**:
   **取連續 3 個月訓練 → 預測第 4 個月 → 往後移一個月**(walk-forward),
   而不是隨機切分。⏳ 等 `news_hist` 累積滿 3~6 個月,第一支新聞探針就照這個做,
   ⛔ 但**特徵選取必須放進每一折裡**(只用該折訓練段的標籤),⛔ 不可像它那樣先對全部資料做。
② **標記規則可以借**:「文章刊登日 D → 看 D+n 天的漲跌;休市順延到最近的開市日」。
   ⚠️ 但它的 **±0.9% 門檻是寫死的** → 照本專案鐵則要改成**相對該標的自己的波動位階**
   (寫死門檻套到低波動股會把幾乎所有日子都丟掉;對航運這種高波動族群才剛好幾乎不影響)。

#### ⛔ 不採用的部分
- **對象是「航運族群加權指數的方向」**,不是個股 → 跟本專案「per-stock 打法」不是同一個問題。
- **CKIP 斷詞 + 1~3gram TF-IDF 需要全文**;本專案 `news_hist` 只存 48 字標題(體積考量)。
  ⚠️ 這是已知取捨:評估紀錄⑫ 實測過「媒體報導那類記者已把重點寫進標題」,
  但要做詞袋模型時 48 字會不會不夠,**要等第一支探針跑過才知道**(⛔ 現在不預先加大)。
- **Dcard / PTT**:本專案沒有社群資料源,而且它自己的比較顯示新聞那組最好。
