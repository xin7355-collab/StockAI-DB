...
### 📋 待辦盤點(2026-09-10,使用者問「還有什麼值得做的」→ ⭐ 先量再答)

⛔ 這一節是**待辦**不是決策紀錄;做完一項就把它改寫成決策紀錄或刪掉,⛔ 別讓它變成第二個 LAB `next` 欄
(那一欄的鐵則是「唯一合法的狀態是**還沒做**」)。

#### 🥇 ① 今日訊號榜沒有「買得到嗎」守門(**實測今天就在畫面上**)

`scripts/daily_signal_scan.mjs` 通篇沒有任何流動性判斷 —— `volume` 只被拿來當**排序的第二鍵**
(`bull.sort((a,b) => (b.exp-a.exp) || (b.v-a.v))`),⛔ 從來沒當過門檻。

📊 實測 2026-09-10 那份 `today_signals.json`(bull 35 筆):
| | 筆數 | 例子 |
|---|---|---|
| 🚨 成交量 **0 張** | **2** | 4150(**停牌 6 天**・現價 **1.58 元**)・7863 |
| ⚠️ 不到 100 張 | **10** | 3646 一天只成交 **2 張** |
...
⭐ 為什麼這是真問題,不是吹毛求疵:
- CLAUDE.md 八道關卡裡,**追加的第一道就是「買得到嗎」**(V75.0.9 策略替換規則)——
  而這張榜是全 App **唯一有實測期望值**的榜,標準反而最鬆。
- `_SIGNAL_EDGE` 的成績是**全市場跑出來的平均**,⛔ 沒有扣滑價、沒有分流動性 →
  對一天成交 2 張的股票,那個 `exp` **物理上不可實現**(掛不到單,掛得到也會被滑價吃光)。
- 使用者點進去是**會真的去買**的(榜上每一列都可點 → `analyze()`)。

⛔ **修法上要避開的三個坑**:
1. ⛔ **不可直接把低量的刪掉** —— 同 `_SIGNAL_EDGE` 對 C 級的處置:**收起來 + 說明為什麼**,
   ⛔ 不是藏起來(使用者以為沒有)。
2. ⛔ **門檻不可寫死一個數字** —— 台股冷門股多,寫死 500 張會把正常中小型股一起砍掉。
   ⭐ 判準應該是「**你要買的那個金額,佔它一天成交金額多少**」——
   用 `_lotsForRisk` 算出的張數 × 現價 ÷(當日成交金額)。
3. ⚠️ **`v` 是「最後一根」的量** —— 停牌股那根可能是好幾天前的 → 要配合 `bar_closed`/`d` 一起判,
   ⛔ 別只看單一根(同陷阱 #14)。

#### 🥈 ② LAB 推薦欄違反使用者自己訂的鐵則(3 條裡有 2 條該歸類)

使用者 2026-08-31 明示:「**驗證完成的條目要當場歸類**,從 `next` 欄移進 ok/trap/blocked,
⛔ 不可留在推薦欄掛『✅已測完』」——⚠️ 而現在 `PRO.LAB.next` 3 條裡:
| | 現況 | 該去哪 |
|---|---|---|
| 🔁 出場後「站回 5 日線就買回」 | 「**已測完**,五關過四關 → 目前不改」 | → `trap`(或 `ok` 但標不採用) |
| 🔄 產業週期濾網要不要做成預設 | 「**已測完**,五關過三關 → 目前不改」 | → `trap` |
| ⏳ 三類新資料累積夠了再回來測 | 還沒做 | ✅ 留 `next` |
→ 使用者上次就是**自己抓到**這件事的,⛔ 別讓它再犯一次。

#### 🥉 ③ 兩支新聞採礦過期 12 小時 —— ⛔ 但這不是新問題,別另外修

`global_news.json` / `radar_news.json` 都停在 09/10 04:08(預期 ≤12h)。
⚠️ 它們由 `news_express` 產出,而那正是 V75.2.0 實測「**固定餓死的五支**」之一(`total_count: 0`)。
→ ⛔ **不要去查 news 的程式**(同 V73.9.0 那個誤判);它跟 `live_snapshot`/`tick_flow` 是同一條線,
   由 `6a26efe7` 的 Cloudflare `repository_dispatch` 涵蓋 —— **等使用者設 `GH_DISPATCH_TOKEN`**。
   設好之後若仍然過期,那才輪到查它自己。

#### ➖ 已確認**不用做**的(⛔ 別當成待辦)

- `us2y_error`(FRED 失敗 → yfinance 內插)與 `margin_official_error`(官方維持率抓不到 → 用推估):
  兩個都**有寫 `*_error` 並說明降級方式** = 陷阱 #22 的**正確做法**,⛔ 不是壞掉。
- E2 抓到的「4150 / 7863 的 K 線日期 ≠ 榜單宣稱」:那份 `today_signals` 是 **V75.2.7 之前**的舊碼跑的
  (連 `dropped_stale` 欄位都沒有)→ V75.2.7 的 `_keep` + V75.2.8 的切根**兩層都會擋掉**,
  ⛔ 不是新 bug,下一輪 daily_miner 就會消失。⚠️ 但要**去驗**,別假設。
- 探針登記表裡標「⭐ 走完一次空頭後要重跑」的那一大批:現在窗口還沒出現空頭,⛔ 現在跑等於白跑。
