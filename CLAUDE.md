...
### 📋 體檢已知缺口(2026-07-30 首跑,尚未修完,每次體檢會重複出現)
| 檔案 | 狀況 | 影響 | 判斷 |
|------|------|------|------|
| `market_stats.json` | 沒上 gh-pages | 全市場 P/B 分位數拿不到 → X 光 P/B 退回固定 <2/>5 門檻 | 前端有 silent catch,**功能退化不會壞**;miner 需 P/B 樣本 ≥50 才寫,疑 TWSE PB 拿不到 |
| `chief_ai_cache.json` | 只在 data 分支,沒到 gh-pages | 首席 AI 快取讀不到 | deploy 沒帶過去,要補 |
| `biz_profile.json` | 從沒產出 | 公司側寫 | `theme_news.py` 沒寫出來 |
| `live_quotes.json` | 不在 gh-pages | 即時報價快照 | 盤中才產,需**盤中**再驗一次才能判定 |
| `day_trade.json` | payload `{}`、6/27 起沒更新 | 已無影響 | **V71.5.4 已改讀 `daytrade.json`**,舊檔留備援 |
