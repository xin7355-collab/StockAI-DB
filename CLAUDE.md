...
| 42 | 🚨 **寫在 template literal 裡的 HTML 註解,只要有一個反引號就會把整個字串切斷** | `SyntaxError: Unexpected identifier 'xxx'`、`window.app` 整個不存在 → **全 App 白畫面**,而錯誤訊息指向一個看起來無關的識別字。⚠️ 這是**註解**造成的 bug —— 讀原始碼時眼睛會自動跳過那幾行。V76.0.6 第一次、V76.0.9 同一天**再踩兩次**(兩次都是在寫「這張卡為什麼下架」的說明) | ⛔ 在 `<!-- ... -->` 註解裡**一個反引號都不要用**(要提函式名就直接寫 `_fooBar`,不加引號);⭐ 或把說明寫成 JS 的 `//` 註解放在 template literal **外面**。`node scripts/smoke_test.mjs` 一定抓得到(它是真載入),⛔ 別只 `node --check` |
