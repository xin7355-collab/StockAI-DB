...
- 📖🚨 V76.2.6 使用者三點(改名 / 一鍵複製)+ ✅ **財報三表回算跑完並驗證**(2327:`ni_src=fs`、最新季淨利率 **21.3%**(修前 5.3%)、**ROE 17.2%**(以前算不出來)、面額變更旗標亮在 2025-09-30 並寫明「由官方淨利÷EPS 反推」;⭐ 2025Q3 EPS 9.74→3.10 但**淨利 50.1→63.9 億是上升的** → 完全證實「真因是面額變更⛔ 不是業外」。⚠️ 切片只推 data 分支,要等一輪 daily_miner 才鋪上 gh-pages)—— ⭐ 而他那張新 AI 圖照出**本站自己算錯**:出場線 533.0 標 −2.0%、融資追繳線 532.3 標 **+2.2%**,兩條都在現價下方卻一正一負 → 真因是**兩把方向相反的尺**(`rel` vs `mcs.distPct`),餵給外部 AI 的已統一 ・🎨 提示詞再補三條:⛔ 刻度尺不可用綠→黃→紅漸層(**燈號鐵則也管刻度尺**)/ 估值尺「同業中位」要照價格排進去 / ⛔ AI 不可自己下「位階偏高」判語(數字 38% 卻寫偏高)・⭐ 通用第三次:**提示詞只要留一個空位給 AI 自己判斷,它就會填**
