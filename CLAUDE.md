...
| **顯示「這一檔是不是一般上市櫃股」** | ⛔ 一律走 `app._offListed(sym)`(V77.1.5)—— 回 `null` = **不知道**(清單沒載好),⛔ **不可把「還沒載好」說成「它不在名單裡」**。🚨 實測(2026-09-15)`data/` 2,358 檔個股裡**379 檔(16%)**不在 `stock_names.json`(它的兩個來源是官方上市櫃公司基本資料與每日收盤行情,**興櫃不在那兩份上**);抽樣 40 檔**只有 2 檔有融資餘額**(有名字組 34 檔)、近 20 日中位成交量 **24 張 vs 151 張**、67% 不到 50 張/日。⭐⭐ **該講的不是「沒有中文名」,是「你可能買不到」** —— 這套回測的進場是「尾盤觸價買」,一天成交 24 張(甚至 0 張)那個價買不到、或你一買就把價格墊高 → `_pbRowHtml`(決策台/會賺訊號/釣魚池**共用那一支**)每列多一顆 **🏷️非上市櫃**。⛔ **名單刻意不刪**(同處置股:刪了就不是那個榜真正的成績)。⏳ **⛔ 不可為了補中文名就去加採礦來源** —— FinMind `TaiwanStockInfo` 補得到,但補完 `_offListed` 就失效 =**拿掉了比名字更重要的資訊**;正確順序是**先有「市場別」欄位**(要靠興櫃專屬端點,陷阱 #23 ⛔ 不可猜網址)。測試 `scripts/test_dupnum.mjs` ⓙ~ⓙ3 |
