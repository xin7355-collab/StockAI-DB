...
**再驗時機**:🚨 **原本寫「等外資資料滿 1 年」—— 那個前提是錯的**(2026-09-15 實跑:
`foreign_net` **2023-05-22 起、805 個交易日**,不是「只回溯到 2026/05」)。
⭐ 用 805 天重跑之後,上面那張表的**結論會翻轉**:當初因為 **n=4** 被判「樣本太少無效」的
**C 版(外資 10 日買 ≥6 天)變成最強的**(+3.89pp・勝率 68.6%・146 個獨立事件),
而現在上線的 **D 版掉到第 3**(+1.99pp)。
⛔⛔ **但那組數字現在還不能拿來換,也不可以跟使用者說「可以跟」** —— 那一跑
**完全沒有八道關卡**,而且最嚴重的一點:**平均值是用 n=893 的原始列算的、不是 338 個獨立事件**
(`_episodes()` 只被拿去印,沒拿去算平均)→ **一次長行情被重複計算**。
→ 要換必須先照 `short_probe.py` 補齊:獨立事件平均 / 六關(全期正・前後半同向・逐年同向・
去最好年・扣成本 0.44%・增量檢定)/ 門檻高原檢定 / `--selftest`。
⚠️ 而且**窗口不含 2022 空頭**(`klines_deep` 只有 OHLCV、沒有 `foreign_net`)→ 那一關補不起來。
...
| 腳本 | 回答什麼 | 何時該重跑 |
|------|---------|-----------|
| `sector_flow_probe.py` | 板塊「偷布局」訊號有沒有用 | 🚨 **「等外資資料滿 1 年」那句已作廢** —— 實跑證實 `foreign_net` **2023-05-22 就有、805 個交易日**(舊敘述差 13 倍,見上方資料回算表)。⚠️ 但**現在的結論仍然不能用**:那一跑**完全沒有八道關卡**,而且**平均值是用 n=893 的原始列算的、不是 338 個獨立事件**(`_episodes()` 只被拿去印)→ 一次長行情被重複計算。⭐ 要重跑就得先照 `short_probe.py` 補齊六關 + 門檻高原檢定 + `--selftest`;⛔ 在那之前**不可拿它的數字下結論**。 |
