...
- 📐 **`node scripts/rwd_audit.mjs`(V76.2.4 擴掃 60 頁)** + 🔠 **`node scripts/font_audit.mjs`(15 頁)** —— ⛔ 兩支都 exit 0、不進四驗證。🚧 它們各有「空過守門」與「shim 自我檢查」:看到 `🚨 版面 shim 有規則沒生效` 或 `掃到 0 個可見元素` 就代表**那一頁的乾淨不算數**,⛔ 不可讀成「沒問題」
