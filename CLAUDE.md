...
### 🚨 上櫃股 K 線「最後兩根 close = null」→ screener 只收得到 1,301 檔(2026-08-29 抓到,**尚未修**)
...
⭐ **關鍵線索**:歷史 795 根都正常、**只壞最後 2 根** → 不是解析邏輯壞掉,
是那兩天**上櫃(TPEX)的收盤價沒進來**。⛔ 別去改解析器。
⚠️ 沙箱連不到 TPEX(V73.6.1 已實測整站 403,連本來會通的端點都 403)→ **只能靠 Actions 實跑診斷**。
⏭️ 下一步:① 先看下一輪 `daily_miner` 會不會自動補(補挖機制是全市場重抓,沒有黑名單)
② 沒補上就去 job log 找 TPEX 那段的輸出 —— ⛔ 別憑推理改程式(V73.7.8 的教訓:
   我以為是「官方 API 壞了」,實際是「配對方式錯」,是**對照組**把方向掰回來的)。
📍 **前端已先誠實處理**(⛔ 不可靜默):`pro.html` 戰情表上方會寫「N 檔的數字目前是「—」…
   不是這幾檔沒在動,也不是程式壞掉」。測試 ⑳ 釘住。
