...
#### 🚨 這次我自己犯的一個推論錯誤(要記住)
國巨 2025Q3 季 EPS 9.74 → **3.10**、營收 ↑、成交量中位 ×5.6 → 我推論「疑似分割/股本變動」。
查 `fin_deep` 的 `cap` 欄:**那一季股本一毛沒變**(5,182,415 千元);真正變的是**淨利率 15.4% → 4.9%**(毛利沒掉 → 業外/一次性)。
股本真的變過的是 **2024Q3 現增 +20%**(旗標②亮在那裡是對的)。
⭐ 通用:**有欄位可以直接查的事,⛔ 不可從別的數字推論**;而且斷言寫錯前提時,真檔會當場打臉(測試 ⑦d 第一版就是這樣紅的)。
...
- 📄 V76.1.8 「法人級 22 節提示詞」對照 —— ⭐ 先量:✅ 8 / ⚠️ 9 / ❌ 5(2 節刻意不做)→ ⛔ 不是取代,是骨架 ・⚡ 快速判別表(零現算 + 來源標籤)・🧱 價格牆 + 壓力測試 ・🧮 敏感度(⛔ 沒有機率)・📦 fin_deep 切片接前端(⛔ 不動 daily_miner)・🚨 國巨 2025Q3 EPS 崩的真因是**業外不是股本**(我第一版推錯,查 `cap` 欄才改口)
