...
- 🎤📄 V77.2.8 法說會 V2 —— ⭐⭐ **最重要的一課:「讓官方自己說」那一步,自己要先驗有沒有解析到**(探針第一輪的結論「GET 不行」是**錯的** —— 官方寫 `name = 'step'`(等號兩邊有空白),我的 regex 吃不到 →**靜默退回我自己猜的欄位** → 四筆全「下載失敗」;修好 regex + 加 `parsed_from_form` 旗標後實測**GET 就回 2.67MB 真 PDF**。⛔ 解析失敗靜默退回猜測,會讓「我猜錯」長得跟「官方不給」一模一樣,而下一步完全相反)・🔎 **代號/中文本來就搜得到**(918 列全帶官方簡稱),真毛病是**搜尋被鎖在目前的篩選裡** → 改「打字就搜全部」+ 全形正規化 +名字改用 confcall 自帶的官方簡稱(⛔ 不只靠 `nameOf`)・📄 簡報一鍵開,網址**從採礦樣板組**(⛔ 前端不寫死;決定性對照:改樣板 href 要跟著變)・📊 會後股價統計 **844 場實測:5 日 −1.26pp vs 對照組 −0.73pp = 反而略差**(⛔ 事實不是訊號;⚠️ 80 檔子樣本量到 +1.02pp **方向相反** → ⛔ 子樣本不可外推)・🔎 法人視角八面向提示詞(含「跟上一次法說比說法變了什麼」「管理層避開了哪些問題」;網址帶精簡版、完整版走剪貼簿)・🏷️ 順手修 pro.html 全站 `loadNames` **只有一條腿**且掛掉不揭露 → 三層 ・🐛 兩個真 bug(`confPick` 沒重畫統計 / `build()` 就地改 rows 不冪等)・🧪 測試自己三個「沒有鑑別力」(拿 `CHAIN` 裡的股票驗「名字表載不到」/ 引擎殘留成 meta 讓長度斷言永遠過 / 量測寫在還原之後)
