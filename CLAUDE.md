...
⚠️ **若裝了 Anthropic 官方的 `frontend-design` 外掛,本專案一律以本節為準**(2026-09-11 評估紀錄㉕):它的設計取向是「bold aesthetic choices・distinctive typography・high-impact animations」,**跟上面的視覺降噪鐵則直接對立**。⛔ 它對別的專案有用,但在這個 App 裡不可以蓋掉「純色卡 + 只用 emoji 與左側色條標示」那套。
