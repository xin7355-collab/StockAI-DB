...
**要修只有兩條路,都要使用者動手**(Claude 讀不到也改不了 Secrets 的值):
...
| 欄位 | 在哪 | 為什麼先不接 |
|------|------|-------------|
| `^TWII.json` 的 `amount`(集中市場官方成交金額,元)/ `mkt_vol`(官方成交股數) | V71.6.2 起由 FMTQIK 併入,涵蓋 ^TWII.json 完整歷史(合併後才併,最多 30 個月份檔) | 前端 `_amtState()` 現在用 `breadth.json` 的 `amt`(**上市+上櫃全市場**,實測 07/29 = 1.129 兆);官方 `amount` 只含**上市**,兩個數字不一樣。**同一個概念不可以有兩個數字**(使用者鐵則:邏輯不打架)→ 等下一輪採礦拿到真值,把兩者對照過再決定顯哪一個、標什麼名字。⛔ 別在還沒對照前就加第二個「成交金額」到 UI 上 |
