...
| `kbar5_miner.py` | 📼 **不是探針,是採礦** —— 收盤後把當日量前 80 檔的 5 分 K 存進 `kbar5` 分支(`--selftest` 7 條:時間對齊 / 盤後濾網 / 跨日 / 緊湊化 / **實跑寫入優先** / 每日清單 / 跨月) | ⏳ **它是時鐘不是結論**:上線當天約 80 個交易日,約 **2027-02** 滿一年才做得動「逐年同向」。⛔ 用它的探針**必須**先算「這檔在窗口內有幾天」並把 `bias`(當日量前 80 = 選樣偏誤)寫進輸出;⛔ 不可把結論講得像全市場。⚠️ 改 `agg5` / `split_days` 要重跑 `--selftest`(⭐ 它的測資刻意是 **4+3 根** —— 5+2 時「每 n 根」剛好切在同一處,注入驗證會叫不出來)|
