...
DAYS_BACK = int(os.environ.get('PROBE_DAYS', '120'))
...
def agg5(bars):
    """1 分 K → 5 分 K(⛔ 用時間對齊,不可用「每 5 根」—— 中間缺 K 會錯位)"""
...
def simulate(b5, ei, side, vwap5, stop0):
...
def main():
    ...
    for sym in SYMS:
        ...
        dates = sorted(by_day)
        # ⭐ RVOL 的「過去 N 天同一分鐘平均量」—— ⛔ 只用**該天之前**的天數(零前視)
        hist = {}         # hhmm → [各天的量](只累積到前一天)
        used = 0
        for di, d in enumerate(dates):
            ...
            for b in b1:
                ...
                prev = hist.get(h, [])
                if len(prev) >= 3:
                    avg = sum(prev[-RVOL_LOOKBACK:]) / len(prev[-RVOL_LOOKBACK:])
                    rv[h] = (b['v'] / avg) if avg > 0 else None
            # 當日 VWAP(累計)
            cum_pv = cum_v = 0.0
            vwap1 = {}
            for b in b1:
            ...
            if len(orb) < 5:
                hist_update(hist, b1); continue
            orh = max(b['h'] for b in orb); orl = min(b['l'] for b in orb)
            b5 = agg5(b1)
            if len(b5) < 25 or not (orh > orl > 0):
                hist_update(hist, b1); continue
            on, mom = ttm_state(b5)
            used += 1
            ...
            for i in range(1, len(b5)):
                t = b5[i]['t']; hm = _hm(t)
                if hm < t0 + OR_MIN or hm >= NO_ENTRY_AFTER[0] * 60 + NO_ENTRY_AFTER[1]:
                    continue
                cc = b5[i]['c']
                vw = vwap1.get(hm) or vwap1.get(hm - 1)
                if vw is None:
                    continue
                # RVOL:取這根 5 分 K 內各分鐘的最大 RVOL(附件講的是「這一分鐘爆量」)
                rr = [rv[x] for x in range(hm, hm + 5) if rv.get(x) is not None]
                rvol = max(rr) if rr else None
                above = cc > vw
                brkU = b5[i - 1]['c'] <= orh < cc
                brkD = b5[i - 1]['c'] >= orl > cc
                ...
                def once(k, side):
                    if fired.get(k):
                        return
                    fired[k] = 1
                    put(k, simulate(b5, i, side, vw, stopL if side == 'long' else stopS))
                ...
                if sq_fire and rvol is not None and rvol >= RVOL_HI:
                    once('⑥ 🚨 終極訊號:TTM 發射 + RVOL≥2', 'long' if (mom[i] or 0) > 0 else 'short')

            # ⭐ 對照組:同樣的出場規則,但進場時間**固定挑幾個**(⛔ 不看任何訊號)
            for (H, M) in CTRL_TIMES:
                tgt = H * 60 + M
                idx = next((i for i, b in enumerate(b5) if _hm(b['t']) >= tgt), None)
                if idx is None or idx + 1 >= len(b5):
                    continue
                vw = vwap1.get(_hm(b5[idx]['t'])) or b5[idx]['c']
                g = simulate(b5, idx, 'long', vw, min(b5[idx]['l'], vw))
                if g is not None:
                    ctrl.append(g - COST_PCT)
            hist_update(hist, b1)
        scanned += used
        _line(f'[{sym}] {len(dates)} 天資料 ・{used} 個可用交易日')
