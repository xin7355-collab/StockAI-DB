...
IDX_CLOSE = 13 * 60 + 45        # 13:45 日盤收盤(inclusive)
#   🚨 V77.4.5 更正:最後一根 5 分 K 的 hm 是 **825 不是 820**。窗口含 13:45,
#   而 13:45 自己就是一個 5 分桶的起點([13:45,13:50))→ agg 後多一根只含那一筆的 K。
#   實測 2026-09-17 那根 = [825, 48029, 48091, 48027, 48077, **1488**],量 1488 vs
#   前 60 根中位 398.5、o/h/l/c 四個都不同 → **那是真的期貨日盤收盤那一盤**,⛔ 不是幽靈棒。
#   ⛔ 不可為了讓最後一根變成 820 就把窗口改成不含 13:45 —— 收盤價就在那一盤
#   (同 V73.9.0 第 ④ 條「收盤那一拍必須跑得到」)。
...
def _selftest():
    ...
    #   🚨 V77.4.5:⑧ 釘的是**未 agg 的 1 分 K**,所以「最後一根 5 分 K 是 820 還是 825」
    #   以前一條都沒釘 → 自我驗證那行寫死 820 一直沒被抓到(每天印一行假警報)。
    #   ⭐ 13:45 自成一桶(825),而且它就是**收盤那一盤** → 收盤價必須在裡面。
    ok('⑧d agg 後最後一根 hm == 825,而且它是 13:45 那一筆(收盤價在裡面)',
       k5f and k5f[-1]['hm'] == IDX_CLOSE // 5 * 5 and k5f[-1]['c'] == 3 and k5f[-1]['v'] == 1,
       k5f and k5f[-1])
    ok('⑧d2 ⛔ 對照組:13:50 那筆⛔ 不可混進收盤那一桶(窗口是 inclusive 不是「到 13:50」)',
       k5f and all(b['c'] != 4 for b in k5f), k5f)
    bys = split_days(fut)
...
def main():
    ...
    for code, cont_code in IDX_SYMS:
        try:
            ...
            if by:
                d0 = sorted(by)[0]
                # 🔎 給 Actions log 用的自我驗證:_t() 的時區假設若對期貨不成立,
                #   這兩個數字就不會是 525 / 825(= 08:45 與 13:45,兩端都 inclusive)。
                # 🚨 V77.4.5:期望值原本寫 820,**那是我寫錯的** → 每天在 log 印一行
                #   「最後一根 hm=825(應為 820)」的假警報。⭐ 對得上就安靜一行 ✅,
                #   對不上才大聲講(⛔ 不可反過來讓人養成忽略那行的習慣)。
                _h0, _h1 = by[d0][0][0], by[d0][-1][0]
                _exp1 = IDX_CLOSE // 5 * 5          # 收盤那一筆落在哪個 5 分桶的起點
                if (_h0, _h1) == (IDX_OPEN, _exp1):
                    _line(f'   ✅ 自我驗證 {d0}:{_h0} ~ {_h1}(08:45~13:45,時區假設成立)')
                else:
                    _line(f'   🚨 自我驗證 {d0} 對不上:第一根 hm={_h0}(應為 {IDX_OPEN})、'
                          f'最後一根 hm={_h1}(應為 {_exp1})→ _t() 的時區假設對期貨不成立,'
                          f'⛔ 這一輪的台指期不要拿去回測')
        except Exception as e:
