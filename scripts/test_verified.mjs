...
// 🔁 V77.6.5 修長期紅燈:那張表 V77.0.4 就改寫成「現行出場下,尾盤 vs 隔天開盤」兩列,這條還釘著更早的四列舊數字
//   → 改釘**用意**:尾盤進場一定要贏隔天開盤、而且每一列都要標是哪一種出場(⛔ 不釘數字)
ok('①e 買點:訊號日尾盤買 > 隔天開盤買(⛔ 不是開盤買),而且每一列都標出場',
   V.entry.length >= 2 && V.entry[0].pl > V.entry[1].pl && V.entry.every(x => x.exit), V.entry);
