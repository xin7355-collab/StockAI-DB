...
for (const [w, h, mode] of [[1440, 900, 'desk'], [390, 844, 'phone']]) {
    ...
    // 💼 V77.1.0 使用者明示「庫存股上下間隔寬一點」→ `py-1` 改 `py-2.5`(4px → 10px)。
    //   ⭐ 釘的是**用意**(一頁還看得到夠多列),⛔ 不是釘當初那個實作值 ——
    //   門檻跟著使用者的決定從 56 調到 62(844px 高仍看得到 10+ 列)。
    //   ⛔ 這不是「為了塞新東西調鬆門檻」—— 是他要的那一項改動本身。
    ck(r.rowH <= 62, `③${mode} 庫存列高 ${r.rowH}px > 62 → 一頁看不到 10 列了`);
