...
def main():
    ...
    print('   ⑤ ⭐⭐ 「跳空」那欄有一大半是**同義反覆**(昨晚美股漲 → 台股本來就會開高)——'
          '真正的預測力要看「開盤買收盤賣」那一欄。')
    # ⚡ V77.5.0 每日分數傾印(⛔ 只印不寫產物)—— 給 `scripts/dt_daily_probe.mjs` 在本機拿去跟**個股**
    #   開→收 對齊,回答「盤前分數能不能當個股當沖的大盤濾網」(這支只測過加權,沒測過個股)。
    #   格式:`SCORES|日期:分數,...`(內部 ±10 分;App 顯示 = 50 + 分數×5),每行 100 天。
    for k in range(0, len(rows), 100):
        print('SCORES|' + ','.join(f"{r['d']}:{r['s']:.2f}" for r in rows[k:k + 100]))
