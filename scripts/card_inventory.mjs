...
// ⚠️ V75.2.6 兩個「安靜地量錯」的修正(實跑 2/2 全是誤報才發現的,⛔ 誤報留著會讓人養成忽略它的習慣):
//   ① **否定形要先拿掉** —— 「⛔ **不要直接跟著做**」「本 App 沒有驗證過」被算成「有下指令」,
//      而那正是**最誠實**的寫法(同 page_sweep 的 denegate;本專案已在別處踩過 7 次)。
//   ② **指標名稱不是指令** —— 「ATR**停利**(代理) 2390.00」「唐奇安」那些是**價位的名字**,
//      K線 HUD 整張都是事實(收/開/高/低/均線),一句指令都沒有。
//      → 「停損 / 停利」只有**帶動詞或帶「就」**時才算指令(如「跌破就停損」「建議停利」)。
//   ③ **「買在 / 賣在」後面要接數字才算指令** —— 實跑三處全是**描述**:
//      「因為**買在低檔**的人不是追價來的」「這只說明他**買在哪個位置**」「專**買在相對低點**」。
//      指令長的是「買在 2465 元」那種**帶價位**的。
const _deneg = t => String(t)
    .replace(/(?:不要|別|⛔|沒有|不可|不准|禁)[^。;,\n]{0,26}(進場|加碼|跟著做|追|做多|停損|停利|買|賣)/g, '')
    .replace(/(?:未|沒有|尚未)[^。;,\n]{0,12}(驗證|回測|實測)/g, '');
const RE_CMD = /(可以進場|可依紀律進場|可放心做多|順勢做多|可加碼|分批試單|可以追|反彈減碼|先出場|全數出場|空手觀望|掛單|買在\s*\d|賣在\s*\d|建議離場|建議買進|建議停[損利]|就停[損利]|停[損利]出場|該停[損利])/;
