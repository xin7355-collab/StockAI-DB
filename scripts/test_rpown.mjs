...
if (!r.no) {
    ...
    // 🚪 V77.5.1 名字改成那條線自己的名字(硬停損 / 跌破前低…)⛔ 不再一律叫「你的出場線」(那是硬停損被叫錯名字)
    ck(/現價已在.+之下/.test(r.aboveTxt) && /觸發/.test(r.aboveTxt), `ⓛ 出場線在現價上面(已跌破)時第一行沒說「現價已在…之下 → 已經觸發」:${r.aboveTxt}`);
    ck(!(r.cl != null && r.aboveTxt.includes((+r.cl).toFixed(1))), `ⓛ2 已跌破出場線時,第一行把融資追繳線 ${r.cl} 填進去了 → 跟使用者那張 AI 圖同一個錯`);
    ck(/^跌破\S+ \d/.test(r.belowTxt) && !/你的出場線/.test(r.belowTxt.replace(/你設定的出場線/g, '')), `ⓛ3 出場線在現價下面時第一行沒寫「跌破 <那條線的名字> 價位」:${r.belowTxt}`);
