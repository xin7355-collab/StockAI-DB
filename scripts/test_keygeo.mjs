...
// ⚠️ V74.0.5 起這句從「沒有回測過預測力」升級成**實測數字**(±0.1pp / 11 萬筆,排除雙北也一樣)
//    —— 那是**變好不是變差**,⛔ 所以不可再釘舊句子(釘死會讓「補上實測」那天變紅)。
//    ⭐ 改成釘用意:①⛔ 不可宣稱預測力 ②要嘛寫「沒回測過」、要嘛附**實測數字**,二選一。
ok('⑤ ⭐ 地緣⛔ 不可宣稱預測力(要嘛說沒驗過,要嘛附實測數字)',
   /沒有回測過/.test(html) || (/實測/.test(html) && /百分點|pp/.test(html)), '');
ok('⑤ ⭐⛔ 明說「事實描述不是買進訊號」', /不是買進訊號|事實描述/.test(html), '');
