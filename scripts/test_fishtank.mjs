...
// ⚠️ V76.3.4 這一條原本釘 `−1.44pp`(舊三層漏斗的實測數字)——
//   V76.3.0 已把來源換成**決策台那一套**,那個數字講的是**已經不存在的東西** → 釘它等於強迫文案說謊。
//   ⭐ 改成釘**用意**:① 不可暗示強弱 ② 要講名單從哪來 ③ 要主動講「為什麼跟決策台看到的不一樣」。
ok('🎣⑪b 必須明寫「沒有強弱之分 / 先出現的不代表比較強」+ 名單來源 + 跟決策台的差異',
   /沒有強弱之分/.test(R.picksBar) && /不代表比較強/.test(R.picksBar)
   && /決策台/.test(R.picksBar) && /(不含持股|排到最前面)/.test(R.picksBar),
   (R.picksBar || '').slice(0, 220));
...
// 🔁 V75.1.0 使用者:「已經好幾天釣魚都是一樣魚,這是對的嗎?」
{
  ...
  // ⚠️ V76.3.4 放寬成「那條分支**也要**吐出黏著度那段」——
  //   舊寫法釘死整行原始碼,而 V76.3.4 在單條分支也補了「名單來源」→ 行變長就假失敗。
  //   ⛔ 仍然不可以只 return ''(那就等於單條時什麼都不講)。
  ok('🔁㉘f 只上鉤 1 條時這段仍要出現(⛔ 那正是最容易「老是同一條」的情況)',
     /if \(ps\.length < 2\)[\s\S]{0,400}_castStickyHtml\(ps\)/.test(src));
