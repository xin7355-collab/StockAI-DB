...
// ⚠️ V77.0.4:這條原本寫死「預設那一組**沒有**經過空頭」—— 那是釘**當時的實作**不是釘**用意**
//    (預設組換成 49 個月含 2022 之後,它說「含 2022 年那一段大跌」反而讓測試紅了)。
//    ⭐ 用意是「**每一組都要講清楚它有沒有涵蓋空頭**」→ 兩種說法**任一**都算數,
//    ⛔ 但不可以兩種都沒有(那才是要擋的:短窗口名次被當成永恆真理)。
const bearTold = t => /含 2022 年那一段大跌/.test(t) || /沒有經過空頭/.test(t);
ok('㉞f 每一組都標窗口 + 說出「有沒有經過空頭」(⛔ 不標的話短窗口名次會被當永恆真理)',
   /📅 這一組是拿 .+ 的資料跑的/.test(CALC.body0) && /📅 這一組是拿/.test(CALC.bodyMkt)
   && bearTold(CALC.bodyMkt) && bearTold(CALC.body0), CALC.body0.slice(0, 90));
...
const NEST = await page.evaluate(() =>
  ['tabVal', 'tabChain', 'tabRot', 'tabLab', 'tabConf', 'tabRod'].every(id => !!document.querySelector('.wrap #' + id)));
ok('㉚ 分頁容器都要在 .wrap 裡(⛔ 在外面會吃到 80px 底部 padding = 分頁頂端一大塊空白;V77.2.7 加 tabConf、V77.3.9 加 tabRod)', NEST);
...
// ⚠️ V74.9.0 map 的上限從 700 → 745:泡泡多了第三個視覺編碼(大小=成交金額),
//    而本專案自己的鐵則是「**沒有圖例的視覺編碼是不可解讀的**」(V74.3.7 / V74.1.3)
//    → 那 ~60 字買的是「大小是什麼」+「⛔ 大不等於值得買」兩句**非說不可**的話。
//    ⛔ 這不是把守門放寬去遷就版面 —— 其餘說明(為什麼用對數、尺標怎麼定)照樣留在收起的長說明裡。
ok('㊲h 🧹 看圖的三頁第一眼不可是一面文字牆(總覽 ≤745 / 明細 ≤700 / 籌碼 ≤1600 字)',
   ROT2.paneLen.map <= 745 && ROT2.paneLen.det <= 700 && ROT2.paneLen.flow <= 1600,
...
// ⚠️ V74.7.5 起**面積會跟著時間軸動了**(採礦端開始存每日成交額)→ 舊斷言(釘「不會回放」)已過期。
//   ⭐ 改成釘**用意**:面積現在到底在做什麼,兩種狀態都必須說得出來,⛔ 不可靜默。
ok('🎞️①b 🚨 磚面積在做什麼必須寫在卡上(會動 / 還沒有歷史暫時固定 —— ⛔ 兩種都要說)',
   /面積與顏色都會動/.test(T.tree.note) || /還沒讀到每日成交額歷史/.test(T.tree.note),
   T.tree.note.slice(0, 200));
