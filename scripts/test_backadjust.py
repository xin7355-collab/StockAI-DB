...
# ⑦ V74.9.4 倍率表補齊 2~10 —— 實測 0052 是**精確的 1/7** 反分割,而舊表只有 2/3/4/5/10
#    ⛔ 這裡刻意用真實數字(245.30 → 35.04),⛔ 不是自己編一個剛好的例子
_r7 = mk([245.30, 243.0, 247.5, 35.04, 34.5, 35.8])
_o7 = closes(B(_r7))
ok('⑦ 1/7 反分割要被認出來(0052 的真實案例:245.30 → 35.04)',
   abs(_o7[0] - 245.30 / 7) < 0.02 and _o7[-1] == 35.80, str(_o7))
ok('⑦b 修完之後整條不再有斷崖',
   all(0.75 < _o7[k] / _o7[k - 1] < 1.33 for k in range(1, len(_o7))), str(_o7))
for _k in (6, 7, 8, 9):
    _rr = mk([100.0 * _k, 101.0 * _k, 99.0 * _k, 100.0, 101.0])
    _oo = closes(B(_rr))
    ok(f'⑦c ×{_k} 分割認得出來(補齊 2~10 之後)', abs(_oo[0] - 100.0) < 0.02, str(_oo))

# ⑦d 🚨 補了倍率**不可以變得比較鬆**:殘差超出漲跌停的一律不認
#     實測那批單日斷崖裡,÷2 殘差 10~20% 的(1808/6734/6885)本來就不該被修 —— 那可能是
#     「減資 + 當天大跌」也可能是髒資料,⛔ 硬修會製造假價格。
_rNo = mk([100.0, 100.0, 100.0, 42.03, 42.0])      # ÷2 殘差 15.9%(6734 的真實比值)
ok('⑦d 🚨 殘差超出漲跌停(÷2 差 15.9%)→ ⛔ 仍然不認(補倍率不可以放寬判準)',
   closes(B(_rNo))[0] == 100.0, str(closes(B(_rNo))))
# ⑦e ⚠️ 補齊倍率的**真實代價**(⭐ 這是算出來才發現的,⛔ 別以為殘差門檻到處都管用):
#   相鄰倍率的間隙 6→7 只有 16.7%、9→10 只有 11.1%,而殘差容許是漲跌停 ±10%
#   → **比值 ≥5 的區域幾乎沒有空隙了**,那裡的殘差門檻形同虛設。
#   ⭐ 但那可以接受:單日比值 ≥5 物理上不可能是真實漲跌(漲跌停 ±10%),
#     不是公司行為就是髒資料 —— 而髒資料由下面 ⑦f 那條守住。
#   ⛔ 低倍率區(2~3)的空隙還在,仍然擋得住:
_rNo2 = mk([100.0, 100.0, 100.0, 250.0, 252.0])    # ×2.5:離 ×2 差 25%、離 ×3 差 16.7% → 兩邊都不認
ok('⑦e ⛔ 2~3 之間湊不到整數倍的仍然不認(低倍率區的空隙還在)',
   closes(B(_rNo2))[0] == 100.0, str(closes(B(_rNo2))))

# ⑦f 🚨 髒資料(單日 ×N、隔天又 ÷N 回來)⛔ 不可以污染**前面**的列
#   實測 `data/3114.json` 2025-04-25:20.90 → 2118.96 → 21.57(×101 隔天回來)= 單日資料錯誤,
#   ⛔ 不是分割。補齊倍率之後最該擔心的就是這種東西被誤認 → 直接釘住:
#   就算中間那根被動到,**它前面的歷史價格必須一個都沒變**(否則等於用一根髒資料改寫整段歷史)。
_rDirty = mk([50.0, 51.0, 49.0, 350.0, 50.5, 51.0])      # 第 4 根是 ×7 的髒資料,隔天回來
_oD = closes(B(_rDirty))
ok('⑦f 🚨 單日髒資料(×7 隔天回來)⛔ 不可以改寫前面的歷史價格',
   _oD[0] == 50.0 and _oD[1] == 51.0 and _oD[2] == 49.0, str(_oD))
ok('⑦f2 而且後面那幾根(真實價)也不准被動', _oD[-1] == 51.0 and _oD[-2] == 50.5, str(_oD))
# ── ④ 邊界:空/太短/壞值不可 throw ────────────────────────────────────
