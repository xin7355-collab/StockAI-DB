    ...
    analysts: [
        { k: 'gooaye', n: '股癌 Gooaye', tag: '🎧 Podcast', src: 'Podcast RSS', error: null, fresh: 2,
          items: [
              ...
              { t: '大盤到底崩不崩', u: 'https://x/2', d: '2026-08-02', kind: 'podcast', csrc: 'ad', syms: [] },
          ] },
        { k: 'kuo_zhe_rong', n: '郭哲榮分析師', tag: '📺 YouTube', src: 'Google News 搜尋「郭哲榮」', fresh: 0,
          error: '本輪沒抓到新的(YouTube 全部候選 handle 都解析不到);以下是先前存下來的 1 則',
          items: [{ t: '郭哲榮:破四萬我就買 0050', u: 'https://x/3', d: '2026-08-01', kind: 'news', csrc: 'headline',
                    syms: [{ s: '0050', n: '0050', px: 90, pxd: '2026-08-01', mkt: 23000, pxn: 99, mktn: 24150 }] }] },
    ],
};
...
ok('⑤ ⭐⭐ 必須老實說逐字稿拿不到(⛔ 不可讓人以為有)', /逐字稿.{0,12}拿不到/.test(R.txt), R.txt.slice(-600));
...
// ⭐ V72.7.4 探針實測後:兩種「沒有摘要」要分開講(⛔ 都寫「抓不到」等於沒說)
ok('⑤b ⭐ 媒體報導 → 說「標題就是重點」(⛔ 不是「抓不到」)',
   /媒體幫他摘要好了/.test(R.txt) && !/內容抓不到/.test(R.txt), R.txt.slice(0, 800));
ok('⑤b ⭐ 節目說明整段是廣告 → 誠實說「不公開當集大綱」',
   /不公開當集大綱/.test(R.txt), R.txt.slice(0, 800));
ok('⑤b ⭐⭐ ETF 標的要抽得到(name_map 只有上市櫃公司,一檔 ETF 都沒有)',
   /0050/.test(R.txt), R.txt.slice(0, 800));
ok('⑤b ⛔ ETF 不可顯示成「0050 0050」', !/0050\s*0050/.test(R.txt), R.txt.slice(0, 800));
