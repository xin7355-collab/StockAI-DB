...
// ── ⑤ 連量偵測:V73.6.0 降級 → **V74.2.0 整張刪除** ──
//   ⚠️ 這一組原本釘「要推進 detail 摺疊區」= 釘住 V73.6.0 那版的**實作**,
//      而 V74.2.0 已依使用者要求(「沒用的就刪除」)把整張卡移除 → 那幾條從此永遠紅。
//   ⭐ 改成釘**用意**:刪掉可以,⛔ 但不可以靜默刪 ——
//      使用者會以為沒這回事,跑去別的地方學了再回來問(`_SIGNAL_EDGE` 對 C 級的同一個原則)。
{
    const i = src.indexOf('🔴 盤中連量偵測」卡已刪除');
    ok('⑤ ⛔ 卡刪掉了,但原地要留墓碑說明(⛔ 不可靜默消失)', i > 0, '找不到 V74.2.0 的刪除註解');
    const blk = i > 0 ? src.slice(i - 200, i + 900) : '';
    ok('⑤b 墓碑要寫出「兩次實測都不成立」', /兩次實測都不成立/.test(blk), blk.slice(0, 200));
    ok('⑤c ⭐ 要引用兩次實測的數字(⛔ 不可只寫「沒用」)',
       /\+0\.14pp/.test(blk) && /18~32%/.test(blk) && /49~52%/.test(blk), blk.slice(0, 400));
    ok('⑤d ⭐ 偵測器本身要保留(signal_backtest 的母體要用),只拆顯示',
       /`_detectVolStreak` \*\*保留\*\*/.test(blk) && /_detectVolStreak\(cs\)/.test(src), blk.slice(400, 900));
    ok('⑤e ⛔ 不可留下沒人用的 _vst 變數', !/_vst/.test(src), '');
    // 🧹 V74.5.1 雜訊清單 + V74.8.8 黑名單透明化:刪掉的東西要查得到「為什麼」
    ok('⑤f ⭐ 要進得了雜訊清單 `_NOISE_GONE`(使用者查得到為什麼刪)',
       /盤中連量偵測', 'V74\.2\.0 刪除'/.test(src), '');
    ok('⑤g ⭐ 黑名單也要擋,而且附數字與來源探針',
       /re: \/連量\/[\s\S]{0,200}volstall\/volseq_probe/.test(src), '');
}
