...
# ② 期貨落後一天 → V73.7.8 起改成「把現貨對齊到期貨那一天」再算
#    ⚠️ 2026-09-09 改過這條:原本釘「跨日一律回 None」——那是**當時的實作**,
#    而 V73.7.8 已改成對齊(真因:官方每日期貨行情要收盤後才更新 → 盤中跑一定落後 1 天,
#    舊守門每次都擋掉,實測 36 天裡只有 18 天有值)。
#    ⭐ 所以這條改成釘**用意**:真正不可以發生的是
#       「期貨 41,613(07/28)配現貨 40,039(07/29)硬減 = +1,574 點的假正價差」。
back, err = run(41613.0, '2026-07-28')
assert err is None, f'② 對齊得到期貨那天就不該回錯誤,實際 err={err}'
assert back == 10.0, f'② 應對齊到 07/28(41613−41603=+10),實際 {back}'
assert back != 1574.0, '② 🚨 絕不可拿跨日的兩條腿硬減(那正是 07/30 那次的假正價差)'
_legs = mm._LAST_BACK_LEGS
assert _legs['taiex_date'] == '2026-07-28' and _legs['taifex_fut_date'] == '2026-07-28', (
    f"② 🚨 taiex_date 必須跟著對齊到期貨那天,否則就是「昨天的價差配今天的日期」"
    f"(陷阱 #34:顯示不該相信的數字比空白更危險),實際 {_legs}")
print(f'✅ ② 期貨(07/28)落後 → 現貨對齊到 07/28,價差 {back:+.0f} 點・標 {_legs["taiex_date"]}')

# ②b 對齊失敗(期貨那天**不在** ^TWII 歷史裡)→ 仍要回 None + 講原因
#     ⭐ 這才是原本 ② 真正想守的東西,⛔ 別讓它跟著過期的斷言一起被拿掉
(DD / '^TWII.json').write_text(json.dumps(
    [{'date': '2026/07/29', 'close': 40039.18}]), encoding='utf-8')
back, err = run(41613.0, '2026-07-28')
assert back is None, f'②b 對齊不到就⛔ 不可硬算,實際 {back}'
assert err and '不同交易日' in err and '2026-07-28' in err, f'②b 原因要講清楚,實際:{err}'
print(f'✅ ②b 期貨那天不在歷史裡 → 回 None 並說明:{err}')
seed_spot('2026/07/29', 40039.18)   # ⚠️ 還原,否則下面的 ③④ 會拿到只有一天的歷史
